...
template <int Dim, CommonParameterBlockType Type> class CommonParameterBlock : public ParameterBlock
{
  public:
    ...
    virtual const Eigen::VectorXd &estimate() const
    {
        return estimate_;
    }
    ...
    {
        Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> identity(jacobian, Dim, Dim);
    ...
    {
#define PRINT_MAP(x)                                                                                                   \
    if (Type == CommonParameterBlockType::x)                                                                           \
        return std::string(#x) + "ParameterBlock";
        PRINT_MAP(Position);
        PRINT_MAP(Velocity);
        PRINT_MAP(Clock);
        PRINT_MAP(Ifb);
