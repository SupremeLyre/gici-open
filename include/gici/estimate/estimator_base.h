...
class EstimatorBase
{
  public:
    ...
    virtual void setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
    {
    }
    ...
    {
        return oldestState().timestamp;
    }

    // Get coordinate
    GeoCoordinatePtr getCoordinate()
    {
        return coordinate_;
    }
    ...
    virtual bool getPoseEstimateAt(const double timestamp, Transformation &T_WS);
    ...
    virtual bool getSpeedAndBiasEstimateAt(const double timestamp, SpeedAndBias &speed_and_bias);
    ...
    virtual bool getCovarianceAt(const double timestamp, Eigen::Matrix<double, 15, 15> &covariance);

    // Get estimator status
    EstimatorStatus getStatus()
    {
        return status_;
    }
  ...
  protected:
    ...
    virtual void convertStateAndCovarianceToBody(Transformation *T_WS, SpeedAndBias *speed_and_bias,
    ...
    // Get current state
    inline State &curState()
    {
        return getCurrent(states_);
    }

    // Get last state
    inline State &lastState()
    {
        return getLast(states_);
    }

    // Get oldest state
    inline State &oldestState()
    {
        return getOldest(states_);
    }

    // Get latest state. Latest states are not always pushed to back.
    inline virtual State &latestState()
    {
        return curState();
    }
