...
class BackendId
{
  public:
    ...
    // Set bits
    template <typename T> inline static uint64_t setBits(T data, int start, int end)
    {
        int length = end - start + 1;
    ...
    // Reset some bits
    template <typename T> inline static uint64_t resetBits(uint64_t id, T data, int start, int end)
    {
        int length = end - start + 1;
        uint32_t bits = static_cast<uint32_t>(data);
        uint64_t out_id = 0;
    ...
    {
        if (type == IdType::cPose || type == IdType::cExtrinsics || type == IdType::cLandmark)
            return SensorType::Camera;
        if (type == IdType::ImuStates)
            return SensorType::IMU;
        if (type == IdType::gAmbiguity || type == IdType::gClock || type == IdType::gExtrinsics ||
            type == IdType::gFrequency || type == IdType::gIonosphere || type == IdType::gPose ||
            type == IdType::gPosition || type == IdType::gTroposphere || type == IdType::gVelocity ||
            type == IdType::gIfb)
    ...
    inline static int32_t adjustBundleId(int32_t bundle_id, IdType type)
    {
        ...
        while (out < RANGE_BUNDLE_ID_MIN)
            out += length;
        while (out > RANGE_BUNDLE_ID_MAX)
            out -= length;
        return out;
    }
    ...
    {
        CHECK(type() != IdType::cLandmark) << "Landmarks do not have a bundle ID.";
...
inline BackendId createNFrameId(int32_t bundle_id)
{
    ...
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::cPose), BITS_BUNDLEID) |
...
{
    CHECK_GE(bundle_id, 0);
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gPosition), BITS_BUNDLEID) |
...
{
    CHECK_GE(bundle_id, 0);
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gVelocity), BITS_BUNDLEID) |
...
{
    CHECK_GE(bundle_id, 0);
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gPose), BITS_BUNDLEID) |
                     BackendId::setBits(IdType::gPose, BITS_IDTYPE));
}

inline BackendId createGnssClockId(char system, int32_t bundle_id)
{
    CHECK_GE(bundle_id, 0);
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gClock), BITS_BUNDLEID) |
                     BackendId::setBits(system, BITS_GNSS_SYSTEM) | BackendId::setBits(IdType::gClock, BITS_IDTYPE));
}

inline BackendId createGnssFrequencyId(char system, int32_t bundle_id)
{
    CHECK_GE(bundle_id, 0);
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gFrequency), BITS_BUNDLEID) |
                     BackendId::setBits(system, BITS_GNSS_SYSTEM) |
                     BackendId::setBits(IdType::gFrequency, BITS_IDTYPE));
}

inline BackendId createGnssAmbiguityId(std::string prn, int phase_id, int32_t bundle_id)
{
    CHECK_GE(bundle_id, 0);
    CHECK_GE(phase_id, 0);
    char system = prn[0];
    int prn_number = atoi(prn.substr(1, 2).data());
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gAmbiguity), BITS_BUNDLEID) |
                     BackendId::setBits(system, BITS_GNSS_SYSTEM) | BackendId::setBits(prn_number, BITS_GNSS_PRN) |
...
{
    CHECK_GE(bundle_id, 0);
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gTroposphere), BITS_BUNDLEID) |
...
inline BackendId createGnssIonosphereId(std::string prn, int32_t bundle_id)
{
    ...
    char system = prn[0];
    int prn_number = atoi(prn.substr(1, 2).data());
    return BackendId(BackendId::setBits(BackendId::adjustBundleId(bundle_id, IdType::gIonosphere), BITS_BUNDLEID) |
                     BackendId::setBits(system, BITS_GNSS_SYSTEM) | BackendId::setBits(prn_number, BITS_GNSS_PRN) |
                     BackendId::setBits(IdType::gIonosphere, BITS_IDTYPE));
}

inline BackendId createGnssIfbId(char system, int code_id, std::string prn = "")
{
    CHECK_GE(code_id, 0);
    int prn_number = 0;
    if (prn != "")
    ...
    return BackendId(BackendId::setBits(0, BITS_BUNDLEID) | BackendId::setBits(system, BITS_GNSS_SYSTEM) |
                     BackendId::setBits(prn_number, BITS_GNSS_PRN) | BackendId::setBits(code_id, BITS_GNSS_CODEID) |
...
inline BackendId changeIdType(BackendId id, IdType type, size_t cam_index = 0)
{
    ...
    CHECK(type != IdType::cLandmark);
    CHECK(cam_index == 0 || type == IdType::cExtrinsics || type == IdType::gVelocity || type == IdType::gExtrinsics ||
          type == IdType::gTroposphere);
...
{
    CHECK(BackendId::getBits(lhs.asInteger(), BITS_IDTYPE) == static_cast<uint32_t>(IdType::gAmbiguity));
    CHECK(BackendId::getBits(rhs.asInteger(), BITS_IDTYPE) == static_cast<uint32_t>(IdType::gAmbiguity));
...
{
    CHECK(BackendId::getBits(lhs.asInteger(), BITS_IDTYPE) == static_cast<uint32_t>(IdType::gIonosphere));
    CHECK(BackendId::getBits(rhs.asInteger(), BITS_IDTYPE) == static_cast<uint32_t>(IdType::gIonosphere));
...
struct State
{
    ...
    bool valid()
    {
        return (id != BackendId(0) && timestamp != 0.0);
    }
    ...
    {
        for (auto it = overlaps.lower_bound(state.id_in_graph); it != overlaps.upper_bound(state.id_in_graph); it++)
        {
        ...
        if (overlaps.count(state.id_in_graph) == 1)
            overlaps.erase(state.id_in_graph);
    }
...
class EstimatorDataCluster
{
  public:
    ...
    EstimatorDataCluster(const GnssMeasurement &data)
        : gnss(std::make_shared<GnssMeasurement>(data)), gnss_role(data.role), tag(data.tag), timestamp(data.timestamp)
    {
    }

    EstimatorDataCluster(const ImuMeasurement &data, const ImuRole &role, const std::string &tag)
        : imu(std::make_shared<ImuMeasurement>(data)), imu_role(role), tag(tag), timestamp(data.timestamp)
    {
    }

    EstimatorDataCluster(const std::shared_ptr<cv::Mat> &data, const CameraRole &role, const std::string &tag,
                         const double time)
        : image(data), image_role(role), tag(tag), timestamp(time)
    {
    }

    EstimatorDataCluster(const FrameBundlePtr &data, const std::string &tag)
        : frame_bundle(data), timestamp(data->getMinTimestampSeconds()), tag(tag)
    {
    }

    EstimatorDataCluster(const Solution &data, const SolutionRole &role, const std::string &tag)
        : solution(std::make_shared<Solution>(data)), solution_role(role), timestamp(data.timestamp), tag(tag)
    {
    }
