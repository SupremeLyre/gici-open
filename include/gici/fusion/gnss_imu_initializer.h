...
class GnssImuInitializer : public MultisensorInitializerBase, public GnssLooseEstimatorBase, public ImuEstimatorBase
{
  public:
    ...
    bool arrangeToEstimator(const int window_length, const std::shared_ptr<MarginalizationError> &marginalization_error,
                            std::deque<State> &states, ceres::ResidualBlockId &marginalization_residual_id,
                            BackendId &gnss_extrinsics_id, std::deque<GnssSolution> &gnss_solution_measurements,
  ...
  protected:
    ...
    void putMeasurementAndStateToGraph(const Eigen::Quaterniond &q_WS_0, const SpeedAndBias &speed_and_bias_0);
