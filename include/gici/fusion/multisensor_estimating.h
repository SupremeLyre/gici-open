...
class MultiSensorEstimating : public EstimatingBase
{
  ...
  private:
    ...
    inline bool estimatorTypeContains(SensorType sensor_type, EstimatorType estimator_type)
    {
        if (sensor_type == SensorType::GNSS)
        {
            return (
                estimator_type == EstimatorType::Spp || estimator_type == EstimatorType::Sdgnss ||
                ...
                estimator_type == EstimatorType::SppImuTc || estimator_type == EstimatorType::DgnssImuTc ||
                estimator_type == EstimatorType::RtkImuTc || estimator_type == EstimatorType::PppImuTc ||
                estimator_type == EstimatorType::GnssImuCameraSrr || estimator_type == EstimatorType::SppImuCameraRrr ||
                estimator_type == EstimatorType::DgnssImuCameraRrr ||
                estimator_type == EstimatorType::RtkImuCameraRrr || estimator_type == EstimatorType::PppImuCameraRrr);
        }
        else if (sensor_type == SensorType::IMU)
        {
            return (estimator_type == EstimatorType::GnssImuLc || estimator_type == EstimatorType::SppImuTc ||
                    estimator_type == EstimatorType::DgnssImuTc || estimator_type == EstimatorType::RtkImuTc ||
                    estimator_type == EstimatorType::PppImuTc || estimator_type == EstimatorType::GnssImuCameraSrr ||
    ...
    {
        return (estimator_type == EstimatorType::GnssImuCameraSrr || estimator_type == EstimatorType::SppImuCameraRrr ||
                estimator_type == EstimatorType::DgnssImuCameraRrr ||
                estimator_type == EstimatorType::RtkImuCameraRrr || estimator_type == EstimatorType::PppImuCameraRrr);
    }
