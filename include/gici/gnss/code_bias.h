...
class CodeBias
{
  public:
    ...
    void setDcb(const std::string prn, const int code, const int code_base, const double value);

    // Set Time Group Delay (TGD) or Inter-System Corrections (ISC)
    void setTgdIsc(const std::string prn, const TgdIscType type, const double value);

    // Set zero-difference code bias
    void setZdcb(const std::string prn, const int code, const double value);
    ...
    double getCodeBias(const std::string prn, const int code, const bool accept_coarse = true);

    // Get base frequencies
    const BaseFrequencies &getBase() const
    {
        return bases_;
    }
  ...
  private:
    ...
    void arrangeAllSourceDcbs(std::multimap<std::string, Dcb> &all_source_dcbs, BiasMap &biases,
