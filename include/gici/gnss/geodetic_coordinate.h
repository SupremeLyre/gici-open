...
class GeoCoordinate
{
  public:
    ...
    GeoCoordinate(const Eigen::Vector3d &position_zero, const GeoType type);
    GeoCoordinate() : lla_zero_(Eigen::Vector3d::Zero()), lla_zero_setted_(false)
    {
    }
    ~GeoCoordinate()
    {
    ...
    void setZero(const Eigen::Vector3d &position, const GeoType type);

    // Check if zero position was setted
    bool isZeroSetted()
    {
        return lla_zero_setted_;
    }

    // Convert coordinate
    Eigen::Vector3d convert(const Eigen::Vector3d &position, const GeoType in_type, const GeoType out_type);

    // Rotate coordinate
    Eigen::Vector3d rotate(const Eigen::Vector3d &position, const GeoType in_type, const GeoType out_type);

    // Convert covariance
    Eigen::Matrix3d convertCovariance(const Eigen::Matrix3d &cov, const GeoType in_type, const GeoType out_type);
