...
class GnssEstimatorBase : public virtual EstimatorBase
{
  public:
    ...
    GnssEstimatorBase(const GnssEstimatorBaseOptions &options, const EstimatorBaseOptions &base_options);
    ~GnssEstimatorBase();

    // Add GNSS measurements and state for undifferenced estimators
    virtual bool addGnssMeasurementAndState(const GnssMeasurement &measurement)
    {
        return false;
    }

    // Add GNSS measurements and state for differenced estimators
    virtual bool addGnssMeasurementAndState(const GnssMeasurement &measurement_rov,
    ...
    {
        if (ionosphere_states_.size() == 0)
            return;
        eraseIonosphereParameterBlocks(curIonosphereState(), false);
    }

    // Get solution status
    inline GnssSolutionStatus getSolutionStatus()
    {
        return lastState().status;
    }

    // Get number of satellite
    inline int getNumberSatellite()
    {
        return num_satellites_;
    }

    // Get differential age
    inline int getDifferentialAge()
    {
        return differential_age_;
    }

    // Check if we have velocity estimate
    inline bool hasVelocityEstimate()
    {
        return has_velocity_estimate_;
    }

  protected:
    // Add GNSS position block to graph
    BackendId addGnssPositionParameterBlock(const int32_t id, const Eigen::Vector3d &prior);

    // Add GNSS velocity block to graph
    BackendId addGnssVelocityParameterBlock(const int32_t id, const Eigen::Vector3d &prior = Eigen::Vector3d::Zero());

    // Add GNSS extrinsics block to graph
    BackendId addGnssExtrinsicsParameterBlock(const int32_t id, const Eigen::Vector3d &t_SR_S_prior);

    // Add clock blocks to graph
    void addClockParameterBlocks(const GnssMeasurement &measurement, const int32_t id, int &num_valid_system,
    ...
    void addSdClockParameterBlocks(const GnssMeasurement &measurement_rov, const GnssMeasurement &measurement_ref,
                                   const GnssMeasurementSDIndexPairs &index_pairs, const int32_t id,
    ...
    void addIfbParameterBlocks(GnssMeasurement &measurement, const int32_t id);

    // Add frequency blocks to graph
    void addFrequencyParameterBlocks(const GnssMeasurement &measurement, const int32_t id, int &num_valid_system,
                                     const std::map<char, double> &prior = std::map<char, double>(),
                                     bool use_single_frequency = false);
    ...
    void addIonosphereParameterBlocks(const GnssMeasurement &measurement, const int32_t id, IonosphereState &state);

    // Add ambiguity blocks to graph
    void addAmbiguityParameterBlocks(const GnssMeasurement &measurement, const int32_t id, AmbiguityState &state);

    // Add single-differenced ambiguity blocks to graph
    void addSdAmbiguityParameterBlocks(const GnssMeasurement &measurement_rov, const GnssMeasurement &measurement_ref,
                                       const GnssMeasurementDDIndexPairs &index_pairs, const int32_t id,
    ...
    void computeIonosphereDelay(GnssMeasurement &measurement, bool use_single_frequency = false);

    // Add position residual block to graph
    void addGnssPositionResidualBlock(const State &state, const Eigen::Vector3d &position, const double std);

    // Add velocity residual block to graph
    void addGnssVelocityResidualBlock(const State &state, const Eigen::Vector3d &velocity, const double std);

    // Add pseudorange residual blocks to graph
    void addPseudorangeResidualBlocks(const GnssMeasurement &measurement, const State &state, int &num_valid_satellite,
                                      bool use_single_frequency = false);

    // Add single-differenced pseudorange residual block to graph
    void addSdPseudorangeResidualBlocks(const GnssMeasurement &measurement_rov, const GnssMeasurement &measurement_ref,
                                        const GnssMeasurementSDIndexPairs &index_pairs, const State &state,
                                        int &num_valid_satellite, bool use_single_frequency = false);

    // Add double-differenced pseudorange residual block to graph
    void addDdPseudorangeResidualBlocks(const GnssMeasurement &measurement_rov, const GnssMeasurement &measurement_ref,
                                        const GnssMeasurementDDIndexPairs &index_pairs, const State &state,
                                        int &num_valid_satellite, bool use_single_frequency = false);

    // Add phaserange residual blocks to graph
    void addPhaserangeResidualBlocks(const GnssMeasurement &measurement, const State &state);

    // Add double-differenced phaserange residual blocks to graph
    void addDdPhaserangeResidualBlocks(const GnssMeasurement &measurement_rov, const GnssMeasurement &measurement_ref,
                                       const GnssMeasurementDDIndexPairs &index_pairs, const State &state);

    // Add doppler residual blocks to graph
    void addDopplerResidualBlocks(const GnssMeasurement &measurement, const State &state, int &num_valid_satellite,
                                  bool use_single_frequency = false,
                                  const Eigen::Vector3d &angular_velocity = Eigen::Vector3d::Zero());

    // Add troposphere residual block to graph
    void addTroposphereResidualBlock(const BackendId &tropo_id, const double value, const double std);

    // Add ionosphere residual block to graph
    void addIonosphereResidualBlock(const BackendId &iono_id, const double value, const double std);

    // Add ambiguity residual block to graph
    void addAmbiguityResidualBlock(const BackendId &amb_id, const double value, const double std);

    // Add relative position block to graph
    void addRelativePositionResidualBlock(const State &last_state, const State &cur_state);

    // Add relative position and velocity block to graph
    void addRelativePositionAndVelocityResidualBlock(const State &last_state, const State &cur_state);

    // Add relative ISB block to graph
    void addRelativeIsbResidualBlock(const State &last_state, const State &cur_state);

    // Add relative frequency block to graph
    void addRelativeFrequencyResidualBlock(const State &last_state, const State &cur_state);

    // Add relative troposphere block to graph
    void addRelativeTroposphereResidualBlock(const State &last_state, const State &cur_state);

    // Add relative ionosphere block to graph
    void addRelativeIonosphereResidualBlock(const IonosphereState &last_state, const IonosphereState &cur_state);

    // Add relative ambiguity block to graph
    void addRelativeAmbiguityResidualBlock(GnssMeasurement &last_measurement, GnssMeasurement &cur_measurement,
    ...
    size_t rejectPseudorangeOutlier(const State &state, bool reject_one = false);

    // Reject pseudorange outlier together with corresponding ambiguity parameter
    size_t rejectPseudorangeOutlier(const State &state, AmbiguityState &ambiguity_state, bool reject_one = false);

    // Reject phaserange outlier
    size_t rejectPhaserangeOutlier(const State &state, AmbiguityState &ambiguity_state, bool reject_one = false);

    // Reject doppler outlier
    size_t rejectDopplerOutlier(const State &state, bool reject_one = false);
    ...
    void eraseRelativePositionResidualBlock(const State &last_state, const State &cur_state);

    // Erase relative position and velocity residual blocks
    void eraseRelativePositionAndVelocityBlock(const State &last_state, const State &cur_state);

    // Erase relative frequency residual blocks
    void eraseRelativeFrequencyBlock(const State &last_state, const State &cur_state);

    // Erase relative troposphere residual blocks
    void eraseRelativeTroposphereResidualBlock(const State &last_state, const State &cur_state);

    // Erase relative ionosphere residual blocks
    void eraseRelativeIonosphereResidualBlock(const IonosphereState &last_state, const IonosphereState &cur_state);

    // Erase relative ambiguity residual blocks
    void eraseRelativeAmbiguityResidualBlock(const AmbiguityState &last_state, const AmbiguityState &cur_state);

    // Convert from estimated states (in ENU) to body states
    void convertStateAndCovarianceToBody(Transformation *T_WS, SpeedAndBias *speed_and_bias,
    ...
    inline void updateGdop(const GnssMeasurement &measurement_rov, const GnssMeasurementSDIndexPairs &indexes)
    {
        gdop_ = gnss_common::computeDops(measurement_rov, indexes, gnss_base_options_.common).x();
    }
    inline void updateGdop(const GnssMeasurement &measurement_rov, const GnssMeasurementDDIndexPairs &indexes)
    {
        gdop_ = gnss_common::computeDops(measurement_rov, indexes, gnss_base_options_.common).x();
    }
    ...
    {
        if (gdop_ > gnss_base_options_.good_observation_max_gdop)
            return false;
        if (num_satellites_ < gnss_base_options_.good_observation_min_num_satellites)
            return false;
        return true;
    }
    ...
    inline std::deque<GnssMeasurement>::iterator gnssMeasurementAt(const double timestamp)
    {
        auto it = gnss_measurements_.begin();
        for (; it != gnss_measurements_.end(); it++)
        {
            if (checkEqual(it->timestamp, timestamp))
                break;
        }
        return it;
    }
    ...
    inline std::deque<std::pair<GnssMeasurement, GnssMeasurement>>::iterator gnssMeasurementPairAt(
        const double timestamp)
    {
        auto it = gnss_measurement_pairs_.begin();
        for (; it != gnss_measurement_pairs_.end(); it++)
        {
            if (checkEqual(it->first.timestamp, timestamp))
                break;
        }
        return it;
    }
    ...
    inline std::deque<AmbiguityState>::iterator ambiguityStateAt(const double timestamp)
    {
    ...
    inline std::deque<IonosphereState>::iterator ionosphereStateAt(const double timestamp)
    {
    ...
    inline State &latestGnssState()
    {
        for (auto it = states_.rbegin(); it != states_.rend(); it++)
        {
            ...
            if (!state.valid())
                continue;
            if (state.id.type() == IdType::gPose || state.id.type() == IdType::gPosition)
                return state;
        }
        return null_state_;
    }
    ...
    inline bool checkSufficientSatellite(const int num_valid_satellite, const int num_valid_system, bool log = true)
    {
        int base = is_use_phase_ ? 4 : 3;
        int thr = num_valid_system + base + gnss_base_options_.common.min_num_satellite_redundancy;
        if (num_valid_satellite < thr)
        {
            if (!log)
                return false;
            LOG(INFO) << "Insufficient satellites! We need at least " << thr << " satellites, "
    ...
    inline bool checkObservationValid(const GnssMeasurement &measurement, const GnssMeasurementIndex &index,
                                      const bool is_doppler = false)
    {
        if (is_doppler)
        {
            if (!gnss_common::checkObservationValid(measurement, index, ObservationType::Doppler,
        ...
        if (is_use_phase_ && !gnss_common::checkObservationValid(measurement, index, ObservationType::Phaserange,
