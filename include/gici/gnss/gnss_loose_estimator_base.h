...
class GnssLooseEstimatorBase : public virtual EstimatorBase
{
  public:
    ...
    GnssLooseEstimatorBase(const GnssLooseEstimatorBaseOptions &options, const EstimatorBaseOptions &base_options);
    ~GnssLooseEstimatorBase();

    // Add GNSS solution measurement and state
    virtual bool addGnssSolutionMeasurementAndState(const GnssSolution &measurement)
    {
        return false;
    }

    // Convert solution to GNSS solution
    GnssSolution convertSolutionToGnssSolution(const Solution &solution, const SolutionRole &role);

    // Compute coarse velocity from relative position
    bool computeCoarseVelocityFromPosition(const GnssSolution &last_gnss_solution,
                                           const GnssSolution &cur_gnss_solution, Eigen::Vector3d &velocity);
    ...
    {
        return lastGnssSolution().num_satellites;
    }

    // Get differential age
    inline int getDifferentialAge()
    {
        return lastGnssSolution().differential_age;
    }
  ...
  protected:
    ...
    void addGnssPositionResidualBlock(const GnssSolution &measurement, const State &state);

    // Add velocity block to graph
    void addGnssVelocityResidualBlock(const GnssSolution &measurement, const State &state,
                                      const Eigen::Vector3d &angular_velocity);

    // Add initial GNSS extrinsics error
    void addGnssExtrinsicsResidualBlock(const BackendId &gnss_extrinsics_id, const Eigen::Vector3d &t_SR_S_prior,
