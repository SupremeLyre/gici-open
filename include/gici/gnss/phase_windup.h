...
class PhaseWindup
{
  public:
    ...
    // Get phase-windup
    double get(const double time, const std::string &prn, const Eigen::Vector3d &satellite_position,
               const Eigen::Vector3d &receiver_position);

  protected:
    // Phase windup model
    void windupcorr(const double time, const double *rs, const double *rr, double *phw);
