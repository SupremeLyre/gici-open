...
class ImuEstimatorBase : public virtual EstimatorBase
{
  public:
    ...
    ImuEstimatorBase(const ImuEstimatorBaseOptions &options, const EstimatorBaseOptions &base_options);
    ~ImuEstimatorBase();
    ...
    static inline Eigen::Vector3d rotateImuToBody(const Eigen::Vector3d &p_I, const ImuEstimatorBaseOptions &options)
    {
        Eigen::Quaterniond q_BI = eulerAngleToQuaternion(options.body_to_imu_rotation * D2R);
        return q_BI * p_I;
    }

    static inline Eigen::Vector3d rotateStdImuToBody(const Eigen::Vector3d &std_p_I,
    ...
    static inline Transformation rotateImuToBody(const Transformation &T_IX, const ImuEstimatorBaseOptions &options)
    {
        Eigen::Quaterniond q_BI = eulerAngleToQuaternion(options.body_to_imu_rotation * D2R);
    ...
    // IMU integration
    bool imuIntegration(const double last_timestamp, const double timestamp, Transformation &T_WS,
    ...
    bool getMotionFromEstimateAndImuIntegration(const State &estimated_state, const double timestamp,
    ...
    bool getPoseEstimateAt(const double timestamp, Transformation &T_WS) override;

    // Get speed and bias estimate at given timestmap
    bool getSpeedAndBiasEstimateAt(const double timestamp, SpeedAndBias &speed_and_bias) override;

    // Get covariance at given timestamp
    bool getCovarianceAt(const double timestamp, Eigen::Matrix<double, 15, 15> &covariance) override;
  ...
  protected:
    ...
    void addImuSpeedAndBiasParameterBlock(const BackendId backend_id, const SpeedAndBias &prior);

    // Add pose block to graph
    void addPoseParameterBlock(const BackendId backend_id, const Transformation &T_WS_prior);
    ...
    void addImuSpeedAndBiasResidualBlock(const State &state, const SpeedAndBias &speed_and_bias, const double std_speed,
                                         const double std_bg, const double std_ba);

    // Add pose residual block to graph
    void addPoseResidualBlock(const State &state, const Transformation &T_WS, const double std_roll_pitch,
                              const double std_yaw);

    // Add yaw residual block to graph
    void addYawResidualBlock(const State &state, const double yaw, const double std_yaw);

    // Add roll and pitch residual block to graph
    void addRollPitchResidualBlock(const State &state, const Eigen::Vector2d &pitch_and_roll,
    ...
    size_t insertImuState(const double timestamp, const BackendId &backend_id,
    ...
    void downWeightImuResidualBlock(const ceres::ResidualBlockId residual_id, double factor);
