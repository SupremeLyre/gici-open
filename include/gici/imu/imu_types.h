...
struct ImuMeasurement
{
    ...
    ImuMeasurement(const double timestamp, const Eigen::Vector3d &angular_velocity,
                   const Eigen::Vector3d &linear_acceleration)
        : timestamp(timestamp), angular_velocity(angular_velocity), linear_acceleration(linear_acceleration)
    {
    }
};
