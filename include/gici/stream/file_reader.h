...
class FileReaderBase
{
  public:
    ...
    bool get(DataClusterPair &data, const double timestamp = INFINITY);
...
{
  public:
    ImuTextReader(const YAML::Node &node) : FileReaderBase(node)
    {
    }
    ~ImuTextReader()
    {
...
{
  public:
    Sp3FileReader(const YAML::Node &node) : FileReaderBase(node)
    {
    }
    ~Sp3FileReader()
    {
...
{
  public:
    ClkFileReader(const YAML::Node &node) : FileReaderBase(node)
    {
    }
    ~ClkFileReader()
    {
