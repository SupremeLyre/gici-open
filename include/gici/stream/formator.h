...
class DataCluster
{
  public:
    ...
    DataCluster(const Solution &data) : solution(std::make_shared<Solution>(data))
    {
    }
...
extern void updateSsr(ssr_t *ssr, std::shared_ptr<DataCluster::GNSS> &gnss_data,
                      std::vector<UpdateSsrType> type = {UpdateSsrType::Ephemeris, UpdateSsrType::CodeBias,
                                                         UpdateSsrType::PhaseBias},
...
class FormatorBase
{
  public:
    ...
    virtual int encode(const std::shared_ptr<DataCluster> &data, uint8_t *buf) = 0;

    // Get formator type
    FormatorType getType()
    {
        return type_;
    }

    // Get data handle
    std::vector<std::shared_ptr<DataCluster>> &getDataHandle()
    {
        return data_;
    }
...
class RTCM2Formator : public FormatorBase
{
  public:
    ...
    // Decode stream to data
    int decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data) override;
...
#define MAKE_FORMATOR(Formator)                                                                                        \
    inline std::shared_ptr<FormatorBase> makeFormator(Formator::Option &option)                                        \
    {                                                                                                                  \
