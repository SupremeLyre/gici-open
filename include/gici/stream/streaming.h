...
class Streaming
{
  public:
    ...
    // Get formators
    std::vector<FormatorCtrl> &getFormators()
    {
        return formators_;
    }
    ...
    void pipelineConvertCallback(const std::string &tag, const std::shared_ptr<DataCluster> &data);

    // Send solution data to output stream
    virtual void outputDataCallback(const std::string tag, const std::shared_ptr<DataCluster> &data);
    ...
    void setPipelineConvert(const std::string &input_tag, const std::string &log_tag, const PipelineConvert &pipeline)
    {
    ...
    static std::vector<Streaming *> &getObjects()
    {
        return static_this_;
    }
