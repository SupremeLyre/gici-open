...
inline Eigen::Matrix<double, 3, 3> skewSymmetric(const double w1, const double w2, const double w3)
{
    return svo::skewSymmetric(w1, w2, w3);
}

inline Eigen::Matrix<double, 3, 3> skewSymmetric(const Eigen::Ref<const Eigen::Matrix<double, 3, 1>> &w)
{
...
inline Eigen::Matrix<double, 4, 4> quaternionPlusMatrix(const Eigen::Quaternion<double> &q_AB)
{
    return svo::quaternionPlusMatrix(q_AB);
}

//! Opposite-Plus matrix for a quaternion q_AB x q_BC = oplus(q_BC) * q_AB.coeffs().
inline Eigen::Matrix<double, 4, 4> quaternionOplusMatrix(const Eigen::Quaternion<double> &q_BC)
{
