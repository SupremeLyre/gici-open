...
class EpipolarError
{
  public:
    ...
    EpipolarError(const double &sqrt_information, const Eigen::Matrix<double, 2, 1> p_1,
    ...
    template <typename T> bool operator()(const T *const t_rel, const T *const q_rel, T *e) const
    {
        ...
        tx << T(0), -t_eig(2, 0), t_eig(1, 0), t_eig(2, 0), T(0), -t_eig(0, 0), -t_eig(1, 0), t_eig(0, 0), T(0);
    ...
    template <typename T> Eigen::Matrix<T, 3, 3> quatToRotMat(Eigen::Quaternion<T> q) const
    {
    ...
    static ceres::CostFunction *Create(const double &sqrt_information, const Eigen::Matrix<double, 2, 1> &p_1,
                                       const Eigen::Matrix<double, 2, 1> &p_2)
    {
        return new ceres::AutoDiffCostFunction<EpipolarError, 1, 3, 4>(new EpipolarError(sqrt_information, p_1, p_2));
    }
