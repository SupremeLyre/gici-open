...
class FeatureHandler
{
  public:
    ...
    FeatureHandler(const FeatureHandlerOptions &options, const ImuEstimatorBaseOptions &imu_options);
    ~FeatureHandler();

    // Add images
    bool addImageBundle(const std::vector<std::shared_ptr<cv::Mat>> &imgs, const double timestamp);
    ...
    void setPoseRequestSource(PoseRequest &f)
    {
        pose_request_ = f;
    }

    // Get current map
    inline const MapPtr &getMap() const
    {
        return map_;
    }

    // Get camera bundle parameters
    inline const CameraBundle::Ptr &getNCamera() const
    {
        return cams_;
    }

    // Get camera parameters
    inline CameraPtr getCamera() const
    {
        return cams_->getCameraShared(0);
    }

    // Get feature detector
    inline const DetectorPtr &getDetector() const
    {
        return detector_;
    }

    // Get current processed frame bundle
    FrameBundlePtr getFrameBundle()
    {
        return lastFrames();
    }

    // Get all frame bundles
    std::deque<FrameBundlePtr> &getFrameBundles()
    {
        return frame_bundles_;
    }

    // Check if it is the first frame
    bool isFirstFrame()
    {
        return frame_bundles_.size() < 2;
    }
    ...
    void setGlobalScaleInitialized()
    {
