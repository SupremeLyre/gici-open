...
class FeatureMatcher
{
  public:
    ...
    struct Options
    {
        ...
        double max_epi_length_optim =
            2.0; //!< max length of epipolar line to skip epipolar search and directly go to img align
    ...
    double epi_length_pyramid_; //!< length of epipolar line segment in pixels on pyrimid level (only used for epipolar
                                //!< search)
    ...
    /// IMPORTANT! This function assumes that px_cur is already set to an estimate that is within ~2-3 pixel of the
    /// final result!
    MatchResult findMatchDirect(const Frame &ref_frame, const Frame &cur_frame, const FeatureWrapper &ref_ftr,
                                const FloatType &ref_depth, Keypoint &px_cur);

    /// Find a match by searching along the epipolar line without using any features.
    MatchResult findEpipolarMatchDirect(const Frame &ref_frame, const Frame &cur_frame, const FeatureWrapper &ref_ftr,
                                        const double d_estimate_inv, const double d_min_inv, const double d_max_inv,
                                        double &depth);

    MatchResult findEpipolarMatchDirect(const Frame &ref_frame, const Frame &cur_frame, const Transformation &T_cur_ref,
    ...
    MatchResult findEpipolarMatchWithBound(const Frame &ref_frame, const Frame &cur_frame,
                                           const FeatureWrapper &ref_ftr, const double d_estimate_inv,
                                           const double d_min_inv, const double d_max_inv, const double bound_position,
                                           const double bound_rotation, double &depth, Keypoint &image_best,
                                           int &zmssd_best);
    ...
    void scanEpipolarLine(const Frame &frame, const Eigen::Vector3d &A, const Eigen::Vector3d &B,
  ...
  public:
    ...
    MatchResult findLocalMatch(const Frame &frame, const Eigen::Ref<GradientVector> &direction, const int patch_level,
                               Keypoint &px_cur);

    /// update best zmssd, if a better one is find, return true
    bool updateZMSSD(const Frame &frame, const Eigen::Vector2i &pxi, const int patch_level,
                     const PatchScore &patch_score, int *zmssd_best);

    /// check is patch is fully within image
    bool isPatchWithinImage(const Frame &frame, const Eigen::Vector2i &pxi, const int patch_level);
    ...
    void scanEpipolarUnitPlane(const Frame &frame, const Eigen::Vector3d &A, const Eigen::Vector3d &B,
                               const Eigen::Vector3d &C, const PatchScore &patch_score, const int patch_level,
                               Keypoint *image_best, int *zmssd_best);

    /// search along the epipolar line on image plane
    /// we sample on the UNIT SPHERE and check corresponding patches on the image plane
    void scanEpipolarUnitSphere(const Frame &frame, const Eigen::Vector3d &A, const Eigen::Vector3d &B,
                                const Eigen::Vector3d &C, const PatchScore &patch_score, const int patch_level,
                                Keypoint *image_best, int *zmssd_best);
};
