...
class FeatureTracker
{
  public:
    ...
    void track(const FramePtr &ref_frame, const FramePtr &cur_frame, OccupandyGrid2D &grid,
               const Eigen::Quaterniond &q_cur_ref = Eigen::Quaterniond::Identity());

  private:
    // Predict initial point for optical flow tracking
    void predictFeatureTracking(const FramePtr &ref_frame, const FramePtr &cur_frame,
                                const Eigen::Quaterniond &q_cur_ref, const std::vector<cv::Point2f> &ref_points,
