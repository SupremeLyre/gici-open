...
class VisualEstimatorBase : public virtual EstimatorBase
{
  public:
    ...
    VisualEstimatorBase(const VisualEstimatorBaseOptions &options, const EstimatorBaseOptions &base_options);
    ~VisualEstimatorBase();

    // Set visual front-end handler.
    inline void setFeatureHandler(const std::shared_ptr<FeatureHandler> &feature_handler)
    {
  ...
  protected:
    ...
    BackendId addCameraExtrinsicsParameterBlock(const int32_t id, const Transformation &T_SC_S_prior);
    ...
    ceres::ResidualBlockId addReprojectionErrorResidualBlock(const State &state, const FramePtr &frame,
                                                             const size_t keypoint_idx);

    // Add landmark reprojection error blocks
    void addReprojectionErrorResidualBlocks(const State &state, const FramePtr &frame);

    // Add initial camera extrinsics error
    void addCameraExtrinsicsResidualBlock(const BackendId &camera_extrinsics_id, const Transformation &T_SC_S_prior,
    ...
    void eraseReprojectionErrorResidualBlock(ceres::ResidualBlockId residual_block_id);
