...
RosNodeHandle::RosNodeHandle(ros::NodeHandle &nh, const NodeOptionHandlePtr &nodes) : NodeHandle(nodes)
{
    ...
    for (size_t i = 0; i < nodes->streamers.size(); i++)
    {
        ...
        int n_io = nodes->streamers[i]->input_tags.size() + nodes->streamers[i]->output_tags.size();
        if (n_io == 0)
        {
...
void RosNodeHandle::bindStreamerToFormatorToRosStreamer(const NodeOptionHandlePtr &nodes)
{
    ...
    {
        if (ros_stream->getIoType() != StreamIOType::Output)
            continue;

        std::string ros_stream_tag = ros_stream->getTag();
        NodeOptionHandle::StreamerNodeBasePtr ros_stream_node =
            std::static_pointer_cast<NodeOptionHandle::StreamerNodeBase>(nodes->tag_to_node.at(ros_stream_tag));
        ...
        NodeOptionHandle::FormatorNodeBasePtr input_node =
            std::static_pointer_cast<NodeOptionHandle::FormatorNodeBase>(nodes->tag_to_node.at(input_tag));
        if (input_node->io != "input")
            continue;
        ...
        Streaming::DataCallback callback =
            std::bind(&RosStream::outputDataCallback, ros_stream, std::placeholders::_1, std::placeholders::_2);
...
void RosNodeHandle::bindRosStreamerToFormatorToStreamer(const NodeOptionHandlePtr &nodes)
{
    for (auto ros_stream : ros_streams_)
    {
        if (ros_stream->getIoType() != StreamIOType::Input)
            continue;
        ...
        for (const auto &output_tag : output_tags)
        {
            NodeOptionHandle::FormatorNodeBasePtr output_formator =
                std::static_pointer_cast<NodeOptionHandle::FormatorNodeBase>(nodes->tag_to_node.at(output_tag));
            if (output_formator->io != "log")
                continue;
        ...
        for (auto &output_tag : output_formator_tags)
        {
            ...
            Streaming::PipelineConvert pipeline = std::bind(&Streaming::pipelineConvertCallback, output_streaming.get(),
...
void RosNodeHandle::bindEstimatorToRosStreamer(const NodeOptionHandlePtr &nodes)
{
    for (auto estimating : estimatings_)
    {
        ...
        for (size_t i = 0; i < output_tags.size(); i++)
        {
            ...
            if (output_tag.substr(0, 4) != "str_")
                continue;

            std::shared_ptr<RosStream> stream = getRosStreamFromTag(output_tag);
            if (stream == nullptr)
                continue;
            EstimatingBase::OutputDataCallback out_callback =
                std::bind(&RosStream::outputDataCallback, stream.get(), std::placeholders::_1, std::placeholders::_2);
...
void RosNodeHandle::rebindAllStreamerToEstimator(const NodeOptionHandlePtr &nodes)
{
    ...
    for (auto &streaming : streamings_)
        streaming->clearDataCallbacks();
    ...
    for (size_t i = 0; i < estimatings_.size(); i++)
    {
        ...
        std::vector<std::shared_ptr<Streaming>> gnss_streamings, imu_streamings, image_streamings, solution_streamings;
        for (size_t i = 0; i < input_tags.size(); i++)
        {
            ...
            if (input_tag.substr(0, 4) != "str_" && input_tag.substr(0, 4) != "fmt_")
                continue;
            ...
            if (option_tools::sensorType(roles[0]) == SensorType::GNSS)
            {
                ...
                if (!is_ros)
                { // we do not need to check ROS because they are injective
                    bool found = false;
                    ...
                    if (!found)
                        gnss_streamings.push_back(stream);
                }
                else
                    gnss_streamings.push_back(stream);
            }
            else if (option_tools::sensorType(roles[0]) == SensorType::IMU)
            {
                ...
                if (!is_ros)
                {
                    ...
                    if (!found)
                        imu_streamings.push_back(stream);
                }
                else
                    imu_streamings.push_back(stream);
            }
            else if (option_tools::sensorType(roles[0]) == SensorType::Camera)
            {
                ...
                if (!is_ros)
                {
                    ...
                    if (!found)
                        image_streamings.push_back(stream);
                }
                else
                    image_streamings.push_back(stream);
            }
        ...
        data_integrations.push_back(
            std::make_shared<GnssDataIntegration>(estimating, gnss_streamings, gnss_tags, gnss_roles));
        data_integrations.push_back(
            std::make_shared<ImuDataIntegration>(estimating, imu_streamings, imu_tags, imu_roles));
        data_integrations.push_back(
            std::make_shared<ImageDataIntegration>(estimating, image_streamings, image_tags, image_roles));
        data_integrations.push_back(
            std::make_shared<SolutionDataIntegration>(estimating, solution_streamings, solution_tags, solution_roles));
