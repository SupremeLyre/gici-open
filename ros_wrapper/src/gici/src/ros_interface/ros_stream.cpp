    ...
    : Streaming(), nh_(nh), frame_id_("World"), valid_(false)
{
    ...
    for (const auto &input_tag : streamer_node->input_tags)
    {
        ...
        if (input_ros_stream_tag_.empty())
            input_ros_stream_tag_ = input_tag;
        else
        {
    ...
    if (io_type_ == StreamIOType::Log)
        io_type_ = StreamIOType::Output;
...
void RosStream::outputDataCallback(const std::string tag, const std::shared_ptr<DataCluster> &data)
{
    if (data->solution)
    {
...
void RosStream::pipelineCallback(const std::string &tag, const std::shared_ptr<DataCluster> &data)
{
...
void RosStream::bindLogWithInput()
{
    ...
    {
        if (static_this_[i]->input_ros_stream_tag_.empty())
            continue;
        ...
        {
            if (static_this_[j]->tag_ != input_tag)
                continue;
            auto &streaming_in = static_this_[j];
            PipelineCallback pipeline =
                std::bind(&RosStream::pipelineCallback, streaming_log, std::placeholders::_1, std::placeholders::_2);
    ...
    if (data_format_ == RosDataFormat::GnssRaw)
    {
        ...
        for (auto type : gnss.types)
        {
            if (type == GnssDataType::Observation &&
                ((format = std::find(gnss_formats_.begin(), gnss_formats_.end(), RosGnssDataFormat::Observations)) !=
                 gnss_formats_.end()))
            {
            ...
            {
                if ((format = std::find(gnss_formats_.begin(), gnss_formats_.end(), RosGnssDataFormat::CodeBias)) !=
                    gnss_formats_.end())
                {
                    size_t i = format - gnss_formats_.begin();
                    publishGnssSsrCodeBiases(publishers_[i], gnss);
                }
                if ((format = std::find(gnss_formats_.begin(), gnss_formats_.end(), RosGnssDataFormat::PhaseBias)) !=
                    gnss_formats_.end())
                {
                    size_t i = format - gnss_formats_.begin();
...
void RosStream::gnssObservationsCallback(const gici_ros::GnssObservationsConstPtr &msg)
{
    // Convert observation data
    std::shared_ptr<DataCluster> data_cluster = std::make_shared<DataCluster>(FormatorType::GnssRaw);
...
void RosStream::gnssEphemeridesCallback(const gici_ros::GnssEphemeridesConstPtr &msg)
{
...
void RosStream::gnssAntennaPositionCallback(const gici_ros::GnssAntennaPositionConstPtr &msg)
{
...
void RosStream::gnssIonosphereParameterCallback(const gici_ros::GnssIonosphereParameterConstPtr &msg)
{
...
void RosStream::gnssSsrCodeBiasesCallback(const gici_ros::GnssSsrCodeBiasesConstPtr &msg)
{
...
void RosStream::gnssSsrPhaseBiasesCallback(const gici_ros::GnssSsrPhaseBiasesConstPtr &msg)
{
    ...
    for (const auto &b : msg->biases)
    {
        ...
        for (size_t i = 0; i < b.phase.size(); i++)
        {
            ...
            {
                if (gnss_common::getPhaseID(b.prn[0], code) == phase)
                    break;
            }
            CHECK(code < MAXCODE);
...
void RosStream::gnssSsrEphemeridesCallback(const gici_ros::GnssSsrEphemeridesConstPtr &msg)
{
...
void RosStream::navSatFixCallback(const sensor_msgs::NavSatFixConstPtr &msg)
{
    ...
    else
    {
        ...
        {
            for (size_t i = 0; i < 3; i++)
                solution.covariance(i, i) = 1.0e-4;
        }
        else
        {
            for (size_t i = 0; i < 3; i++)
                solution.covariance(i, i) = 1.0e2;
        }
    }
    std::shared_ptr<DataCluster> data_cluster = std::make_shared<DataCluster>(solution);
