...
EstimatorBase::EstimatorBase(const EstimatorBaseOptions &options)
    : base_options_(options), graph_(std::make_shared<Graph>()), cauchy_loss_function_(new ceres::CauchyLoss(1)),
      huber_loss_function_(new ceres::HuberLoss(1)), marginalization_residual_id_(0),
...
bool EstimatorBase::applyMarginalization()
{
    ...
    marginalization_error_->marginalizeOut(parameter_blocks_to_be_marginalized, marginalization_keep_parameter_blocks_);
    ...
    if (marginalization_error_)
    {
        ...
        marginalization_residual_id_ = graph_->addResidualBlock(marginalization_error_, nullptr, parameter_block_ptrs);
...
bool EstimatorBase::getPoseEstimateAt(const double timestamp, Transformation &T_WS)
{
...
bool EstimatorBase::getSpeedAndBiasEstimateAt(const double timestamp, SpeedAndBias &speed_and_bias)
{
    for (size_t i = 0; i < states_.size() - 1; i++)
    {
        if (checkEqual(timestamp, states_[i].timestamp))
...
bool EstimatorBase::getCovarianceAt(const double timestamp, Eigen::Matrix<double, 15, 15> &covariance)
{
    for (size_t i = 0; i < states_.size() - 1; i++)
    {
        if (checkEqual(timestamp, states_[i].timestamp))
...
Transformation EstimatorBase::getPoseEstimate(const State &state)
{
    ...
    if (id.type() == IdType::gPosition)
    {
        std::shared_ptr<PositionParameterBlock> block_ptr =
            std::static_pointer_cast<PositionParameterBlock>(graph_->parameterBlockPtr(id.asInteger()));
    ...
    else
    {
        std::shared_ptr<PoseParameterBlock> block_ptr =
            std::static_pointer_cast<PoseParameterBlock>(graph_->parameterBlockPtr(id.asInteger()));
        CHECK(block_ptr != nullptr);
...
SpeedAndBias EstimatorBase::getSpeedAndBiasEstimate(const State &state)
{
    ...
    if (id.type() == IdType::gPosition)
    {
        ...
        std::shared_ptr<VelocityParameterBlock> block_ptr =
            std::static_pointer_cast<VelocityParameterBlock>(graph_->parameterBlockPtr(speed_id.asInteger()));
        CHECK(block_ptr != nullptr);
    ...
    else
    {
        ...
        std::shared_ptr<SpeedAndBiasParameterBlock> block_ptr = std::static_pointer_cast<SpeedAndBiasParameterBlock>(
...
Eigen::Matrix<double, 15, 15> EstimatorBase::computeAndGetCovariance(const State &state)
{
    ...
    if (id.type() == IdType::gPosition)
    {
        ...
        if (!has_speed)
        {
            ...
            // transform to local coordinate
            Eigen::Matrix3d p_covariance_enu =
                coordinate_->convertCovariance(p_covariance, GeoType::ECEF, GeoType::ENU);
        ...
        else
        {
            ...
            Eigen::Matrix3d rot = coordinate_->rotationMatrix(GeoType::ECEF, GeoType::ENU);
            Eigen::Matrix<double, 6, 6> rot_extend;
            rot_extend.setZero();
            rot_extend.topLeftCorner(3, 3) = rot;
            rot_extend.bottomRightCorner(3, 3) = rot;
            Eigen::Matrix<double, 6, 6> covariance_enu = rot_extend * covariance * rot_extend.transpose();
            full_covariance.setZero();
    ...
    else
    {
        ...
        parameters << T_WS.getPosition().x(), T_WS.getPosition().y(), T_WS.getPosition().z(), T_WS.getRotation().x(),
            T_WS.getRotation().y(), T_WS.getRotation().z(), T_WS.getRotation().w();
...
void EstimatorBase::updateCovariance(const State &state)
{
    ...
    {
        covariances_.insert(std::make_pair(state.timestamp, computeAndGetCovariance(state)));
    }
}
