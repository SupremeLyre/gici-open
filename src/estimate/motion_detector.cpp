...
bool MotionDetector::isImageMoving(double &sigma) const
{
    ...
    const int n_moving_pixels_threshold = static_cast<int>(opt_.ratio_moving_pixels_threshold * num_correspondences);
    int n_moving_pixels = 0;
    for (int i = 0; i < num_correspondences; ++i)
    {
        // compute pixel distance
        if ((features_last.col(i) - features_new.col(i)).norm() > opt_.px_diff_threshold)
        {
            ++n_moving_pixels;
...
int MotionDetector::findFeatureCorrespondences(Keypoints &features_last, Keypoints &features_new) const
{
...
bool MotionDetector::findFeatureCorrespondence(const size_t cam_index, const int track_id, Keypoint *last_px) const
{
    // find the matching track ID in last frames
    const int num_features = static_cast<int>(last_frames_->at(cam_index)->numFeatures());
    for (int i = 0; i < num_features; ++i)
    {
