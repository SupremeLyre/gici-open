...
GnssImuCameraSrrEstimator::GnssImuCameraSrrEstimator(const GnssImuCameraSrrEstimatorOptions &options,
    ...
    : srr_options_(options), GnssLooseEstimatorBase(gnss_loose_base_options, base_options),
      ...
      EstimatorBase(base_options)
{
    ...
    gnss_imu_initializer_.reset(
        new GnssImuInitializer(init_options, gnss_loose_base_options, imu_base_options, base_options, graph_));
}
...
bool GnssImuCameraSrrEstimator::addMeasurement(const EstimatorDataCluster &measurement)
{
    ...
    if (measurement.solution &&
        (measurement.solution_role == SolutionRole::Position || measurement.solution_role == SolutionRole::Velocity ||
         measurement.solution_role == SolutionRole::PositionAndVelocity))
    {
        GnssSolution gnss_solution = convertSolutionToGnssSolution(*measurement.solution, measurement.solution_role);
...
bool GnssImuCameraSrrEstimator::addGnssSolutionMeasurementAndState(const GnssSolution &measurement)
{
    ...
    // GNSS velocity
    addGnssVelocityResidualBlock(curGnssSolution(), states_[index], getImuMeasurementNear(timestamp).angular_velocity);
    // ZUPT
...
bool GnssImuCameraSrrEstimator::addImageMeasurementAndState(const FrameBundlePtr &frame_bundle,
...
bool GnssImuCameraSrrEstimator::visualInitialization(const FrameBundlePtr &frame_bundle)
{
    ...
    if (std_yaw > srr_options_.min_yaw_std_init_visual * D2R)
        return false;
    // set poses
...
bool GnssImuCameraSrrEstimator::estimate()
{
    ...
    if (base_options_.verbose_output)
    {
        LOG(INFO) << estimatorTypeToString(type_) << ": "
                  ...
                  << static_cast<int>(BackendId::sensorType(new_state.id.type())) << ", Fix status: " << std::setw(1)
                  << static_cast<int>(new_state.status);
    }
    ...
    if (new_state_type == IdType::cPose)
        frame_bundles_.push_back(nullptr);
    states_.push_back(State());
    while (!visual_initialized_ && states_.size() > srr_options_.max_gnss_window_length_minor)
    {
        states_.pop_front();
    }
...
void GnssImuCameraSrrEstimator::setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
{
    ...
    gnss_imu_initializer->arrangeToEstimator(srr_options_.max_gnss_window_length_minor, marginalization_error_, states_,
...
bool GnssImuCameraSrrEstimator::frameMarginalization()
{
    ...
    if (states_[latest_state_index_].is_keyframe && sizeOfKeyframeStates() > srr_options_.max_keyframes)
    {
        ...
        for (auto it = states_.begin(); it != states_.end();)
        {
            ...
            if (passed_first_keyframe && state.is_keyframe)
                break;
...
void GnssImuCameraSrrEstimator::sparsifyGnssStates()
{
    ...
    if (gnss_ids.size() <= srr_options_.max_keyframes)
        return;
