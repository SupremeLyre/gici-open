...
GnssImuInitializer::GnssImuInitializer(const GnssImuInitializerOptions &options,
                                       const GnssLooseEstimatorBaseOptions &gnss_loose_base_options,
                                       const ImuEstimatorBaseOptions &imu_base_options,
                                       const EstimatorBaseOptions &base_options, const std::shared_ptr<Graph> &graph,
                                       const std::shared_ptr<EstimatorBase> &sub_gnss_estimator)
    : options_(options), MultisensorInitializerBase(base_options),
      GnssLooseEstimatorBase(gnss_loose_base_options, base_options), ImuEstimatorBase(imu_base_options, base_options),
...
bool GnssImuInitializer::addGnssSolutionMeasurement(const GnssSolution &measurement)
{
    ...
    if (measurement.has_velocity)
        has_any_velocity_measurement_ = true;
    ...
    if (cur_timestamp - oldest_timestamp > options_.time_window_length_dynamic_motion)
    {
        gnss_solution_measurements_.pop_front();
        imu_mutex_.lock();
        while (imu_measurements_.front().timestamp < gnss_solution_measurements_.front().timestamp - 1.0)
        {
    ...
    for (size_t i = 1; i < gnss_solution_measurements_.size(); i++)
    {
        if (has_any_velocity_measurement_)
        {
            Eigen::Vector3d velocity =
                coordinate_->rotate(gnss_solution_measurements_[i].velocity, GeoType::ECEF, GeoType::ENU);
            Eigen::Vector3d last_velocity =
                coordinate_->rotate(gnss_solution_measurements_[i - 1].velocity, GeoType::ECEF, GeoType::ENU);
            if (imu_base_options_.car_motion)
            {
                const double angular_velocity_norm =
                    getImuMeasurementNear(gnss_solution_measurements_[i - 1].timestamp).angular_velocity.norm();
                if (last_velocity.norm() < imu_base_options_.car_motion_min_velocity ||
                    angular_velocity_norm > imu_base_options_.car_motion_max_anguler_velocity)
                {
                    gnss_solution_measurements_.clear();
                    dynamic_window_full_ = false;
                    break;
                }
            }
            initial_velocity = last_velocity;
            double dt = gnss_solution_measurements_[i].timestamp - gnss_solution_measurements_[i - 1].timestamp;
            double horizontal_acc = (velocity.head<2>().norm() - last_velocity.head<2>().norm()) / dt;
            if (horizontal_acc > max_acc)
                max_acc = horizontal_acc;
            if (horizontal_acc > options_.min_acceleration)
                acc_ensured = true;
        }
        else if (i > 1)
        {
...
bool GnssImuInitializer::arrangeToEstimator(
    const int window_length, const std::shared_ptr<MarginalizationError> &marginalization_error,
    std::deque<State> &states, ceres::ResidualBlockId &marginalization_residual_id, BackendId &gnss_extrinsics_id,
    std::deque<GnssSolution> &gnss_solution_measurements, ImuMeasurements &imu_measurements)
{
    if (!finished_)
        return false;
...
void GnssImuInitializer::slowMotionInitialization()
{
    ...
    const double timestamp_start = timestamp_end - options_.time_window_length_slow_motion;
    if (timestamp_start >= imu_measurements_.front().timestamp)
    {
        ...
        if (initPoseAndBiases(imu_measurements_, imu_base_options_.imu_parameters.g, T_WS_0_, speed_and_bias_0_))
        {
...
void GnssImuInitializer::putMeasurementAndStateToGraph(const Eigen::Quaterniond &q_WS_0,
                                                       const SpeedAndBias &speed_and_bias_0)
{
    // Clear old graph
    states_.clear();
    ...
    for (size_t i = 0; i < gnss_solution_measurements_.size(); i++)
    {
        ...
        // pose prior
        Eigen::Vector3d cur_position = coordinate_->convert(gnss.position, GeoType::ECEF, GeoType::ENU);
        if (i == 0)
        {
        ...
        {
            gnss_extrinsics_id_ = addGnssExtrinsicsParameterBlock(gnss.id, options_.gnss_extrinsics);
        }
        ...
        // GNSS velocity
        addGnssVelocityResidualBlock(gnss, states_[index], getImuMeasurementNear(timestamp).angular_velocity);
        // Initial errors
        if (isFirstEpoch())
        {
            // GNSS extrinsics
            addGnssExtrinsicsResidualBlock(gnss_extrinsics_id_, options_.gnss_extrinsics,
                                           options_.gnss_extrinsics_initial_std);
            // bias error
            double speed_std = speed_and_bias.head<3>().norm() * 2.0;
            if (speed_std < 1.0)
                speed_std = 1.0;
            addImuSpeedAndBiasResidualBlock(states_[index], speed_and_bias, speed_std,
