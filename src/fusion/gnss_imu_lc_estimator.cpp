...
GnssImuLcEstimator::GnssImuLcEstimator(const GnssImuLcEstimatorOptions &options,
    ...
    : lc_options_(options), GnssLooseEstimatorBase(gnss_loose_base_options, base_options),
...
bool GnssImuLcEstimator::addGnssSolutionMeasurementAndState(const GnssSolution &measurement)
{
    ...
    // GNSS velocity
    addGnssVelocityResidualBlock(curGnssSolution(), curState(), getImuMeasurementNear(timestamp).angular_velocity);
    // ZUPT
...
bool GnssImuLcEstimator::estimate()
{
    ...
    if (gnss_loose_base_options_.use_outlier_rejection)
        while (1)
        {
            ...
            // reject outlier
            if (!rejectGnssPositionAndVelocityOutliers(curState()))
                break;
        }
    ...
    if (num_cotinuous_reject_gnss_ > gnss_loose_base_options_.diverge_min_num_continuous_reject)
    {
...
void GnssImuLcEstimator::setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
{
    ...
    gnss_imu_initializer->arrangeToEstimator(lc_options_.max_window_length, marginalization_error_, states_,
