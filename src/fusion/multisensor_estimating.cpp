...
MultiSensorEstimating::MultiSensorEstimating(const NodeOptionHandlePtr &nodes, size_t i_estimator)
    : EstimatingBase(nodes, i_estimator), latest_imu_timestamp_(0.0)
{
    ...
    if (estimatorTypeContains(SensorType::GNSS, type_) && gnss_base_node.IsDefined())
    {
        option_tools::loadOptions(gnss_base_node, gnss_base_options_);
    }
    if (estimatorTypeContains(SensorType::GNSS, type_) && gnss_loose_base_node.IsDefined())
    {
    ...
    if (estimatorTypeContains(SensorType::IMU, type_) && imu_base_node.IsDefined())
    {
    ...
    if (estimatorTypeContains(SensorType::Camera, type_) && visual_estimator_base_node.IsDefined())
    {
        option_tools::loadOptions(visual_estimator_base_node, visual_estimator_base_options_);
    }
    if (estimatorTypeContains(SensorType::Camera, type_) && feature_handler_node.IsDefined())
    {
    ...
    if (type_ == EstimatorType::Spp)
    {
        ...
        estimator_.reset(new SppEstimator(spp_options_, gnss_base_options_, base_options_));
    }
    ...
    else if (type_ == EstimatorType::Sdgnss)
    {
        ...
        estimator_.reset(new SdgnssEstimator(sdgnss_options_, gnss_base_options_, base_options_));
    }
    ...
    else if (type_ == EstimatorType::Dgnss)
    {
        ...
        estimator_.reset(new DgnssEstimator(dgnss_options_, gnss_base_options_, base_options_));
    }
    // real-time kinematic
    else if (type_ == EstimatorType::Rtk)
    {
        ...
        estimator_.reset(new RtkEstimator(rtk_options_, gnss_base_options_, base_options_, ambiguity_options_));
    }
    ...
    else if (type_ == EstimatorType::Ppp)
    {
        ...
        estimator_.reset(new PppEstimator(ppp_options_, gnss_base_options_, base_options_, ambiguity_options_));
    }
    ...
    else if (type_ == EstimatorType::GnssImuLc)
    {
        ...
        estimator_.reset(new GnssImuLcEstimator(gnss_imu_lc_options_, gnss_imu_init_options_, gnss_loose_base_options_,
    ...
    else if (type_ == EstimatorType::SppImuTc)
    {
        ...
        estimator_.reset(new SppImuTcEstimator(spp_imu_tc_options_, gnss_imu_init_options_, spp_options_,
                                               gnss_base_options_, gnss_loose_base_options_, imu_base_options_,
                                               base_options_));
    }
    ...
    else if (type_ == EstimatorType::RtkImuTc)
    {
        ...
        estimator_.reset(new RtkImuTcEstimator(rtk_imu_tc_options_, gnss_imu_init_options_, rtk_options_,
                                               gnss_base_options_, gnss_loose_base_options_, imu_base_options_,
                                               base_options_, ambiguity_options_));
    }
    ...
    else if (type_ == EstimatorType::PppImuTc)
    {
        ...
        estimator_.reset(new PppImuTcEstimator(ppp_imu_tc_options_, gnss_imu_init_options_, ppp_options_,
                                               gnss_base_options_, gnss_loose_base_options_, imu_base_options_,
                                               base_options_, ambiguity_options_));
    }
    ...
    else if (type_ == EstimatorType::GnssImuCameraSrr)
    {
        ...
        estimator_.reset(new GnssImuCameraSrrEstimator(gnss_imu_camera_srr_options_, gnss_imu_init_options_,
                                                       gnss_loose_base_options_, visual_estimator_base_options_,
                                                       imu_base_options_, base_options_));
    ...
    else if (type_ == EstimatorType::SppImuCameraRrr)
    {
        ...
        estimator_.reset(new SppImuCameraRrrEstimator(
            ...
            gnss_loose_base_options_, visual_estimator_base_options_, imu_base_options_, base_options_));
        std::shared_ptr<VisualEstimatorBase> visual_estimator =
            std::dynamic_pointer_cast<VisualEstimatorBase>(estimator_);
        CHECK_NOTNULL(visual_estimator);
        visual_estimator->setFeatureHandler(feature_handler_);
...
MultiSensorEstimating::~MultiSensorEstimating()
{
    ...
    {
        image_frontend_thread_->join();
        image_frontend_thread_ = nullptr;
    }
    if (measurement_thread_)
    {
        measurement_thread_->join();
        measurement_thread_ = nullptr;
    }
    if (backend_thread_)
    {
...
void MultiSensorEstimating::resetProcessors()
{
    ...
    if (estimatorTypeContains(SensorType::IMU, type_))
    {
        ...
        std::shared_ptr<ImuEstimatorBase> imu_estimator = std::dynamic_pointer_cast<ImuEstimatorBase>(estimator_);
...
void MultiSensorEstimating::process()
{
    ...
    if (image_frontend_thread_ == nullptr && estimatorTypeContains(SensorType::Camera, type_))
    {
        image_frontend_thread_.reset(new std::thread(&MultiSensorEstimating::runImageFrontend, this));
    }

    // Put measurements from addin buffer to measurement buffer
    if (measurement_thread_ == nullptr)
    {
        measurement_thread_.reset(new std::thread(&MultiSensorEstimating::runMeasurementAddin, this));
    }

    // Process backend in a separated thread
    if (backend_thread_ == nullptr)
    {
        backend_thread_.reset(new std::thread(&MultiSensorEstimating::runBackend, this));
    }
...
bool MultiSensorEstimating::updateSolution()
{
    ...
    {
        LOG(WARNING) << "Erasing output timestamp " << std::fixed << output_timestamps_.front()
    ...
    if (!estimator_->getPoseEstimateAt(timestamp, solution_.pose) ||
        ...
        (compute_covariance_ && !estimator_->getCovarianceAt(timestamp, solution_.covariance)))
    {
        return false;
    }
...
void MultiSensorEstimating::handleTimePropagationSensors(EstimatorDataCluster &data)
{
    ...
    if (estimator_)
        estimator_->addMeasurement(data);
    mutex_input_.lock();
...
void MultiSensorEstimating::handleNonTimePropagationSensors(EstimatorDataCluster &data)
{
    ...
    if (enable_input_align_)
    {
        ...
        else if (measurement_align_buffer_.size() == 0 || data.timestamp >= measurement_align_buffer_.back().timestamp)
        {
            measurement_align_buffer_.push_back(data);
        }
        else if (data.timestamp <= measurement_align_buffer_.front().timestamp)
        {
            ...
            for (auto it = measurement_align_buffer_.begin(); it != measurement_align_buffer_.end(); it++)
            {
    ...
    for (auto it = measurement_align_buffer_.begin(); it != measurement_align_buffer_.end();)
    {
        ...
        if (measurement_align_buffer_.back().timestamp - measurement_align_buffer_.front().timestamp <
        ...
        if (estimatorTypeContains(SensorType::IMU, type_) && measurement.timestamp > latest_imu_timestamp_)
        {
            it++;
            continue;
        }
        ...
        if (enable_backend_data_sparsify_)
        {
            ...
            {
                LOG(WARNING) << "Backend pending! Sparsifying measurements with counter " << pending_sparsify_num_
                             << ".";
                for (int i = 0; i < pending_sparsify_num_; i++)
                {
                    // some measurements we cannot erase
                    if (measurements_.front().frame_bundle && measurements_.front().frame_bundle->isKeyframe())
                        break;
...
bool MultiSensorEstimating::processEstimator()
{
    ...
    // Check pending
    const int pending_warning_threshold = enable_backend_data_sparsify_ ? pending_num_threshold_ : 5;
    if (measurements_.size() > pending_warning_threshold)
    {
        if (last_backend_pending_num_ != measurements_.size())
        {
            LOG(WARNING) << "Large backend pending: " << measurements_.size() << " measurements are waiting!";
        }
    ...
    if (solution_.coordinate == nullptr)
    {
        ...
        if (force_initial_global_position_)
        {
            ...
            position_ecef =
                coordinate.convert(GeoCoordinate::degToRad(initial_global_position_), GeoType::LLA, GeoType::ECEF);
        }
        // get coordinate zero from GNSS raw
        else if (measurement.gnss && estimatorTypeContains(SensorType::GNSS, type_))
        {
        ...
        {
            position_ecef = measurement.solution->coordinate->convert(measurement.solution->pose.getPosition(),
        ...
        // set coordinate
        solution_.coordinate = std::make_shared<GeoCoordinate>(position_ecef, GeoType::ECEF);
        Eigen::Vector3d lla = solution_.coordinate->convert(position_ecef, GeoType::ECEF, GeoType::LLA);
        estimator_->setCoordinate(solution_.coordinate);
    ...
    {
        // solve
        if (estimator_->estimate())
            is_updated = true;
        // check if estimator valid
        if (estimator_->getStatus() == EstimatorStatus::Diverged)
        {
            if (type_ == EstimatorType::PppImuCameraRrr)
            {
                LOG(WARNING) << "PPP/IMU/Camera RRR reported divergence. Keep the initialized estimator alive "
                             << "and keep publishing propagated states.";
            }
            else
            {
                // reset estimator
    ...
    if (is_updated)
    {
        ...
        if (measurements_.size() > 1)
        {
            double pending_period = measurements_.back().timestamp - solution_.timestamp;
            // add feedbacks here
        }
    }
...
void MultiSensorEstimating::runImageFrontend()
{
    ...
    while (!quit_thread_ && SpinControl::ok())
    {
        ...
        if (!feature_handler_->isFirstFrame() &&
            feature_handler_->getFrameBundle()->getMinTimestampSeconds() >= front_measurement.timestamp)
        {
            LOG(WARNING) << "Image timestamp descending detected! (" << std::fixed << front_measurement.timestamp
                         << " vs " << feature_handler_->getFrameBundle()->getMinTimestampSeconds() << ")";
            image_frontend_measurements_.pop_front();
            mutex_image_input_.unlock();
            spin.sleep();
            continue;
        ...
        if (image_frontend_measurements_.size() > 5)
        {
            ...
            {
                LOG(WARNING) << "Large image frontend pending: " << image_frontend_measurements_.size()
                             << " frames are waiting!";
            }
