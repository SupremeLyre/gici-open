...
MultiSensorSingleThreadEstimating::MultiSensorSingleThreadEstimating(const NodeOptionHandlePtr &nodes,
                                                                     size_t i_estimator)
    : EstimatingBase(nodes, i_estimator), latest_imu_timestamp_(0.0)
{
    ...
    {
        LOG(WARNING) << "Input align should be enabled in one-thread estimating. Default values setted!";
...
void MultiSensorSingleThreadEstimating::process()
{
    ...
    {
        if (!processImageFrontend(measurement, measurement))
            return;
    }
...
bool MultiSensorSingleThreadEstimating::processImageFrontend(const EstimatorDataCluster &data,
                                                             EstimatorDataCluster &out)
{
    const EstimatorDataCluster &front_measurement = data;
    if (feature_handler_ == nullptr)
        return false;
