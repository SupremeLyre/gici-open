...
PppImuTcEstimator::PppImuTcEstimator(
    const PppImuTcEstimatorOptions &options, const GnssImuInitializerOptions &init_options,
    ...
    : tc_options_(options), ppp_options_(ppp_options), GnssEstimatorBase(gnss_base_options, base_options),
      ImuEstimatorBase(imu_base_options, base_options), EstimatorBase(base_options)
{
    ...
    initializer_sub_estimator_.reset(new PppEstimator(ppp_options, gnss_base_options, base_options, ambiguity_options));
...
bool PppImuTcEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement)
{
    // Get prior states
    ...
    if (isFirstEpoch())
    {
        addGnssPositionResidualBlock(curState(), position_prior, gnss_base_options_.error_parameter.initial_position);
        addGnssVelocityResidualBlock(curState(), velocity_prior, gnss_base_options_.error_parameter.initial_velocity);
    }
...
bool PppImuTcEstimator::estimate()
{
    ...
    if (ppp_options_.use_ambiguity_resolution)
    {
        ...
        AmbiguityResolution::Result ret =
            ambiguity_resolution_->solvePpp(curState().id, curAmbiguityState().ids, ambiguity_covariance, curGnss());
...
void PppImuTcEstimator::setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
{
