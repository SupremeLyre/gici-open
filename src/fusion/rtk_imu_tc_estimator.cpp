...
RtkImuTcEstimator::RtkImuTcEstimator(
    const RtkImuTcEstimatorOptions &options, const GnssImuInitializerOptions &init_options,
    const RtkEstimatorOptions rtk_options, const GnssEstimatorBaseOptions &gnss_base_options,
    const GnssLooseEstimatorBaseOptions &gnss_loose_base_options, const ImuEstimatorBaseOptions &imu_base_options,
    const EstimatorBaseOptions &base_options, const AmbiguityResolutionOptions &ambiguity_options)
    : tc_options_(options), rtk_options_(rtk_options), GnssEstimatorBase(gnss_base_options, base_options),
      ImuEstimatorBase(imu_base_options, base_options), EstimatorBase(base_options)
{
    ...
    initializer_.reset(new GnssImuInitializer(init_options, gnss_loose_base_options, imu_base_options, base_options,
                                              graph_, initializer_sub_estimator_));
}
...
bool RtkImuTcEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement_rov,
                                                   const GnssMeasurement &measurement_ref)
{
    ...
    addDdPseudorangeResidualBlocks(curGnssRov(), curGnssRef(), code_index_pairs, curState(), num_valid_satellite);
    ...
    addDdPhaserangeResidualBlocks(curGnssRov(), curGnssRef(), phase_index_pairs, curState());

    // Add doppler residual blocks
    addDopplerResidualBlocks(curGnssRov(), curState(), num_valid_satellite, false,
...
bool RtkImuTcEstimator::estimate()
{
    ...
    if (gnss_base_options_.use_outlier_rejection)
        while (1)
        {
            ...
            if (!rejectPseudorangeOutlier(curState(), curAmbiguityState(),
                ...
                !rejectDopplerOutlier(curState(), gnss_base_options_.reject_one_outlier_once) &&
                !rejectPhaserangeOutlier(curState(), curAmbiguityState(), gnss_base_options_.reject_one_outlier_once))
                break;
            if (!gnss_base_options_.reject_one_outlier_once)
                break;
        }
    ...
    double ratio_pseudorange =
        n_pseudorange == 0.0 ? 0.0 : 1.0 - getDivide(numPseudorangeError(curState()), n_pseudorange);
    double ratio_phaserange = n_phaserange == 0.0 ? 0.0 : 1.0 - getDivide(numPhaserangeError(curState()), n_phaserange);
    double ratio_doppler = n_doppler == 0.0 ? 0.0 : 1.0 - getDivide(numDopplerError(curState()), n_doppler);
    ...
    if (rtk_options_.use_ambiguity_resolution)
    {
        ...
        AmbiguityResolution::Result ret = ambiguity_resolution_->solveRtk(
            curState().id, curAmbiguityState().ids, ambiguity_covariance, gnss_measurement_pairs_.back());
...
void RtkImuTcEstimator::setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
{
    ...
    gnss_imu_initializer->arrangeToEstimator(tc_options_.max_window_length, marginalization_error_, states_,
