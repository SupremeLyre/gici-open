...
SppImuCameraRrrEstimator::SppImuCameraRrrEstimator(
    const SppImuCameraRrrEstimatorOptions &options, const GnssImuInitializerOptions &init_options,
    const SppEstimatorOptions spp_options, const GnssEstimatorBaseOptions &gnss_base_options,
    ...
    : rrr_options_(options), spp_options_(spp_options), GnssEstimatorBase(gnss_base_options, base_options),
      ...
      EstimatorBase(base_options)
{
    ...
    initializer_sub_estimator_.reset(new SppEstimator(spp_options, gnss_base_options, base_options));
...
bool SppImuCameraRrrEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement)
{
    // Get prior states
    ...
    addFrequencyParameterBlocks(curGnss(), curGnss().id, num_valid_doppler_system, std::map<char, double>(), true);
    ...
    addDopplerResidualBlocks(curGnss(), curState(), num_valid_satellite, true,
...
bool SppImuCameraRrrEstimator::addImageMeasurementAndState(const FrameBundlePtr &frame_bundle,
...
bool SppImuCameraRrrEstimator::estimate()
{
    ...
    if (new_state_type == IdType::gPose)
    {
        ...
        if (isGnssGoodObservation() && (ratio_pseudorange > thr || ratio_doppler > thr))
        {
...
void SppImuCameraRrrEstimator::setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
{
