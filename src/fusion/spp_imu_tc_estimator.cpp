...
SppImuTcEstimator::SppImuTcEstimator(const SppImuTcEstimatorOptions &options,
    ...
    : tc_options_(options), spp_options_(spp_options), GnssEstimatorBase(gnss_base_options, base_options),
...
bool SppImuTcEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement)
{
    // Get prior states
...
bool SppImuTcEstimator::estimate()
{
    ...
    if (base_options_.verbose_output)
    {
        LOG(INFO) << estimatorTypeToString(type_) << ": "
                  ...
                  << "Final cost: " << graph_->summary.final_cost << ", Sat number: " << std::setw(2)
                  << num_satellites_;
    }
...
void SppImuTcEstimator::setInitializationResult(const std::shared_ptr<MultisensorInitializerBase> &initializer)
{
