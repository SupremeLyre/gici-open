...
void cycleSlipDetection(GnssMeasurement &measurement_pre, GnssMeasurement &measurement_cur,
                        const GnssCommonOptions &options, const Eigen::Vector3d position_pre,
...
void cycleSlipDetectionSD(GnssMeasurement &measurement_rov_pre, GnssMeasurement &measurement_ref_pre,
                          GnssMeasurement &measurement_rov_cur, GnssMeasurement &measurement_ref_cur,
                          const GnssCommonOptions &options, const Eigen::Vector3d position_pre,
                          const Eigen::Vector3d position_cur)
{
    // Detect by LLI
    ...
    GnssMeasurementSDIndexPairs pairs_pre = gnss_common::formPhaserangeSDPair(measurement_rov_pre, measurement_ref_pre);
    ...
    GnssMeasurementSDIndexPairs pairs_cur = gnss_common::formPhaserangeSDPair(measurement_rov_cur, measurement_ref_cur);
...
void cycleSlipDetectionLLI(GnssMeasurement &measurement_pre, GnssMeasurement &measurement_cur)
{
    for (auto &sat : measurement_cur.satellites)
    {
        for (auto &obs : sat.second.observations)
        {
            ...
            if (it_sat == measurement_pre.satellites.end())
                continue;
            auto it_obs = it_sat->second.observations.find(obs.first);
            if (it_obs == it_sat->second.observations.end())
                continue;
...
void cycleSlipDetectionMW(GnssMeasurement &measurement_pre, GnssMeasurement &measurement_cur, double threshold)
{
    GnssMeasurementSDIndexPairs pairs = gnss_common::formPhaserangeSDPair(measurement_pre, measurement_cur);
...
#if LOG_CYCLE_SLIP
                LOG(INFO) << "Detected cycle slip by MW at " << measurement_cur.getSat(index_cur_0).prn << ".";
#endif
            }
        }
    }
...
void cycleSlipDetectionGF(GnssMeasurement &measurement_pre, GnssMeasurement &measurement_cur, double threshold)
{
    GnssMeasurementSDIndexPairs pairs = gnss_common::formPhaserangeSDPair(measurement_pre, measurement_cur);
    ...
    for (size_t i = 0; i < pair_indexes.size(); i++)
    {
        ...
        for (size_t j = 1; j < pair_indexes[i].size(); j++)
        {
            ...
            if (fabs(gf_pre - gf_cur) > threshold)
            {
                ...
                observation_cur_1.slip = true;
#if LOG_CYCLE_SLIP
                LOG(INFO) << "Detected cycle slip by GF at " << measurement_cur.getSat(index_cur_0).prn << ".";
#endif
            }
        }
    }
...
void cycleSlipDetectionPosition(GnssMeasurement &measurement_pre, GnssMeasurement &measurement_cur,
                                const Eigen::Vector3d position_pre, const Eigen::Vector3d position_cur,
                                double threshold)
{
...
void cycleSlipDetectionTimeGap(GnssMeasurement &measurement_pre, GnssMeasurement &measurement_cur, double max_time_gap)
{
#ifndef NDEBUG
    ...
    for (auto sat : measurement_cur.satellites)
    {
        ...
        {
            if (gnss_common::checkObservationValid(measurement_cur, GnssMeasurementIndex(sat.first, obs.first),
        ...
        if (num_phases > 1)
            is_single_frequency.push_back(false);
        else
            is_single_frequency.push_back(true);
    }
...
double getInitialAmbiguitySD(const GnssMeasurement &measurement_rov, const GnssMeasurement &measurement_ref,
                             const GnssMeasurementIndex &index_rov, const GnssMeasurementIndex &index_ref)
{
    auto &observation_1 = measurement_rov.satellites.at(index_rov.prn).observations.at(index_rov.code_type);
    double pseudorange_1 = observation_1.pseudorange;
    double phaserange_1 = observation_1.phaserange;

    auto &observation_2 = measurement_ref.satellites.at(index_ref.prn).observations.at(index_ref.code_type);
...
bool solveAmbiguityLambda(const Eigen::VectorXd &float_ambiguities, const Eigen::MatrixXd &covariance,
                          const double ratio_threshold, Eigen::VectorXd &fixed_ambiguities, double &ratio)
{
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> fixed_template =
        Eigen::MatrixXd::Zero(length, 2);
    Eigen::Vector2d residuals;
    // Lambda search
    lambda(float_ambiguities.size(), 2, float_ambiguities.data(), covariance.data(), fixed_template.data(),
           residuals.data());
