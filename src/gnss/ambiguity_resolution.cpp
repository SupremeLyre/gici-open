...
AmbiguityResolution::AmbiguityResolution(const AmbiguityResolutionOptions options, const std::shared_ptr<Graph> &graph)
    : options_(options), graph_(graph), cauchy_loss_function_(new ceres::CauchyLoss(1)),
...
AmbiguityResolution::Result AmbiguityResolution::solvePpp(const BackendId &epoch_id,
                                                          ...
                                                          const GnssMeasurement &measurements)
{
    ...
    for (size_t i = 0; i < ambiguity_ids.size(); i++)
    {
        ...
        if (!gnss_common::useSystem(options_.system_exclude, system))
            continue;
        if (!gnss_common::useSatellite(options_.satellite_exclude, prn))
            continue;
        if (!gnss_common::usePhase(options_.phase_exclude, system, phase_id))
            continue;

        const Eigen::Vector3d &position = measurements.position;
        const Eigen::Vector3d &sat_position = measurements.satellites.at(prn).sat_position;
        double elevation = gnss_common::satelliteElevation(sat_position, position);
        if (gnss_common::radToDegree(elevation) < options_.min_elevation)
            continue;
    ...
    for (size_t p = 0; p < parameters.size(); p++)
    {
        ...
        parameter.value = Eigen::Map<Eigen::VectorXd>(parameters[p].second->parameters(), parameter.size);
        ...
        else
        {
            ...
            parameter.covariance_start_index = last_parameter.covariance_start_index + last_parameter.minimal_size;
        }
    ...
    for (int i = lane_groups_.size() - 1; i >= 0; i--)
    {
        ...
        int num_fixed = trySolveLanes(lane_groups_.at(i), min_percentage_fixation, use_rounding);
    ...
    if (num_success_nl == 0 && num_success_wl == 0 && num_success_uwl == 0)
        return Result::NoFix;
    ...
    {
        LOG(INFO) << "Invalid ambiguity resolution: Total cost increases from " << std::scientific
                  << std::setprecision(3) << range_cost_before << " to " << range_cost << ".";
...
void AmbiguityResolution::formSatellitePairPpp()
{
    ...
    for (size_t i = 0; i < getGnssSystemList().size(); i++)
    {
        ...
        for (auto it : prn_to_number_phases)
        {
            if (it.first[0] != system)
                continue;
            if (system == 'C' && has_bds_meo && !gnss_common::isBdsMeo(it.first))
                continue;
    ...
    for (size_t i = 0; i < getGnssSystemList().size(); i++)
    {
        ...
        for (size_t j = 0; j < curAmbs().size(); j++)
        {
            ...
            if (ambiguity.id.gSystem() != system)
                continue;

            // we prefer to use BDS MEO satellites
            if (system == 'C' && has_bds_meo && !gnss_common::isBdsMeo(ambiguity.prn))
                continue;

            // we only select satellites with max phase number
            if (prn_to_number_phases.at(ambiguity.id.gPrn()) != system_to_num_phases.at(system))
                continue;
    ...
    for (size_t i = 0; i < curAmbs().size(); i++)
    {
        ...
        std::string prn = ambiguity.id.gPrn();
        if (system_to_reference_prn.find(system) == system_to_reference_prn.end())
            continue;
        ...
        for (auto it = prn_to_specs.lower_bound(prn_ref); it != prn_to_specs.upper_bound(prn_ref); it++)
        {
    ...
    for (size_t i = 0; i < getGnssSystemList().size(); i++)
    {
        ...
        if (it == system_to_wavelengths.end())
            continue;
        ...
        for (std::vector<double>::reverse_iterator wavelength = wavelengths.rbegin(); wavelength != wavelengths.rend();
             wavelength++)
        {
    ...
    for (size_t i = 0; i < curAmbPairs().size(); i++)
    {
        ...
        for (auto it = system_to_wavelengths.lower_bound(system); it != system_to_wavelengths.upper_bound(system); it++)
        {
        ...
        if (desired_wavelength_to_combine == 1.0e6)
            continue;
        for (size_t j = 0; j < curAmbPairs().size(); j++)
        {
            ...
            if (wavelength_j != desired_wavelength_to_combine)
                continue;
...
std::vector<std::vector<size_t>> AmbiguityResolution::groupingSatellitePairForBand(const LaneType &type,
                                                                                   const size_t min_num)
{
    ...
    {
        if (curAmbLanePairs()[i].laneType() != type)
            continue;
    ...
    for (auto it = wavelength_to_indexes.begin(); it != wavelength_to_indexes.end();)
    {
        double wavelength = it->first;
        size_t index = it->second;
        char system = index_to_systems.at(index);
        if (current_wavelength == 0.0)
        ...
        else
        {
            ...
            if (group.size() < min_num)
            {
                ...
                {
                    double last_group_wavelength = lane_group_wavelengths.at(current_group_index - 1).back();
                    double current_group_wavelength = lane_group_wavelengths.at(current_group_index).front();
...
// Solve ambiguities
int AmbiguityResolution::solveLanes(const std::vector<size_t> &indexes, const double min_percentage_fixation,
                                    const DelMethod method, const bool use_rounding, const bool skip_full)
{
    // Get combinations
    ...
    if (method == DelMethod::Variance)
    {
        std::sort(lane_pairs.begin(), lane_pairs.end(),
                  [](LanePair lhs, LanePair rhs) -> bool { return lhs.std > rhs.std; });
    }
    // by elevation
    else if (method == DelMethod::Elevation)
    {
        std::sort(lane_pairs.begin(), lane_pairs.end(),
                  [](LanePair lhs, LanePair rhs) -> bool { return lhs.elevation > rhs.elevation; });
    }
    // by frational part
    else if (method == DelMethod::Fractional)
    {
        std::sort(lane_pairs.begin(), lane_pairs.end(), [](LanePair lhs, LanePair rhs) -> bool {
    ...
    Eigen::MatrixXd float_covariance = Eigen::MatrixXd::Zero(lane_pairs.size(), lane_pairs.size());
    Eigen::MatrixXd differential_jacobian = Eigen::MatrixXd::Zero(lane_pairs.size(), curAmbs().size());
    ...
    float_covariance = differential_jacobian * ambiguity_covariance_ * differential_jacobian.transpose();

    // Sovle ambiguity
    Eigen::VectorXd fixed_ambiguities;
    int num_active = float_ambiguities.size();
    int min_num_fixation = static_cast<int>(min_percentage_fixation * static_cast<double>(num_active));
    ...
    if (!use_rounding)
    {
        ...
        while (num_active >= min_num_fixation)
        {
            if (!(skip_full && num_active == float_ambiguities.size()))
            {
                if (sqrt(active_float_covariance(num_active - 1, num_active - 1)) < 0.25)
                    if (solveAmbiguityLambda(active_float_ambiguities, active_float_covariance, options_.ratio,
                                             fixed_ambiguities, ratio))
                        break;
            }
            // reduce subsets
            --num_active;
        ...
        // if (ratio != 0.0) LOG(INFO) << (int)lane_type << ": " << ratio << " | " <<
        // active_float_ambiguities.transpose();
    }
    // solve by rounding
    else
    {
        ...
        for (size_t i = 0; i < float_ambiguities.size(); i++)
        {
            ...
            if (sqrt(float_covariance(i, i)) > 0.25)
                break;
            if (fabs(fractional) > 0.25)
                break;
    ...
    if (num_active < min_num_fixation)
        return 0;
    ...
    jacobian.resize(num_active, ambiguity_size);
    jacobian = differential_jacobian.topRows(num_active);
    Eigen::MatrixXd fix_ambiguity_covariance = Eigen::MatrixXd::Identity(num_active, num_active) * 1e-6; // 0.001 cycles

    // apply Kalman upadte to constraint the parameters
    Eigen::MatrixXd kalman_gain =
        ambiguity_covariance_ * jacobian.transpose() *
        (jacobian * ambiguity_covariance_ * jacobian.transpose() + fix_ambiguity_covariance).inverse();
    Eigen::VectorXd dx = kalman_gain * (fixed_ambiguities - jacobian * ambiguity_parameters);
    ambiguity_covariance_ =
        (Eigen::MatrixXd::Identity(ambiguity_size, ambiguity_size) - kalman_gain * jacobian) * ambiguity_covariance_;
    ...
    for (int i = 0; i < num_active; i++)
    {
        ...
        {
            if (ambiguity_lane_pairs[j].bsd_pair_id_higher == fixed_lane_pair.bsd_pair_id_higher &&
                ambiguity_lane_pairs[j].bsd_pair_id_lower == fixed_lane_pair.bsd_pair_id_lower)
            {
    ...
    for (int i = 0; i < num_active; i++)
    {
        ...
        size_t id_higher_raw = ambiguity_pairs[lane_pair.bsd_pair_id_higher].spec_id;
        size_t id_higher_ref = ambiguity_pairs[lane_pair.bsd_pair_id_higher].spec_id_ref;

        if (lane_pair.laneType() != LaneType::NL)
        {
            size_t id_lower_raw = ambiguity_pairs[lane_pair.bsd_pair_id_lower].spec_id;
            size_t id_lower_ref = ambiguity_pairs[lane_pair.bsd_pair_id_lower].spec_id_ref;
            double wave_higher = ambiguity_pairs[lane_pair.bsd_pair_id_higher].wavelength;
            double wave_lower = ambiguity_pairs[lane_pair.bsd_pair_id_lower].wavelength;
            ...
            std::shared_ptr<AmbiguityError4Coef> ambiguity_error =
                std::make_shared<AmbiguityError4Coef>(fixed_ambiguities(i), information, coefficients);
            CHECK(lane_pair.residual_id == nullptr);
            lane_pair.residual_id =
                graph_->addResidualBlock(ambiguity_error, cauchy_loss_function_ ? cauchy_loss_function_.get() : nullptr,
        ...
        else
        {
            ...
            std::shared_ptr<AmbiguityError2Coef> ambiguity_error =
                std::make_shared<AmbiguityError2Coef>(fixed_ambiguities(i), information, coefficients);
            CHECK(lane_pair.residual_id == nullptr);
            lane_pair.residual_id =
                graph_->addResidualBlock(ambiguity_error, cauchy_loss_function_ ? cauchy_loss_function_.get() : nullptr,
                                         graph_->parameterBlockPtr(curAmbs()[id_higher_raw].id.asInteger()),
...
int AmbiguityResolution::trySolveLanes(const std::vector<size_t> &indexes, const double min_percentage_fixation,
                                       const bool use_rounding)
{
    static const std::vector<DelMethod> sequence = {DelMethod::Elevation, DelMethod::Variance, DelMethod::Fractional};
    ...
    for (size_t i = 0; i < sequence.size(); i++)
    {
        ...
        num_fixed = solveLanes(indexes, min_percentage_fixation, method, use_rounding, skip_full);
    }
...
bool AmbiguityResolution::findMatch(LanePair &lane_pair, std::vector<LanePair> &matches,
                                    std::vector<double> &coefficients)
{
    if (!lane_pair.is_fixed)
        return false;
    ...
    {
        if (last_lane_pairs[i].wavelength != wavelength)
            continue;
        if (!last_lane_pairs[i].is_fixed)
            continue;
        ...
        if (lane_pair.laneType() != LaneType::NL)
        {
            if (last_phase_id_higher != phase_id_higher || last_phase_id_lower != phase_id_lower)
            {
                continue;
            }
        }
...
void AmbiguityResolution::eraseAmbiguityResidualBlocks(std::vector<LanePair> &lane_pairs)
{
...
void AmbiguityResolution::setGraphParameters(std::vector<Parameter> &parameters)
{
    ...
    {
        Eigen::Map<Eigen::VectorXd> value(parameter.handle->parameters(), parameter.size);
...
{
    Graph::ResidualBlockCollection residual_blocks = graph_->residuals(epoch_id.asInteger());
    ...
    for (size_t i = 0; i < residual_blocks.size(); i++)
    {
        ...
        if (!(type == ErrorType::kPseudorangeError || type == ErrorType::kPseudorangeErrorSD ||
              type == ErrorType::kPseudorangeErrorDD || type == ErrorType::kPhaserangeError ||
              type == ErrorType::kPhaserangeErrorSD || type == ErrorType::kPhaserangeErrorDD))
            continue;
        double residual[1];
