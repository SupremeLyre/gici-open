...
AmbiguityResolution::Result AmbiguityResolution::solveRtk(
    const BackendId &epoch_id, const std::vector<BackendId> &ambiguity_ids, const Eigen::MatrixXd &ambiguity_covariance,
    const std::pair<GnssMeasurement, GnssMeasurement> &measurements)
{
    ...
    for (size_t i = 0; i < ambiguity_ids.size(); i++)
    {
        ...
        const Eigen::Vector3d &sat_position = measurements_rov.satellites.at(prn).sat_position;
    ...
    {
        LOG(INFO) << "Invalid ambiguity resolution: Total cost changes from " << std::scientific << std::setprecision(3)
