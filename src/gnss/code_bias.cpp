...
void CodeBias::setDcb(const std::string prn, const int code, const int code_base, const double value)
{
    ...
    {
        if (it_dcb->second.code1 == code_base && it_dcb->second.code2 == code)
        {
            found = true;
            break;
        }
...
void CodeBias::setTgdIsc(const std::string prn, const TgdIscType type, const double value)
{
    // check if exist
    bool found = false;
...
void CodeBias::setZdcb(const std::string prn, const int code, const double value)
{
    CHECK(prn.size() == 3);
...
double CodeBias::getCodeBias(const std::string prn, const int code, const bool accept_coarse)
{
...
void CodeBias::arrange()
{
    ...
    for (auto it = all_source_dcbs_coarse_.begin(); it != all_source_dcbs_coarse_.end();)
    {
        if (biases_updated_.at(it->first))
        {
...
void CodeBias::arrangeAllSourceDcbs(std::multimap<std::string, Dcb> &all_source_dcbs, BiasMap &biases,
                                    bool clear_update_flag)
{
    ...
    {
        if (!biases_updated_.at(dcb.first))
            continue;
        ...
        if (clear_update_flag)
            biases_updated_.at(dcb.first) = false;
    }
    ...
    for (auto prn : prns)
    {
        ...
        std::vector<Dcb> dcbs;
        for (auto dcb = all_source_dcbs.lower_bound(prn); dcb != all_source_dcbs.upper_bound(prn); dcb++)
        {
        ...
        {
            LOG(INFO) << "Input DCBs are not closed for " << prn << "! H = " << std::endl << H;
            continue;
        }
...
void CodeBias::putZdcbsToAllSourceDcbs()
{
    ...
    for (auto it = zdcbs_.begin(); it != zdcbs_.end(); it++)
    {
        ...
        if (!biases_updated_.at(prn))
            continue;

        zdcbs.push_back(zdcb);
        if (current_prn.empty())
            current_prn = prn;
...
void CodeBias::putDefaultDcbsToAllSourceDcbs()
{
    ...
    for (auto prn : prns)
    {
        ...
        for (auto dcb = all_source_dcbs_coarse_.lower_bound(prn); dcb != all_source_dcbs_coarse_.upper_bound(prn);
             dcb++)
        {
        ...
        for (auto default_dcb : default_dcbs_)
        {
            ...
            if ((std::find(phases.begin(), phases.end(), phase1) == phases.end()) &&
                (std::find(phases.begin(), phases.end(), phase2) == phases.end()))
                continue;
...
bool CodeBias::checkExist(const std::string prn, const int code, BiasMap &biases)
{
    ...
    if (code_to_bias.find(code) != code_to_bias.end())
        return true;
    return false;
}
