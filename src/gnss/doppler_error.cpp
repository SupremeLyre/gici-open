...
template <int... Ns>
DopplerError<Ns...>::DopplerError(const GnssMeasurement &measurement, const GnssMeasurementIndex index,
                                  const GnssErrorParameter &error_parameter)
{
    ...
    // Group 1
    if (dims_.kNumParameterBlocks == 3 && dims_.GetDim(0) == 3 && dims_.GetDim(1) == 3 && dims_.GetDim(2) == 1)
    {
        is_estimate_body_ = false;
        parameter_block_group_ = 1;
    }
    // Group 2
    else if (dims_.kNumParameterBlocks == 4 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 9 && dims_.GetDim(2) == 3 &&
             dims_.GetDim(3) == 1)
    {
...
template <int... Ns>
DopplerError<Ns...>::DopplerError(const GnssMeasurement &measurement, const GnssMeasurementIndex index,
                                  const GnssErrorParameter &error_parameter, const Eigen::Vector3d &angular_velocity)
...
// Set the information.
template <int... Ns> void DopplerError<Ns...>::setInformation(const GnssErrorParameter &error_parameter)
{
    // compute variance
...
template <int... Ns>
bool DopplerError<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool DopplerError<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                       double **jacobians, double **jacobians_minimal) const
{
    ...
    double range_rate = vs.dot(e) + OMGE / CLIGHT *
                                        (v_sat(1) * t_WR_ECEF(0) + p_sat(1) * v_WR_ECEF(0) - v_sat(0) * t_WR_ECEF(1) -
                                         p_sat(0) * v_WR_ECEF(1));
    double doppler_estimate = range_rate + clock_frequency - satellite_.sat_frequency;

    // Compute error
    double doppler = observation_.doppler;
    Eigen::Matrix<double, 1, 1> error = Eigen::Matrix<double, 1, 1>(doppler - doppler_estimate);
    ...
    if (jacobians != nullptr)
    {
        ...
        Eigen::Matrix<double, 1, 3> J_v_ECEF = -((t_WR_ECEF - satellite_.sat_position) / rho).transpose();
        ...
        if (is_estimate_body_)
        {
            ...
            Eigen::Matrix<double, 1, 3> J_q_WS =
                J_v_W * skewSymmetric(angular_velocity_) * -skewSymmetric(q_WS.toRotationMatrix() * t_SR_S);
