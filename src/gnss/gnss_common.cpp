...
{
    auto it = std::find(options.system_exclude.begin(), options.system_exclude.end(), system);
    if (it == options.system_exclude.end())
        return true;
    else
        return false;
}
...
{
    auto it = std::find(system_exclude.begin(), system_exclude.end(), system);
    if (it == system_exclude.end())
        return true;
    else
        return false;
}
...
{
    auto it = std::find(options.satellite_exclude.begin(), options.satellite_exclude.end(), prn);
    if (it == options.satellite_exclude.end())
        return true;
    else
        return false;
}
bool useSatellite(const std::vector<std::string> &satellite_exclude, const std::string prn)
{
    auto it = std::find(satellite_exclude.begin(), satellite_exclude.end(), prn);
    if (it == satellite_exclude.end())
        return true;
    else
        return false;
}
...
{
    auto it = std::find(options.code_exclude.begin(), options.code_exclude.end(), std::make_pair(system, code_type));
    if (it == options.code_exclude.end())
        return true;
    else
        return false;
}
bool useCode(const std::vector<std::pair<char, int>> &code_exclude, char system, const int code_type)
{
    auto it = std::find(code_exclude.begin(), code_exclude.end(), std::make_pair(system, code_type));
    if (it == code_exclude.end())
        return true;
    else
        return false;
}
...
bool usePhase(const std::vector<std::pair<char, int>> &phase_exclude, char system, const int phase_id)
{
    auto it = std::find(phase_exclude.begin(), phase_exclude.end(), std::make_pair(system, phase_id));
    if (it == phase_exclude.end())
        return true;
    else
        return false;
}
...
bool checkElevation(const GnssCommonOptions &options, const GnssMeasurement &measurement, std::string prn)
{
    // no elevation mask
    if (options.min_elevation == 0.0)
        return true;
    ...
    double elevation = satelliteElevation(satellite.sat_position, measurement.position);
    if (radToDegree(elevation) > options.min_elevation)
        return true;
    else
        return false;
}
...
// Check SNR mask
bool checkSNR(const GnssCommonOptions &options, const GnssMeasurement &measurement, const GnssMeasurementIndex &index)
{
...
#undef MAP
            ...
            else
            {
                ...
                double default_bias = code_bias->getCodeBias(prn, default_code, accept_coarse);
...
bool checkObservationValid(const GnssMeasurement &measurement, const GnssMeasurementIndex &index,
                           const ObservationType type, const GnssCommonOptions &options,
                           const bool need_precise_ephemeris)
{
    // Cannot find given satellite
    auto sat = measurement.satellites.find(index.prn);
    if (sat == measurement.satellites.end())
        return false;

    auto &satellite = sat->second;

    // System not used
    if (!gnss_common::useSystem(options, satellite.getSystem()))
        return false;

    // Satellite not used
    if (!gnss_common::useSatellite(options, satellite.prn))
        return false;

    // Ephemeris invalid
    if (satellite.sat_type == SatEphType::None || checkZero(satellite.sat_position) || satellite.sat_clock == 0.0)
    {
        return false;
    }
    ...
    if (obs == satellite.observations.end())
        return false;
    ...
    {
        if (observation.wavelength == 0.0 || observation.pseudorange == 0.0)
        {
            return false;
        }
    }
    ...
    {
        if (observation.wavelength == 0.0 || observation.phaserange == 0.0)
        {
            return false;
        }
    }
    ...
    {
        if (observation.wavelength == 0.0 || observation.doppler == 0.0)
        {
            return false;
        }
    }
...
GnssMeasurementSDIndexPairs formPseudorangeSDPair(const GnssMeasurement &measurement_rov,
                                                  ...
                                                  const GnssCommonOptions &options)
{
    ...
    for (auto &sat : measurement_rov.satellites)
    {
        ...
        if (!gnss_common::useSystem(options, system))
            continue;
        for (auto obs : satellite.observations)
        {
            GnssMeasurementIndex index_rov(satellite.prn, obs.first);
            if (checkObservationValid(measurement_rov, index_rov, ObservationType::Pseudorange, options))
            {
    ...
    for (auto index : indexes_1)
    {
        ...
        for (auto &sat : measurement_ref.satellites)
        {
            ...
            if (satellite_2.prn != satellite.prn)
                continue;

            if (!checkObservationValid(measurement_ref, GnssMeasurementIndex(satellite.prn, index.code_type),
            ...
            index_pairs.push_back(
                GnssMeasurementSDIndexPair(index, GnssMeasurementIndex(satellite.prn, index.code_type)));
        }
    }
...
GnssMeasurementSDIndexPairs formPhaserangeSDPair(const GnssMeasurement &measurement_rov,
                                                 const GnssMeasurement &measurement_ref,
                                                 const GnssCommonOptions &options)
{
    GnssMeasurementSDIndexPairs index_pairs;
    ...
    for (auto &sat : measurement_rov.satellites)
    {
        ...
        {
            GnssMeasurementIndex index_rov(satellite.prn, obs.first);
            if (checkObservationValid(measurement_rov, index_rov, ObservationType::Phaserange, options) &&
                checkObservationValid(measurement_rov, index_rov, ObservationType::Pseudorange, options))
            {
                indexes_1.push_back(index_rov);
            }
        }
    ...
    for (auto index : indexes_1)
    {
        ...
        for (auto &sat : measurement_ref.satellites)
        {
            ...
            {
                if (index.code_type != obs_2.first)
                    continue;
...
GnssMeasurementDDIndexPairs formPseudorangeDDPair(const GnssMeasurement &measurement_rov,
                                                  const GnssMeasurement &measurement_ref,
                                                  std::map<char, std::string> &system_to_base_prn,
                                                  const GnssCommonOptions &options)
{
    // Form SD pair
    GnssMeasurementSDIndexPairs sd_pairs = formPseudorangeSDPair(measurement_rov, measurement_ref, options);
    ...
    for (size_t i = 0; i < getGnssSystemList().size(); i++)
    {
        ...
        if (system_to_base_prn.find(system) != system_to_base_prn.end() && !system_to_base_prn.at(system).empty())
            continue;
        ...
        {
            if (sd_pairs[j].rov.prn[0] != system)
                continue;

            // we only select satellites with max phase number
            if (prn_to_number_codes.at(sd_pairs[j].rov.prn) != system_to_num_codes.at(system))
                continue;

            double elevation =
                satelliteElevation(measurement_ref.getSat(sd_pairs[j].ref).sat_position, measurement_ref.position);
    ...
    for (size_t i = 0; i < sd_pairs.size(); i++)
    {
        ...
        for (auto it = prn_to_indexes.lower_bound(prn_base); it != prn_to_indexes.upper_bound(prn_base); it++)
        {
            GnssMeasurementSDIndexPair &sd_pair_base = sd_pairs[it->second];
            if (sd_pair_base.rov.code_type == sd_pairs[i].rov.code_type)
            {
                dd_pairs.push_back(
                    GnssMeasurementDDIndexPair(sd_pairs[i].rov, sd_pairs[i].ref, sd_pair_base.rov, sd_pair_base.ref));
                break;
            }
        }
    }
...
GnssMeasurementDDIndexPairs formPhaserangeDDPair(const GnssMeasurement &measurement_rov,
                                                 const GnssMeasurement &measurement_ref,
                                                 std::map<char, std::string> &system_to_base_prn,
                                                 const GnssCommonOptions &options)
{
    // Form SD pair
    GnssMeasurementSDIndexPairs sd_pairs = formPhaserangeSDPair(measurement_rov, measurement_ref, options);
    ...
    for (size_t i = 0; i < getGnssSystemList().size(); i++)
    {
        ...
        for (size_t j = 0; j < sd_pairs.size(); j++)
        {
            ...
            // we only select satellites with max phase number
            if (prn_to_number_phases.at(sd_pairs[j].rov.prn) != system_to_num_phases.at(system))
                continue;
...
double troposphereSaastamoinen(double time, const Eigen::Vector3d &ecef, double elevation, double humi)
{
...
void troposphereGMF(double time, const Eigen::Vector3d &ecef, double elevation, double *gmfh, double *gmfw)
{
    ...
    static double ah_mean[55] = {+1.2517e+02, +8.503e-01, +6.936e-02, -6.760e+00, +1.771e-01, +1.130e-02, +5.963e-01,
    ...
    static double bh_mean[55] = {+0.000e+00, +0.000e+00, +3.249e-02, +0.000e+00, +3.324e-02, +1.850e-02, +0.000e+00,
    ...
    static double ah_amp[55] = {-2.738e-01, -2.837e+00, +1.298e-02, -3.588e-01, +2.413e-02, +3.427e-02, -7.624e-01,
    ...
    static double bh_amp[55] = {+0.000e+00, +0.000e+00, -1.136e-01, +0.000e+00, -1.868e-01, -1.399e-02, +0.000e+00,
    ...
    static double aw_mean[55] = {+5.640e+01, +1.555e+00, -1.011e+00, -3.975e+00, +3.171e-02, +1.065e-01, +6.175e-01,
    ...
    static double bw_mean[55] = {+0.000e+00, +0.000e+00, +2.592e-01, +0.000e+00, +2.974e-02, -5.471e-01, +0.000e+00,
    ...
    static double aw_amp[55] = {+1.023e-01, -2.695e+00, +3.417e-01, -1.405e-01, +3.175e-01, +2.116e-01, +3.536e+00,
    ...
    static double bw_amp[55] = {+0.000e+00, +0.000e+00, -8.865e-02, +0.000e+00, -4.309e-01, +6.340e-02, +0.000e+00,
    ...
    for (i = 0; i <= nmax; i++)
    {
        for (j = 0; j <= std::min(i, mmax); j++)
        {
            ...
            for (k = 0; k <= ir; k++)
            {
                sum = sum + pow(-1.0, k) * dfac[2 * i - 2 * k] / dfac[k] / dfac[i - k] / dfac[i - j - 2 * k] *
                                pow(t, i - j - 2 * k);
            }
    ...
    ch = c0h + ((cos(doy / 365.25 * TWOPI + phh) + 1.0) * c11h / 2.0 + c10h) * (1.0 - cos(lat));
...
double ionosphereBroadcast(double time, const Eigen::Vector3d &ecef, double azimuth, double elevation,
                           double wavelength, const Eigen::VectorXd &parameters)
{
    gtime_t gtime = doubleToGtime(time);
    gtime = utc2gpst(gtime);
    double azel[2];
    ...
    if (parameters.size() < 8)
        parameters_local = Eigen::VectorXd::Zero(8);
...
double ionosphereDualFrequency(const Observation &obs_1, const Observation &obs_2)
{
    CHECK(obs_1.wavelength < obs_2.wavelength);
    return (obs_1.pseudorange - obs_2.pseudorange) / (1.0 - square(obs_2.wavelength / obs_1.wavelength));
}

// Compute Receiver to satellite distance considering the earth rotation effect
double satelliteToReceiverDistance(const Eigen::Vector3d satellite_ecef, const Eigen::Vector3d receiver_ecef)
{
...
double satelliteElevation(const Eigen::Vector3d satellite_ecef, const Eigen::Vector3d receiver_ecef)
{
...
double satelliteAzimuth(const Eigen::Vector3d satellite_ecef, const Eigen::Vector3d receiver_ecef)
{
    Eigen::Vector3d rr = satellite_ecef - receiver_ecef;
    double rho0 = rr.norm();
...
// Compute DOPs
Eigen::Vector4d computeDops(const GnssMeasurement &measurement, const GnssCommonOptions &options)
{
    if (checkZero(measurement.position))
    {
    ...
    for (auto sat : measurement.satellites)
    {
        ...
        for (auto obs : sat.second.observations)
        {
            ...
            if (!checkObservationValid(measurement, index, ObservationType::Pseudorange, options))
                continue;
            double elevation = satelliteElevation(sat.second.sat_position, measurement.position);
            double azimuth = satelliteAzimuth(sat.second.sat_position, measurement.position);
            azel[2 * ns] = azimuth;
...
Eigen::Vector4d computeDops(const GnssMeasurement &measurement_rov, const GnssMeasurementSDIndexPairs &indexes,
                            const GnssCommonOptions &options)
{
    ...
    {
        if (index.rov.prn == current_prn)
            continue;
        current_prn = index.rov.prn;

        const auto &satellite = measurement_rov.getSat(index.rov);
        double elevation = satelliteElevation(satellite.sat_position, measurement_rov.position);
        double azimuth = satelliteAzimuth(satellite.sat_position, measurement_rov.position);
        azel[2 * ns] = azimuth;
        azel[2 * ns + 1] = elevation;
        ns++;
    }
...
Eigen::Vector4d computeDops(const GnssMeasurement &measurement_rov, const GnssMeasurementDDIndexPairs &indexes,
                            const GnssCommonOptions &options)
{
    if (checkZero(measurement_rov.position))
    {
    ...
    for (auto index : indexes)
    {
        if (index.rov_base.prn == current_prn)
            continue;
...
double combinationMW(const Observation &observation_1, const Observation &observation_2)
{
    ...
    double mw = (L1 * lam2 - L2 * lam1) / (lam2 - lam1) - (P1 * lam2 + P2 * lam1) / (lam2 + lam1);
    return mw;
}

// Geometry Free (GF) combination
double combinationGF(const Observation &observation_1, const Observation &observation_2)
{
    double L1 = observation_1.phaserange;
    double L2 = observation_2.phaserange;
...
double getBdsSatelliteMultipath(const std::string prn, const double elevation, const double code_type)
{
    ...
    {
        if (b < 0)
            return igso_coef[icoef][0];
        else if (b >= 9)
            return igso_coef[icoef][9];
        else
        {
    ...
    {
        if (b < 0)
            return meo_coef[icoef][0];
        else if (b >= 9)
            return meo_coef[icoef][9];
        else
        {
...
// Solid earth tide
Eigen::Vector3d solidEarthTide(const double time, const Eigen::Vector3d receiver_ecef)
{
    Eigen::Vector3d tide = Eigen::Vector3d::Zero();
    tidedisp(doubleToGtime(time), receiver_ecef.data(), 1, NULL, NULL, tide.data());
    if (tide.norm() > 10.0 || isnan(tide.norm()))
        tide = Eigen::Vector3d::Zero();
    return tide;
}
