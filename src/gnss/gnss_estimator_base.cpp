...
GnssEstimatorBase::GnssEstimatorBase(const GnssEstimatorBaseOptions &options, const EstimatorBaseOptions &base_options)
...
BackendId GnssEstimatorBase::addGnssPositionParameterBlock(const int32_t id, const Eigen::Vector3d &prior)
{
...
BackendId GnssEstimatorBase::addGnssVelocityParameterBlock(const int32_t id, const Eigen::Vector3d &prior)
{
...
BackendId GnssEstimatorBase::addGnssExtrinsicsParameterBlock(const int32_t id, const Eigen::Vector3d &t_SR_S_prior)
{
    ...
    std::shared_ptr<PositionParameterBlock> gnss_extrinsic_parameter_block =
        std::make_shared<PositionParameterBlock>(t_SR_S_prior, gnss_extrinsics_id.asInteger());
    CHECK(graph_->addParameterBlock(gnss_extrinsic_parameter_block));
    return gnss_extrinsics_id;
}

// Add GNSS clock blocks to graph
void GnssEstimatorBase::addClockParameterBlocks(const GnssMeasurement &measurement, const int32_t id,
                                                int &num_valid_system, const std::map<char, double> &prior,
                                                bool use_single_frequency)
{
    ...
    if (!is_verbose_model_)
    {
        for (auto &sat : measurement.satellites)
        {
            ...
            {
                if (!checkObservationValid(measurement, GnssMeasurementIndex(satellite.prn, obs.first)))
                    continue;
    ...
    for (auto it_system : system_observation_cnt)
    {
        ...
        if (prior.find(system) != prior.end())
        {
            ...
            information =
                Eigen::MatrixXd::Identity(1, 1) * square(1.0 / gnss_base_options_.error_parameter.initial_clock);
        }
        std::shared_ptr<ClockError> clock_error = std::make_shared<ClockError>(measurement, information);
        graph_->addResidualBlock(clock_error, nullptr, graph_->parameterBlockPtr(clock_id.asInteger()));
    }
}

// Add Inter-Frequency Bias (IFB) to graph
void GnssEstimatorBase::addIfbParameterBlocks(GnssMeasurement &measurement, const int32_t id)
{
    for (auto &sat : measurement.satellites)
    {
        auto &satellite = sat.second;
        ...
        for (auto &obs : satellite.observations)
        {
            ...
            if (std::find(ifbs_.begin(), ifbs_.end(), ifb) != ifbs_.end())
                continue;
            if (!checkObservationValid(measurement, GnssMeasurementIndex(satellite.prn, obs.first)))
                continue;
            ...
            CodeBias::BaseFrequencies bases = measurement.code_bias->getBase();
            std::pair<int, int> base_pair = bases.at(system);
            int phase_id_base_lhs = gnss_common::getPhaseID(system, base_pair.first);
            int phase_id_base_rhs = gnss_common::getPhaseID(system, base_pair.second);
            int phase_id = gnss_common::getPhaseID(system, obs.first);
            ...
            if (!is_base)
            {
                ...
                {
                    if (!checkObservationValid(measurement, GnssMeasurementIndex(satellite.prn, j_obs.first)))
                        continue;
            ...
            {
                Eigen::Matrix<double, 1, 1> init = Eigen::Matrix<double, 1, 1>::Identity() * init_ifb;
...
void GnssEstimatorBase::addFrequencyParameterBlocks(const GnssMeasurement &measurement, const int32_t id,
                                                    int &num_valid_system, const std::map<char, double> &prior,
                                                    bool use_single_frequency)
{
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        for (auto obs : satellite.observations)
        {
            const Observation &observation = measurement.getObs(GnssMeasurementIndex(satellite.prn, obs.first));
            if (!checkObservationValid(measurement, GnssMeasurementIndex(satellite.prn, obs.first), true))
                continue;
    ...
    for (auto it_system : system_observation_cnt)
    {
        ...
        std::shared_ptr<FrequencyError> freq_error = std::make_shared<FrequencyError>(measurement, information);
        graph_->addResidualBlock(freq_error, nullptr, graph_->parameterBlockPtr(freq_id.asInteger()));
    }
}
...
void GnssEstimatorBase::addTroposphereParameterBlock(const int32_t id)
{
    ...
    if (isFirstEpoch())
    {
        addTroposphereResidualBlock(tropo_id, tropo_init[0], gnss_base_options_.error_parameter.initial_troposphere);
    }
}

// Add ionosphere blocks to graph
void GnssEstimatorBase::addIonosphereParameterBlocks(const GnssMeasurement &measurement, const int32_t id,
                                                     IonosphereState &state)
{
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        if (ionosphere_states_.size() > 1)
        {
            ...
            {
                if (!sameIonosphere(lastIonosphereState().ids[j], iono_id))
                    continue;
                has_last = true;
                break;
            }
        ...
        if (!has_last)
        {
            double initial_ionosphere = gnss_base_options_.error_parameter.initial_ionosphere;
            if (satellite.ionosphere_type == IonoType::Broadcast || satellite.ionosphere_type == IonoType::None)
                initial_ionosphere += 1000.0;
...
void GnssEstimatorBase::addAmbiguityParameterBlocks(const GnssMeasurement &measurement, const int32_t id,
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        for (auto obs : satellite.observations)
        {
            ...
            CHECK(!graph_->parameterBlockExists(ambiguity_id.asInteger()))
                << "Ambiguity parameter for " << satellite.prn << " in phase " << phase_id
            ...
            if (ambiguity_states_.size() > 1)
                ...
                {
                    if (!sameAmbiguity(lastAmbiguityState().ids[j], ambiguity_id))
                        continue;
            ...
            if (!has_last)
            {
                double initial_ambiguity = gnss_base_options_.error_parameter.initial_ambiguity;
                if (satellite.ionosphere_type == IonoType::Broadcast || satellite.ionosphere_type == IonoType::None)
                    initial_ambiguity += 1000.0;
...
// Correct code biases
void GnssEstimatorBase::correctCodeBias(GnssMeasurement &measurement, const bool accept_coarse)
{
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        for (auto &obs : satellite.observations)
        {
            ...
            if (observation.pseudorange == 0.0)
                continue;
...
void GnssEstimatorBase::correctPhaseBias(GnssMeasurement &measurement)
{
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        std::string prn = satellite.prn;
        char system = prn[0];
        if (!gnss_common::useSystem(system_exclude, system) || !gnss_common::useSatellite(satellite_exclude, prn))
            continue;
        ...
        for (auto &obs : satellite.observations)
        {
            int code = obs.first;
            Observation &observation = obs.second;
            if (observation.phaserange == 0.0)
                continue;
            int phase_id = gnss_common::getPhaseID(prn[0], code);
            if (!gnss_common::usePhase(phase_exclude, system, phase_id))
                continue;
...
void GnssEstimatorBase::correctBdsSatelliteMultipath(GnssMeasurement &measurement)
{
    for (auto &sat : measurement.satellites)
    {
        ...
        if (checkZero(satellite.sat_position))
            return;
        double elevation = gnss_common::satelliteElevation(satellite.sat_position, measurement.position);
        for (auto &obs : satellite.observations)
        {
            int code = obs.first;
            if (obs.second.pseudorange == 0.0)
                continue;
            obs.second.pseudorange += gnss_common::getBdsSatelliteMultipath(prn, elevation, code);
        }
    }
}

// Compute and set ionosphere delays using dual-frequency or model
void GnssEstimatorBase::computeIonosphereDelay(GnssMeasurement &measurement, bool use_single_frequency)
{
    for (auto &sat : measurement.satellites)
    {
        auto &satellite = sat.second;
        ...
        if (satellite.ionosphere_type == IonoType::Augmentation)
            continue;
        ...
        for (auto obs : satellite.observations)
        {
            ...
            if (!found)
                valid_observations.push_back(obs.second);
            ...
            if (!found)
                dual_frequency_observations.push_back(obs.second);
        }
        ...
        if (!use_single_frequency && dual_frequency_observations.size() > 1)
        {
            ...
            std::sort(dual_frequency_observations.begin(), dual_frequency_observations.end(),
                      [](Observation &lhs, Observation &rhs) { return lhs.wavelength < rhs.wavelength; });

            // check error ratio
            double ratio = fabs(1.0 / (1.0 - square(dual_frequency_observations.back().wavelength /
                                                    dual_frequency_observations.front().wavelength)));
            if (ratio < 3.0)
            {
                // compute
                double ionosphere = gnss_common::ionosphereDualFrequency(dual_frequency_observations.front(),
                                                                         dual_frequency_observations.back());
                ionosphere_delay =
                    gnss_common::ionosphereConvertToBase(ionosphere, dual_frequency_observations.front().wavelength);
        ...
        if (!computed)
        {
            ...
            {
                double azimuth = gnss_common::satelliteAzimuth(satellite.sat_position, position);
                double elevation = gnss_common::satelliteElevation(satellite.sat_position, position);
                double wavelength = valid_observations[0].wavelength;
                Eigen::VectorXd iono_parameters = measurement.ionosphere_parameters;
                ionosphere_delay = gnss_common::ionosphereBroadcast(timestamp, position, azimuth, elevation, wavelength,
                                                                    iono_parameters);
                ionosphere_delay = gnss_common::ionosphereConvertToBase(ionosphere_delay, wavelength);
            }
...
void GnssEstimatorBase::addGnssPositionResidualBlock(const State &state, const Eigen::Vector3d &position,
                                                     const double std)
{
    Eigen::Matrix3d information = Eigen::Matrix3d::Identity() * (1.0 / square(std));
    std::shared_ptr<PositionError<3>> position_error = std::make_shared<PositionError<3>>(position, information);
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(position_error, nullptr, graph_->parameterBlockPtr(state.id_in_graph.asInteger()));
}

// Add velocity residual block to graph
void GnssEstimatorBase::addGnssVelocityResidualBlock(const State &state, const Eigen::Vector3d &velocity,
                                                     const double std)
{
    BackendId velocity_id = createGnssVelocityId(state.id_in_graph.bundleId());
    Eigen::Matrix3d information = Eigen::Matrix3d::Identity() * (1.0 / square(std));
    std::shared_ptr<VelocityError<3>> velocity_error = std::make_shared<VelocityError<3>>(velocity, information);
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(velocity_error, nullptr, graph_->parameterBlockPtr(velocity_id.asInteger()));
}

// Add pseudorange residual blocks to graph
void GnssEstimatorBase::addPseudorangeResidualBlocks(const GnssMeasurement &measurement, const State &state,
    ...
    else
    {
        for (auto &sat : measurement.satellites)
        {
            ...
            for (auto obs : satellite.observations)
            {
                ...
                if (!graph_->parameterBlockExists(ifb_id.asInteger()))
                {
                    LOG(WARNING) << "IFB for code " << obs.first << " for system " << satellite.prn[0]
                ...
                if (parameter_id.type() == IdType::gPosition)
                {
                    ...
                    std::shared_ptr<PseudorangeError<3, 1, 1, 1, 1>> pseudorange_error =
                        std::make_shared<PseudorangeError<3, 1, 1, 1, 1>>(
                            measurement, GnssMeasurementIndex(satellite.prn, obs.first),
                            gnss_base_options_.error_parameter);
                    residual_id = graph_->addResidualBlock(
                        pseudorange_error, huber_loss_function_ ? huber_loss_function_.get() : nullptr,
                        graph_->parameterBlockPtr(parameter_id.asInteger()),
                        graph_->parameterBlockPtr(clock_id.asInteger()), graph_->parameterBlockPtr(ifb_id.asInteger()),
                ...
                else
                {
                    ...
                    std::shared_ptr<PseudorangeError<7, 3, 1, 1, 1, 1>> pseudorange_error =
                        std::make_shared<PseudorangeError<7, 3, 1, 1, 1, 1>>(
                            measurement, GnssMeasurementIndex(satellite.prn, obs.first),
                            gnss_base_options_.error_parameter);
                    pseudorange_error->setCoordinate(coordinate_);
                    residual_id = graph_->addResidualBlock(
...
void GnssEstimatorBase::addPhaserangeResidualBlocks(const GnssMeasurement &measurement, const State &state)
{
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        for (auto obs : satellite.observations)
        {
            ...
            BackendId ambiguity_id = createGnssAmbiguityId(satellite.prn, phase_id, parameter_id.bundleId());
            BackendId troposphere_id = createGnssTroposphereId(measurement.id);
            BackendId ionosphere_id = createGnssIonosphereId(satellite.prn, measurement.id);
...
void GnssEstimatorBase::addDopplerResidualBlocks(const GnssMeasurement &measurement, const State &state,
                                                 int &num_valid_satellite, bool use_single_frequency,
    ...
    for (auto &sat : measurement.satellites)
    {
        ...
        for (auto obs : satellite.observations)
        {
            ...
            if (parameter_id.type() == IdType::gPosition)
            {
                ...
                std::shared_ptr<DopplerError<3, 3, 1>> doppler_error = std::make_shared<DopplerError<3, 3, 1>>(
                    measurement, GnssMeasurementIndex(satellite.prn, obs.first), gnss_base_options_.error_parameter);
                graph_->addResidualBlock(doppler_error, huber_loss_function_ ? huber_loss_function_.get() : nullptr,
                                         graph_->parameterBlockPtr(parameter_id.asInteger()),
            ...
            else
            {
                ...
                BackendId freq_id = createGnssFrequencyId(system, measurement.id);
                std::shared_ptr<DopplerError<7, 9, 3, 1>> doppler_error = std::make_shared<DopplerError<7, 9, 3, 1>>(
                    measurement, GnssMeasurementIndex(satellite.prn, obs.first), gnss_base_options_.error_parameter,
                    angular_velocity);
...
void GnssEstimatorBase::addTroposphereResidualBlock(const BackendId &tropo_id, const double value, const double std)
{
    CHECK(tropo_id.type() == IdType::gTroposphere);
    Eigen::Matrix<double, 1, 1> covariance = Eigen::Matrix<double, 1, 1>::Identity() * square(std);
    ...
    graph_->addResidualBlock(tropo_error, nullptr, graph_->parameterBlockPtr(tropo_id.asInteger()));
}

// Add ionosphere residual block to graph
void GnssEstimatorBase::addIonosphereResidualBlock(const BackendId &iono_id, const double value, const double std)
{
    CHECK(iono_id.type() == IdType::gIonosphere);
    Eigen::Matrix<double, 1, 1> covariance = Eigen::Matrix<double, 1, 1>::Identity() * square(std);
    const Eigen::Map<const Eigen::VectorXd> iono_init(&value, 1);
    std::shared_ptr<IonosphereError> iono_error = std::make_shared<IonosphereError>(iono_init, covariance.inverse());
    graph_->addResidualBlock(iono_error, nullptr, graph_->parameterBlockPtr(iono_id.asInteger()));
}

// Add ambiguity residual block to graph
void GnssEstimatorBase::addAmbiguityResidualBlock(const BackendId &amb_id, const double value, const double std)
{
    ...
    graph_->addResidualBlock(amb_error, nullptr, graph_->parameterBlockPtr(amb_id.asInteger()));
}

// Add relative position block to graph
void GnssEstimatorBase::addRelativePositionResidualBlock(const State &last_state, const State &cur_state)
{
    ...
    for (size_t i = 0; i < 3; i++)
        CHECK(dp_error(i) != 0.0);
    Eigen::Matrix3d dp_covariance = (cwiseSquare(dp_error) * dt).asDiagonal();
    dp_covariance = coordinate_->convertCovariance(dp_covariance, GeoType::ENU, GeoType::ECEF);

    std::shared_ptr<RelativePositionError> relative_position_error =
        std::make_shared<RelativePositionError>(dp_covariance.inverse());
    graph_->addResidualBlock(relative_position_error, nullptr, graph_->parameterBlockPtr(last_id.asInteger()),
                             graph_->parameterBlockPtr(cur_id.asInteger()));
}

// Add relative position and velocity block to graph
void GnssEstimatorBase::addRelativePositionAndVelocityResidualBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    CHECK(last_id.type() == IdType::gPosition);
    ...
    graph_->addResidualBlock(relative_error, nullptr, graph_->parameterBlockPtr(last_id.asInteger()),
                             graph_->parameterBlockPtr(cur_id.asInteger()),
...
void GnssEstimatorBase::addRelativeIsbResidualBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    ...
    {
        if (system == 'R')
            continue; // GLONASS clock is considered as unstable
        ...
        if (!graph_->parameterBlockExists(last_clock_id.asInteger()))
            continue;
        if (!graph_->parameterBlockExists(cur_clock_id.asInteger()))
            continue;
        cur_clock_ids.push_back(cur_clock_id);
        last_clock_ids.push_back(last_clock_id);
    }
    CHECK(cur_clock_ids.size() == last_clock_ids.size());
    if (cur_clock_ids.size() < 2)
        return;
    ...
    for (size_t i = 1; i < cur_clock_ids.size(); i++)
    {
        ...
        graph_->addResidualBlock(relative_isb_error, nullptr, graph_->parameterBlockPtr(last_clock_ids[0].asInteger()),
...
void GnssEstimatorBase::addRelativeFrequencyResidualBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    ...
    for (auto system : getGnssSystemList())
    {
        ...
        if (!graph_->parameterBlockExists(last_freq_id.asInteger()))
            continue;
        if (!graph_->parameterBlockExists(cur_freq_id.asInteger()))
            continue;
        ...
        graph_->addResidualBlock(relative_freq_error, nullptr, graph_->parameterBlockPtr(last_freq_id.asInteger()),
...
void GnssEstimatorBase::addRelativeTroposphereResidualBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    ...
    Eigen::Matrix<double, 1, 1> dtropo_covariance = Eigen::Matrix<double, 1, 1>::Identity() * square(dtropo_error) * dt;
    ...
    graph_->addResidualBlock(relative_tropo_error, nullptr, graph_->parameterBlockPtr(last_tropo_id.asInteger()),
...
void GnssEstimatorBase::addRelativeIonosphereResidualBlock(const IonosphereState &last_state,
                                                           const IonosphereState &cur_state)
{
    double dt = cur_state.timestamp - last_state.timestamp;
    CHECK(dt >= 0.0);
    double diono_error = gnss_base_options_.error_parameter.relative_ionosphere;
    CHECK(diono_error != 0.0);
    Eigen::Matrix<double, 1, 1> diono_covariance = Eigen::Matrix<double, 1, 1>::Identity() * square(diono_error) * dt;

    for (size_t i = 0; i < cur_state.ids.size(); i++)
    {
        for (size_t j = 0; j < last_state.ids.size(); j++)
        {
            if (!sameIonosphere(last_state.ids[j], cur_state.ids[i]))
                continue;
...
void GnssEstimatorBase::addRelativeAmbiguityResidualBlock(GnssMeasurement &last_measurement,
                                                          GnssMeasurement &cur_measurement,
                                                          const AmbiguityState &last_state,
                                                          const AmbiguityState &cur_state)
{
    double dt = cur_state.timestamp - last_state.timestamp;
    CHECK(dt >= 0.0);
    double damb_error = gnss_base_options_.error_parameter.relative_ambiguity;
    CHECK(damb_error != 0.0);
    Eigen::Matrix<double, 1, 1> damb_covariance = Eigen::Matrix<double, 1, 1>::Identity() * square(damb_error) * dt;

    for (size_t i = 0; i < cur_state.ids.size(); i++)
    {
        for (size_t j = 0; j < last_state.ids.size(); j++)
        {
            if (!sameAmbiguity(last_state.ids[j], cur_state.ids[i]))
                continue;
...
size_t GnssEstimatorBase::numPseudorangeError(const State &state)
{
    ...
    Graph::ResidualBlockCollection residual_blocks = graph_->residuals(parameter_id.asInteger());
    for (size_t i = 0; i < residual_blocks.size(); i++)
    {
        ...
        if (!(type == ErrorType::kPseudorangeError || type == ErrorType::kPseudorangeErrorSD ||
...
size_t GnssEstimatorBase::numPhaserangeError(const State &state)
{
    ...
    for (size_t i = 0; i < residual_blocks.size(); i++)
    {
        ...
        std::shared_ptr<ErrorInterface> interface = residual_block.error_interface_ptr;
        ErrorType type = interface->typeInfo();
        if (!(type == ErrorType::kPhaserangeError || type == ErrorType::kPhaserangeErrorSD ||
...
size_t GnssEstimatorBase::rejectPseudorangeOutlier(const State &state, bool reject_one)
{
    ...
    for (size_t i = 0; i < residual_blocks.size(); i++)
    {
        ...
        if (!(type == ErrorType::kPseudorangeError || type == ErrorType::kPseudorangeErrorSD ||
            ...
            continue;
        double residual[1];
        graph_->problem()->EvaluateResidualBlock(residual_block.residual_block_id, false, nullptr, residual, nullptr);
        interface->deNormalizeResidual(residual);
        residuals.push_back(*residual);
        residual_index_to_id.insert(std::make_pair(residuals.size() - 1, residual_block.residual_block_id));
        residual_index_to_interface.insert(std::make_pair(residuals.size() - 1, residual_block.error_interface_ptr));
    }
    if (residuals.size() == 0)
        return 0;
    ...
    if (outlier_indexes.size() > 0)
    {
        ...
        {
            graph_->removeResidualBlock(residual_index_to_id.at(static_cast<size_t>(index)));
            if (base_options_.verbose_output)
            {
                ...
                std::string code_str;
#define MAP(S, R, C)                                                                                                   \
    if (system == S && code_type == C)                                                                                 \
    {                                                                                                                  \
        code_str = R;                                                                                                  \
    }
                if (type == ErrorType::kPseudorangeError)
                {
                    GnssMeasurementIndex index = getGnssMeasurementIndexFromErrorInterface(error_interface);
                    system = index.prn[0];
                ...
                {
                    GnssMeasurementSDIndexPair index = getGnssMeasurementSDIndexPairFromErrorInterface(error_interface);
                    ...
                    info_message += " at " + index.rov.prn + "|" + code_str_rov + "&" + code_str_ref;
                }
                if (type == ErrorType::kPseudorangeErrorDD)
                {
                    GnssMeasurementDDIndexPair index = getGnssMeasurementDDIndexPairFromErrorInterface(error_interface);
                    system = index.rov.prn[0];
                    code_type = index.rov.code_type;
                    RINEX_TO_CODE_MAPS;
                    std::string code_str_rov = code_str;
                    ...
                    info_message += " at " + index.rov.prn + "|" + code_str_rov + "&" + code_str_ref + "-" +
                                    index.rov_base.prn + "|" + code_str_rov_base + "&" + code_str_ref_base;
                }
#undef MAP
                LOG(INFO) << "Rejected pseudorange outlier" << info_message << ": residual = " << std::fixed
                          << residuals[index];
            }
        }
        ...
        for (auto system : getGnssSystemList())
        {
            ...
            if (!graph_->parameterBlockExists(clock_id.asInteger()))
                continue;
            if (graph_->residuals(clock_id.asInteger()).size() != 0)
                continue;
            std::shared_ptr<ClockParameterBlock> block_ptr =
                std::static_pointer_cast<ClockParameterBlock>(graph_->parameterBlockPtr(clock_id.asInteger()));
...
size_t GnssEstimatorBase::rejectPseudorangeOutlier(const State &state, AmbiguityState &ambiguity_state, bool reject_one)
{
        ...
        code_str = R;                                                                                                  \
            ...
            if (type == ErrorType::kPseudorangeError)
            {
                ...
                for (auto it = ambiguity_state.ids.begin(); it != ambiguity_state.ids.end();)
                {
                    ...
                    if (!sameAmbiguity(ambiguity_id, createGnssAmbiguityId(index.prn, phase_id, 0)))
                    {
                        it++;
                        continue;
                    }
            ...
            if (type == ErrorType::kPseudorangeErrorSD)
            {
                ...
                {
                    auto &ambiguity_id = *it;
                    if (!sameAmbiguity(ambiguity_id, createGnssAmbiguityId(index.rov.prn, phase_id, 0)))
                    {
                        it++;
                        continue;
                    }
...
size_t GnssEstimatorBase::rejectPhaserangeOutlier(const State &state, AmbiguityState &ambiguity_state, bool reject_one)
{
    const BackendId &parameter_id = state.id_in_graph;
    ...
    if (outlier_indexes.size() > 0)
    {
        ...
        for (auto index : indexes_to_remove)
        {
            ...
            Graph::ParameterBlockCollection parameters =
                graph_->parameters(residual_index_to_id.at(static_cast<size_t>(index)));
            BackendId id;
            for (auto &parameter : parameters)
            {
                ...
                for (auto it = ambiguity_state.ids.begin(); it != ambiguity_state.ids.end(); it++)
                {
                    if (*it == id)
                    {
                        it = ambiguity_state.ids.erase(it);
    ...
    {                                                                                                                  \
        phase_str = PS;                                                                                                \
                ...
                if (type == ErrorType::kPhaserangeErrorSD)
                {
                    ...
                    info_message += " at " + index.rov.prn + "|" + phase_str_rov + "&" + phase_str_ref;
                }
                if (type == ErrorType::kPhaserangeErrorDD)
                {
                    ...
                    info_message += " at " + index.rov.prn + "|" + phase_str_rov + "&" + phase_str_ref + "-" +
                                    index.rov_base.prn + "|" + phase_str_rov_base + "&" + phase_str_ref_base;
                }
#undef MAP
                LOG(INFO) << "Rejected phaserange outlier" << info_message << ": residual = " << std::fixed
                          << residuals[index];
            }
        }
...
#undef MAP
                LOG(INFO) << "Rejected doppler outlier" << info_message << ": residual = " << std::fixed
                          << residuals[index];
            }
        }
        ...
        for (auto system : getGnssSystemList())
        {
            ...
            if (!graph_->parameterBlockExists(freq_id.asInteger()))
                continue;
            if (graph_->residuals(freq_id.asInteger()).size() != 0)
                continue;
            std::shared_ptr<FrequencyParameterBlock> block_ptr =
                std::static_pointer_cast<FrequencyParameterBlock>(graph_->parameterBlockPtr(freq_id.asInteger()));
            CHECK(block_ptr != nullptr);
            Eigen::VectorXd measurement = block_ptr->estimate();
            Eigen::MatrixXd information = Eigen::MatrixXd::Identity(1, 1) * 1e-6;
            std::shared_ptr<FrequencyError> freq_error = std::make_shared<FrequencyError>(measurement, information);
...
void GnssEstimatorBase::addGnssPositionMarginBlockWithResiduals(const State &state, bool keep)
{
    ...
    {
        Graph::ResidualBlockCollection residuals = graph_->residuals(position_id.asInteger());
        for (size_t r = 0; r < residuals.size(); ++r)
        {
...
void GnssEstimatorBase::addGnssVelocityMarginBlockWithResiduals(const State &state, bool keep)
{
    ...
    {
        Graph::ResidualBlockCollection residuals = graph_->residuals(velocity_id.asInteger());
        for (size_t r = 0; r < residuals.size(); ++r)
        {
            marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
        }
...
void GnssEstimatorBase::addClockMarginBlocksWithResiduals(const State &state, bool keep)
{
    ...
    for (size_t i = 0; i < getGnssSystemList().size(); i++)
    {
        ...
        if (graph_->parameterBlockExists(clock_id.asInteger()))
        {
            Graph::ResidualBlockCollection residuals = graph_->residuals(clock_id.asInteger());
            for (size_t r = 0; r < residuals.size(); ++r)
            {
                marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
            }
...
void GnssEstimatorBase::addFrequencyMarginBlocksWithResiduals(const State &state, bool keep)
{
    ...
    for (auto system : getGnssSystemList())
    {
        ...
        if (graph_->parameterBlockExists(freq_id.asInteger()))
        {
            Graph::ResidualBlockCollection residuals = graph_->residuals(freq_id.asInteger());
            for (size_t r = 0; r < residuals.size(); ++r)
            {
                marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
            }
...
void GnssEstimatorBase::addTroposphereMarginBlockWithResiduals(const State &state, bool keep)
{
    ...
    {
        Graph::ResidualBlockCollection residuals = graph_->residuals(tropo_id.asInteger());
        for (size_t r = 0; r < residuals.size(); ++r)
        {
            marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
        }
...
void GnssEstimatorBase::addIonosphereMarginBlocksWithResiduals(const IonosphereState &state, bool keep)
{
...
void GnssEstimatorBase::addAmbiguityMarginBlocksWithResiduals(const AmbiguityState &state, bool keep)
{
    for (auto id : state.ids)
    {
        if (graph_->parameterBlockExists(id.asInteger()))
...
void GnssEstimatorBase::addGnssMeasurementResidualMarginBlocks(const State &state)
{
    const std::unordered_set<int> gnss_error_types = {
        static_cast<int>(ErrorType::kPseudorangeError),   static_cast<int>(ErrorType::kPseudorangeErrorSD),
        static_cast<int>(ErrorType::kPseudorangeErrorDD), static_cast<int>(ErrorType::kPhaserangeError),
        static_cast<int>(ErrorType::kPhaserangeErrorSD),  static_cast<int>(ErrorType::kPhaserangeErrorDD),
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        ...
        if (gnss_error_types.find(type) == gnss_error_types.end())
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
void GnssEstimatorBase::addGnssResidualMarginBlocks(const State &state)
{
    ...
    const std::unordered_set<int> gnss_error_types = {static_cast<int>(ErrorType::kPseudorangeError),
...
void GnssEstimatorBase::addGnssLooseResidualMarginBlocks(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kPositionError &&
            residuals[r].error_interface_ptr->typeInfo() != ErrorType::kVelocityError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
void GnssEstimatorBase::eraseFrequencyParameterBlocks(const State &state, bool reform)
{
    const BackendId &parameter_id = state.id;
    for (auto system : getGnssSystemList())
    {
    ...
    for (size_t i = 0; i < states_.size(); i++)
    {
        if (!states_[i].valid())
            continue;
        if (states_[i].id.type() != IdType::gPosition && states_[i].id.type() != IdType::gPose)
            continue;
...
void GnssEstimatorBase::eraseTroposphereParameterBlock(const State &state, bool reform)
{
    const BackendId &parameter_id = state.id;
    BackendId tropo_id = changeIdType(parameter_id, IdType::gTroposphere);
    if (graph_->parameterBlockExists(tropo_id.asInteger()))
...
void GnssEstimatorBase::eraseIonosphereParameterBlocks(IonosphereState &state, bool reform)
{
    for (auto id : state.ids)
    {
        if (graph_->parameterBlockExists(id.asInteger()))
    ...
    if (reform && index_lhs != -1 && index_rhs != -1)
    {
        addRelativeIonosphereResidualBlock(ionosphere_states_[index_lhs], ionosphere_states_[index_rhs]);
    }
}

// Erase ambiguity blocks
void GnssEstimatorBase::eraseAmbiguityParameterBlocks(AmbiguityState &state, bool reform)
{
    ...
    {
        double dt = ambiguity_states_[index_rhs].timestamp - ambiguity_states_[index_lhs].timestamp;
        CHECK(dt >= 0.0);
        double damb_error = gnss_base_options_.error_parameter.relative_ambiguity;
        CHECK(damb_error != 0.0);
        Eigen::Matrix<double, 1, 1> damb_covariance = Eigen::Matrix<double, 1, 1>::Identity() * square(damb_error) * dt;
        ...
        for (auto id : state.ids)
        {
            ...
            {
                if (residual.error_interface_ptr->typeInfo() != ErrorType::kRelativeAmbiguityError)
                    continue;
            ...
            if (reform && connected_ids.size() == 2)
            {
                ...
                if (BackendId(connected_ids[0]).bundleId() < BackendId(connected_ids[1]).bundleId())
                {
                    last_id = connected_ids[0];
                    cur_id = connected_ids[1];
                }
                else
                {
                    last_id = connected_ids[1];
                    cur_id = connected_ids[0];
                }

                std::shared_ptr<RelativeAmbiguityError> relative_amb_error =
                    std::make_shared<RelativeAmbiguityError>(damb_covariance.inverse());
                graph_->addResidualBlock(relative_amb_error, nullptr, graph_->parameterBlockPtr(last_id),
                                         graph_->parameterBlockPtr(cur_id));

                // reset initial value
                *graph_->parameterBlockPtr(cur_id)->parameters() = *graph_->parameterBlockPtr(last_id)->parameters();
...
void GnssEstimatorBase::erasePseudorangeResidualBlocks(const State &state)
{
    ...
    {
        if (residual_block.error_interface_ptr->typeInfo() == ErrorType::kPseudorangeError ||
            residual_block.error_interface_ptr->typeInfo() == ErrorType::kPseudorangeErrorSD ||
            residual_block.error_interface_ptr->typeInfo() == ErrorType::kPseudorangeErrorDD)
        {
...
void GnssEstimatorBase::eraseGnssLooseResidualBlocks(const State &state)
{
    ...
    for (auto residual_block : residual_blocks)
    {
        if (residual_block.error_interface_ptr->typeInfo() == ErrorType::kPositionError ||
            residual_block.error_interface_ptr->typeInfo() == ErrorType::kVelocityError)
        {
            graph_->removeResidualBlock(residual_block.residual_block_id);
        }
    }
...
void GnssEstimatorBase::eraseRelativePositionResidualBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;

    Graph::ResidualBlockCollection residual_blocks = graph_->residuals(cur_id.asInteger());
    for (auto residual_block : residual_blocks)
    {
        if (residual_block.error_interface_ptr->typeInfo() != ErrorType::kRelativePositionError)
            continue;
...
void GnssEstimatorBase::eraseRelativePositionAndVelocityBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    ...
    for (auto residual_block : residual_blocks)
    {
        if (residual_block.error_interface_ptr->typeInfo() != ErrorType::kRelativePositionAndVelocityError)
            continue;
        auto parameters = graph_->parameters(residual_block.residual_block_id);
        bool found = false;
        for (auto parameter : parameters)
...
void GnssEstimatorBase::eraseRelativeFrequencyBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    ...
    for (auto system : getGnssSystemList())
    {
        ...
        Graph::ResidualBlockCollection residual_blocks = graph_->residuals(cur_freq_id.asInteger());
        for (auto residual_block : residual_blocks)
        {
            if (residual_block.error_interface_ptr->typeInfo() != ErrorType::kRelativeFrequencyError)
                continue;
            auto parameters = graph_->parameters(residual_block.residual_block_id);
            bool found = false;
            for (auto parameter : parameters)
...
void GnssEstimatorBase::eraseRelativeTroposphereResidualBlock(const State &last_state, const State &cur_state)
{
    const BackendId last_id = last_state.id;
    const BackendId cur_id = cur_state.id;
    ...
    Graph::ResidualBlockCollection residual_blocks = graph_->residuals(cur_tropo_id.asInteger());
    for (auto residual_block : residual_blocks)
    {
        if (residual_block.error_interface_ptr->typeInfo() != ErrorType::kRelativeTroposphereError)
            continue;
        auto parameters = graph_->parameters(residual_block.residual_block_id);
        bool found = false;
        for (auto parameter : parameters)
...
void GnssEstimatorBase::eraseRelativeIonosphereResidualBlock(const IonosphereState &last_state,
                                                             const IonosphereState &cur_state)
{
    if (last_state.ids.size() == 0)
        return;
    for (size_t i = 0; i < cur_state.ids.size(); i++)
    {
        Graph::ResidualBlockCollection residual_blocks = graph_->residuals(cur_state.ids[i].asInteger());
        for (auto residual_block : residual_blocks)
        {
            if (residual_block.error_interface_ptr->typeInfo() != ErrorType::kRelativeIonosphereError)
                continue;
            auto parameters = graph_->parameters(residual_block.residual_block_id);
            bool found = false;
            for (auto parameter : parameters)
            {
                if (BackendId(parameter.first).bundleId() == last_state.ids[0].bundleId())
                {
                    found = true;
                    break;
                }
...
void GnssEstimatorBase::eraseRelativeAmbiguityResidualBlock(const AmbiguityState &last_state,
                                                            const AmbiguityState &cur_state)
{
    if (last_state.ids.size() == 0)
        return;
    for (size_t i = 0; i < cur_state.ids.size(); i++)
    {
        ...
        for (auto residual_block : residual_blocks)
        {
            if (residual_block.error_interface_ptr->typeInfo() != ErrorType::kRelativeAmbiguityError)
                continue;
            auto parameters = graph_->parameters(residual_block.residual_block_id);
            bool found = false;
            for (auto parameter : parameters)
...
void GnssEstimatorBase::convertStateAndCovarianceToBody(Transformation *T_WS, SpeedAndBias *speed_and_bias,
...
Eigen::Vector3d GnssEstimatorBase::getGnssExtrinsicsEstimate()
{
    std::shared_ptr<PositionParameterBlock> block_ptr =
        std::static_pointer_cast<PositionParameterBlock>(graph_->parameterBlockPtr(gnss_extrinsics_id_.asInteger()));
    CHECK(block_ptr != nullptr);
