...
void GnssEstimatorBase::addSdClockParameterBlocks(const GnssMeasurement &measurement_rov,
...
void GnssEstimatorBase::addSdAmbiguityParameterBlocks(const GnssMeasurement &measurement_rov,
                                                      const GnssMeasurement &measurement_ref,
                                                      ...
                                                      AmbiguityState &state)
{
    ...
    for (auto index_pair : index_pairs)
    {
        ...
        const Satellite &satellite_base = measurement_rov.getSat(index_pair.rov_base);
        const Satellite &satellite_ref = measurement_ref.getSat(index_pair.ref);
        const Satellite &satellite_ref_base = measurement_ref.getSat(index_pair.ref_base);
        const Observation &observation = satellite.observations.at(index_pair.rov.code_type);
        const Observation &observation_base = satellite_base.observations.at(index_pair.rov_base.code_type);
        const Observation &observation_ref = satellite_ref.observations.at(index_pair.ref.code_type);
        const Observation &observation_ref_base = satellite_ref_base.observations.at(index_pair.ref_base.code_type);
        char system = satellite.getSystem();
        ...
        CHECK(!graph_->parameterBlockExists(ambiguity_id.asInteger()))
            << "Ambiguity parameter for " << satellite.prn << " in phase " << phase_id << " has already been added!";
        ...
        double ambiguity = phaserange - phaserange_ref - (pseudorange - pseudorange_ref);
        ...
        BackendId ambiguity_base_id = createGnssAmbiguityId(satellite_base.prn, phase_id, id);
        if (!graph_->parameterBlockExists(ambiguity_base_id.asInteger()))
        {
            ...
            ambiguity = phaserange - phaserange_ref - (pseudorange - pseudorange_ref);
            ...
            std::shared_ptr<AmbiguityParameterBlock> ambiguity_base_parameter_block =
                std::make_shared<AmbiguityParameterBlock>(init_base, ambiguity_base_id.asInteger());
        ...
        if (!has_last)
        {
            addAmbiguityResidualBlock(ambiguity_id, *graph_->parameterBlockPtr(ambiguity_id.asInteger())->parameters(),
                                      gnss_base_options_.error_parameter.initial_ambiguity);
        }
    }
}
...
void GnssEstimatorBase::addSdPseudorangeResidualBlocks(const GnssMeasurement &measurement_rov,
                                                       const GnssMeasurement &measurement_ref,
                                                       const GnssMeasurementSDIndexPairs &index_pairs,
                                                       const State &state, int &num_valid_satellite,
                                                       bool use_single_frequency)
{
    ...
    if (!is_verbose_model_)
    {
        ...
        for (auto &index_pair : index_pairs)
        {
            ...
            BackendId clock_id = createGnssClockId(satellite.getSystem(), measurement_rov.id);
            ...
            if (parameter_id.type() == IdType::gPosition)
            {
                ...
                std::shared_ptr<PseudorangeErrorSD<3, 1>> pseudorange_error =
                    std::make_shared<PseudorangeErrorSD<3, 1>>(measurement_rov, measurement_ref, index_pair.rov,
                                                               index_pair.ref, gnss_base_options_.error_parameter);
                graph_->addResidualBlock(pseudorange_error, huber_loss_function_ ? huber_loss_function_.get() : nullptr,
            ...
            else
            {
                ...
                std::shared_ptr<PseudorangeErrorSD<7, 3, 1>> pseudorange_error =
                    std::make_shared<PseudorangeErrorSD<7, 3, 1>>(measurement_rov, measurement_ref, index_pair.rov,
                                                                  index_pair.ref, gnss_base_options_.error_parameter);
...
void GnssEstimatorBase::addDdPseudorangeResidualBlocks(const GnssMeasurement &measurement_rov,
                                                       const GnssMeasurement &measurement_ref,
                                                       ...
                                                       bool use_single_frequency)
{
    ...
    if (!is_verbose_model_)
    {
        ...
        for (auto &index_pair : index_pairs)
        {
            ...
            {
                is_state_pose_ = false;
                std::shared_ptr<PseudorangeErrorDD<3>> pseudorange_error = std::make_shared<PseudorangeErrorDD<3>>(
                    measurement_rov, measurement_ref, index_pair.rov, index_pair.ref, index_pair.rov_base,
                    index_pair.ref_base, gnss_base_options_.error_parameter);
                graph_->addResidualBlock(pseudorange_error, huber_loss_function_ ? huber_loss_function_.get() : nullptr,
                                         graph_->parameterBlockPtr(parameter_id.asInteger()));
            }
            // pose in ENU for fusion
            else
            {
                ...
                std::shared_ptr<PseudorangeErrorDD<7, 3>> pseudorange_error =
                    std::make_shared<PseudorangeErrorDD<7, 3>>(measurement_rov, measurement_ref, index_pair.rov,
                                                               index_pair.ref, index_pair.rov_base, index_pair.ref_base,
...
void GnssEstimatorBase::addDdPhaserangeResidualBlocks(const GnssMeasurement &measurement_rov,
                                                      const GnssMeasurement &measurement_ref,
                                                      const GnssMeasurementDDIndexPairs &index_pairs,
                                                      const State &state)
{
    ...
    if (!is_verbose_model_)
    {
        for (auto &index_pair : index_pairs)
        {
            ...
            BackendId ambiguity_base_id = createGnssAmbiguityId(satellite_base.prn, phase_id, parameter_id.bundleId());
            ...
            if (parameter_id.type() == IdType::gPosition)
            {
                ...
                std::shared_ptr<PhaserangeErrorDD<3, 1, 1>> phaserange_error =
                    std::make_shared<PhaserangeErrorDD<3, 1, 1>>(
                        measurement_rov, measurement_ref, index_pair.rov, index_pair.ref, index_pair.rov_base,
                        index_pair.ref_base, gnss_base_options_.error_parameter);
                graph_->addResidualBlock(phaserange_error, huber_loss_function_ ? huber_loss_function_.get() : nullptr,
                                         graph_->parameterBlockPtr(parameter_id.asInteger()),
