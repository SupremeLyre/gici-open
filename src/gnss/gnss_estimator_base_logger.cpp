...
void GnssEstimatorBase::createAmbiguityLogger(const std::string &directory)
{
    ...
    sprintf(buf, "%04d%02d%02d-%02d%02d%02d", (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3], (int)ep[4], (int)ep[5]);
...
void GnssEstimatorBase::logAmbiguityEstimate()
{
    ...
    for (auto id : ambiguity_state.ids)
    {
        std::shared_ptr<AmbiguityParameterBlock> block_ptr =
            std::static_pointer_cast<AmbiguityParameterBlock>(graph_->parameterBlockPtr(id.asInteger()));
...
#undef MAP
    ...
    char time_buf[64];
    sprintf(time_buf, "> %04d %02d %02d %02d %02d %10.7lf  %17.7lf", (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3],
            (int)ep[4], ep[5], ambiguity_state.timestamp);
    ...
    for (auto i : ambiguity_estimates)
    {
        ...
        for (auto j : i.second)
        {
            ...
            ambiguity_logger_ << phase_str << " " << std::fixed << std::setprecision(4) << value << " ";
        }
...
void GnssEstimatorBase::logIonosphereEstimate()
{
    ...
    for (auto id : ionosphere_state.ids)
    {
        std::shared_ptr<IonosphereParameterBlock> block_ptr =
            std::static_pointer_cast<IonosphereParameterBlock>(graph_->parameterBlockPtr(id.asInteger()));
        CHECK(block_ptr != nullptr);
        double value = block_ptr->estimate().x();
        std::string prn = id.gPrn();
    ...
    char time_buf[64];
    sprintf(time_buf, "> %04d %02d %02d %02d %02d %10.7lf  %17.7lf", (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3],
            (int)ep[4], ep[5], ionosphere_state.timestamp);
    ionosphere_logger_ << time_buf << std::endl;
    for (auto i : ionosphere_estimates)
    {
        const std::string &prn = i.first;
        const double &value = i.second;
        ionosphere_logger_ << prn << " " << std::fixed << std::setprecision(4) << value << std::endl;
    }
}
...
void GnssEstimatorBase::logPseudorangeResidual()
{
    ...
    for (const auto &residual : residuals)
    {
        ...
        if (interface->typeInfo() != ErrorType::kPseudorangeError)
            continue;

        GnssMeasurementIndex index = getGnssMeasurementIndexFromErrorInterface(interface);
        ...
        graph_->problem_->EvaluateResidualBlock(id, false, nullptr, residual_evaluate, nullptr);
    ...
    for (auto i : residual_values)
    {
        ...
        for (auto j : i.second)
        {
            ...
            const double &value = j.second;
            pseudorange_residual_logger_ << code_str << " " << std::fixed << std::setprecision(4) << value << " ";
        }
...
void GnssEstimatorBase::logDdPseudorangeResidual()
{
    ...
    State &state = lastState();
    const auto &residuals = graph_->residuals(state.id.asInteger());
    std::map<std::pair<std::string, std::string>, std::map<std::string, double>> residual_values;
    for (const auto &residual : residuals)
    {
        const ceres::ResidualBlockId id = residual.residual_block_id;
        const std::shared_ptr<ErrorInterface> &interface = residual.error_interface_ptr;
        if (interface->typeInfo() != ErrorType::kPseudorangeErrorDD)
            continue;

        GnssMeasurementDDIndexPair index = getGnssMeasurementDDIndexPairFromErrorInterface(interface);
        std::string prn = index.rov.prn;
        std::string code_str = gnss_common::codeTypeToRinexType(prn[0], index.rov.code_type);
        std::pair<std::string, std::string> prn_pair = std::make_pair(index.rov.prn, index.rov_base.prn);
        double residual_evaluate[1];
        graph_->problem_->EvaluateResidualBlock(id, false, nullptr, residual_evaluate, nullptr);
        interface->deNormalizeResidual(residual_evaluate);
    ...
    for (auto i : residual_values)
    {
        ...
        for (auto j : i.second)
        {
            const std::string &code_str = j.first;
            const double &value = j.second;
            dd_pseudorange_residual_logger_ << code_str << " " << std::fixed << std::setprecision(4) << value << " ";
        }
...
void GnssEstimatorBase::logPhaserangeResidual()
{
    ...
    for (const auto &residual : residuals)
    {
        const ceres::ResidualBlockId id = residual.residual_block_id;
        const std::shared_ptr<ErrorInterface> &interface = residual.error_interface_ptr;
        if (interface->typeInfo() != ErrorType::kPhaserangeError)
            continue;
...
#undef MAP
    ...
    for (auto i : residual_values)
    {
        ...
        for (auto j : i.second)
        {
            const std::string &phase_str = j.first;
            const double &value = j.second;
            phaserange_residual_logger_ << phase_str << " " << std::fixed << std::setprecision(4) << value << " ";
        }
...
void GnssEstimatorBase::logDdPhaserangeResidual()
{
    ...
    for (const auto &residual : residuals)
    {
        const ceres::ResidualBlockId id = residual.residual_block_id;
        const std::shared_ptr<ErrorInterface> &interface = residual.error_interface_ptr;
        if (interface->typeInfo() != ErrorType::kPhaserangeErrorDD)
            continue;
...
#undef MAP
    ...
    for (auto i : residual_values)
    {
        ...
        for (auto j : i.second)
        {
            const std::string &phase_str = j.first;
            const double &value = j.second;
            dd_phaserange_residual_logger_ << phase_str << " " << std::fixed << std::setprecision(4) << value << " ";
        }
