...
GnssSolution GnssLooseEstimatorBase::convertSolutionToGnssSolution(const Solution &solution, const SolutionRole &role)
{
    ...
    gnss_solution.position = solution.coordinate->convert(solution.pose.getPosition(), GeoType::ENU, GeoType::ECEF);
    Eigen::Matrix3d R_ecef_enu = solution.coordinate->rotationMatrix(GeoType::ENU, GeoType::ECEF);
    ...
    covariance_position_velocity.topLeftCorner(3, 3) = solution.covariance.block<3, 3>(0, 0);
    covariance_position_velocity.bottomRightCorner(3, 3) = solution.covariance.block<3, 3>(6, 6);
    covariance_position_velocity.topRightCorner(3, 3) = solution.covariance.block<3, 3>(0, 6);
    covariance_position_velocity.bottomLeftCorner(3, 3) = solution.covariance.block<3, 3>(6, 0);
    gnss_solution.covariance = R_ecef_enu_double * covariance_position_velocity * R_ecef_enu_double.transpose();
...
bool GnssLooseEstimatorBase::computeCoarseVelocityFromPosition(const GnssSolution &last_gnss_solution,
...
BackendId GnssLooseEstimatorBase::addGnssExtrinsicsParameterBlock(const int32_t id, const Eigen::Vector3d &t_SR_S_prior)
{
...
bool GnssLooseEstimatorBase::rejectGnssPositionAndVelocityOutliers(const State &state)
{
    ...
    for (auto residual_block : residual_blocks)
    {
        ...
        if (error_interface->typeInfo() != ErrorType::kPositionError &&
            error_interface->typeInfo() != ErrorType::kVelocityError)
            continue;
        Eigen::VectorXd Residuals = Eigen::VectorXd::Zero(3);
        graph_->problem()->EvaluateResidualBlock(residual_block.residual_block_id, false, nullptr, Residuals.data(),
                                                 nullptr);
        ...
        if (Residuals.norm() > thr)
        {
            ...
            {
                LOG(INFO) << "Rejected GNSS " << kErrorToStr.at(error_interface->typeInfo()) << " with residual "
                          << Residuals.norm();
            }
...
void GnssLooseEstimatorBase::addGnssPositionResidualBlock(const GnssSolution &measurement, const State &state)
{
    ...
    std::shared_ptr<PositionError<7, 3>> position_error = std::make_shared<PositionError<7, 3>>(
        measurement.position, measurement.covariance.topLeftCorner(3, 3).inverse());
...
void GnssLooseEstimatorBase::addGnssVelocityResidualBlock(const GnssSolution &measurement, const State &state,
                                                          const Eigen::Vector3d &angular_velocity)
{
    ...
    std::shared_ptr<VelocityError<7, 9, 3>> velocity_error = std::make_shared<VelocityError<7, 9, 3>>(
        measurement.velocity, measurement.covariance.bottomRightCorner(3, 3).inverse(), angular_velocity);
    velocity_error->setCoordinate(coordinate_);
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(velocity_error, nullptr, graph_->parameterBlockPtr(pose_id.asInteger()),
                                 graph_->parameterBlockPtr(speed_and_bias_id.asInteger()),
                                 graph_->parameterBlockPtr(gnss_extrinsics_id_.asInteger()));
}

// Add initial GNSS extrinsics error
void GnssLooseEstimatorBase::addGnssExtrinsicsResidualBlock(const BackendId &gnss_extrinsics_id,
                                                            ...
                                                            const Eigen::Vector3d &std)
{
    ...
    for (size_t i = 0; i < 3; i++)
        info(i) = square(1.0 / std(i));
    std::shared_ptr<PositionError<3>> extrinsic_error =
        std::make_shared<PositionError<3>>(t_SR_S_prior, info.asDiagonal());
    graph_->addResidualBlock(extrinsic_error, nullptr, graph_->parameterBlockPtr(gnss_extrinsics_id.asInteger()));
}
...
void GnssLooseEstimatorBase::addGnssVelocityResidualMarginBlock(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kVelocityError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
