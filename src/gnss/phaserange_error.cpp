...
template <int... Ns>
PhaserangeError<Ns...>::PhaserangeError(const GnssMeasurement &measurement, const GnssMeasurementIndex index,
                                        const GnssErrorParameter &error_parameter)
{
    ...
    // Group 1
    if (dims_.kNumParameterBlocks == 5 && dims_.GetDim(0) == 3 && dims_.GetDim(1) == 1 && dims_.GetDim(2) == 1 &&
...
// Set the information.
template <int... Ns> void PhaserangeError<Ns...>::setInformation(const GnssErrorParameter &error_parameter)
{
    // compute variance
...
template <int... Ns>
bool PhaserangeError<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool PhaserangeError<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                          double **jacobians, double **jacobians_minimal) const
{
    ...
    // phase wind-up
    phase_windup = measurement_.phase_windup->get(timestamp, satellite_.prn, satellite_.sat_position, t_WR_ECEF);
    phase_windup *= observation_.wavelength;

    // Get estimate derivated measurement
    double phaserange_estimate =
        rho + clock - satellite_.sat_clock + troposphere_delay - ionosphere_delay + ambiguity + phase_windup;

    // Compute error
    double phaserange = observation_.phaserange;
    Eigen::Matrix<double, 1, 1> error = Eigen::Matrix<double, 1, 1>(phaserange - phaserange_estimate);
