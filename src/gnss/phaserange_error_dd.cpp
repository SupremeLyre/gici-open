...
template <int... Ns>
PhaserangeErrorDD<Ns...>::PhaserangeErrorDD(const GnssMeasurement &measurement_rov,
                                            ...
                                            const GnssErrorParameter &error_parameter)
{
    ...
    // Group 3
    else if (dims_.kNumParameterBlocks == 7 && dims_.GetDim(0) == 3 && dims_.GetDim(1) == 1 && dims_.GetDim(2) == 1 &&
    ...
    // Group 4
    else if (dims_.kNumParameterBlocks == 8 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 3 && dims_.GetDim(2) == 1 &&
             dims_.GetDim(3) == 1 && dims_.GetDim(4) == 1 && dims_.GetDim(5) == 1 && dims_.GetDim(6) == 1 &&
             dims_.GetDim(7) == 1)
    {
        is_estimate_body_ = true;
...
// Set the information.
template <int... Ns> void PhaserangeErrorDD<Ns...>::setInformation(const GnssErrorParameter &error_parameter)
{
    // compute variance
    ...
    covariance_ = covariance_t((square(factor(0)) + square(factor(1) / sin(elevation_rov))) * 4.0);
...
template <int... Ns>
bool PhaserangeErrorDD<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool PhaserangeErrorDD<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                            double **jacobians, double **jacobians_minimal) const
{
    ...
    double rho_rov_base = gnss_common::satelliteToReceiverDistance(satellite_rov_base_.sat_position, t_WR_ECEF);
    double rho_ref_base =
        gnss_common::satelliteToReceiverDistance(satellite_ref_base_.sat_position, measurement_ref_.position);
    ...
    double elevation_rov_base = gnss_common::satelliteElevation(satellite_rov_base_.sat_position, t_WR_ECEF);
    double elevation_ref_base =
        gnss_common::satelliteElevation(satellite_ref_base_.sat_position, measurement_ref_.position);
    ...
    else
    {
        ...
        double zhd_rov_base = gnss_common::troposphereSaastamoinen(timestamp, t_WR_ECEF, PI / 2.0);
        double zhd_ref_base = gnss_common::troposphereSaastamoinen(timestamp, t_WR_ECEF, PI / 2.0);
        // mapping
        ...
        gnss_common::troposphereGMF(timestamp, t_WR_ECEF, elevation_rov_base, &gmf_hydro_rov_base, &gmf_wet_rov_base);
        gnss_common::troposphereGMF(timestamp, t_WR_ECEF, elevation_ref_base, &gmf_hydro_ref_base, &gmf_wet_ref_base);
        dtroposphere_delay = zhd_rov * gmf_hydro_rov - zhd_ref * gmf_hydro_ref - zhd_rov_base * gmf_hydro_rov_base +
                             zhd_ref_base * gmf_hydro_ref_base + zwd_rov * gmf_wet_rov - zwd_ref * gmf_wet_ref -
                             zwd_rov * gmf_wet_rov_base + zwd_ref * gmf_wet_ref_base;

        // ionosphere
        dionosphere_delay_cur =
            gnss_common::ionosphereConvertFromBase(dionosphere_delay_cur, observation_rov_.wavelength);
        dionosphere_delay_base =
            gnss_common::ionosphereConvertFromBase(dionosphere_delay_base, observation_rov_base_.wavelength);
        dionosphere_delay = dionosphere_delay_cur - dionosphere_delay_base;
    }

    // Get estimate derivated measurement
    double dphaserange_estimate = rho_rov - rho_ref - rho_rov_base + rho_ref_base + dtroposphere_delay -
                                  dionosphere_delay + dambiguity - dambiguity_base;

    // Compute error
    double dphaserange = observation_rov_.phaserange - observation_ref_.phaserange - observation_rov_base_.phaserange +
                         observation_ref_base_.phaserange;
    ...
    if (jacobians != nullptr)
    {
        ...
        // Troposphere
        Eigen::Matrix<double, 1, 1> J_trop_rov = -(gmf_wet_rov - gmf_wet_rov_base) * Eigen::MatrixXd::Identity(1, 1);
        Eigen::Matrix<double, 1, 1> J_trop_ref = (gmf_wet_ref - gmf_wet_ref_base) * Eigen::MatrixXd::Identity(1, 1);
        ...
        Eigen::Matrix<double, 1, 1> J_iono_base =
        ...
        if (parameter_block_group_ == 2 || parameter_block_group_ == 4)
        {
            ...
            if (is_estimate_atmosphere_ && jacobians[7] != nullptr)
            {
                ...
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> J7_minimal_mapped(jacobians_minimal[7]);
