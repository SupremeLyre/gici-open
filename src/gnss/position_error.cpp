...
template <int... Ns>
PositionError<Ns...>::PositionError(const Eigen::Vector3d &position_measurement, const Eigen::Matrix3d &information)
{
...
template <int... Ns>
bool PositionError<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool PositionError<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                        double **jacobians, double **jacobians_minimal) const
{
    ...
    if (jacobians != nullptr)
    {
        ...
        if (is_estimate_body_)
        {
            ...
            Eigen::Matrix<double, 3, 3> J_t_W = J_t_ECEF * coordinate_->rotationMatrix(GeoType::ENU, GeoType::ECEF);

            // Body rotation in ENU
            Eigen::Matrix<double, 3, 3> J_q_WS = J_t_W * -skewSymmetric(q_WS.toRotationMatrix() * t_SR_S);
        ...
        if (parameter_block_group_ == 2)
        {
            ...
            if (jacobians[1] != nullptr)
            {
                ...
                {
                    Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> J1_minimal_mapped(jacobians_minimal[1]);
