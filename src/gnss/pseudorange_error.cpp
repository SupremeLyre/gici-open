...
template <int... Ns>
PseudorangeError<Ns...>::PseudorangeError(const GnssMeasurement &measurement, const GnssMeasurementIndex index,
                                          const GnssErrorParameter &error_parameter)
{
    ...
    // Group 1
    if (dims_.kNumParameterBlocks == 2 && dims_.GetDim(0) == 3 && dims_.GetDim(1) == 1)
    {
    ...
    // Group 2
    else if (dims_.kNumParameterBlocks == 3 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 3 && dims_.GetDim(2) == 1)
    {
    ...
    // Group 3
    else if (dims_.kNumParameterBlocks == 5 && dims_.GetDim(0) == 3 && dims_.GetDim(1) == 1 && dims_.GetDim(2) == 1 &&
    ...
    // Group 4
    else if (dims_.kNumParameterBlocks == 6 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 3 && dims_.GetDim(2) == 1 &&
...
// Set the information.
template <int... Ns> void PseudorangeError<Ns...>::setInformation(const GnssErrorParameter &error_parameter)
{
    // compute variance
    ...
    double elevation = gnss_common::satelliteElevation(satellite_.sat_position, measurement_.position);
    double azimuth = gnss_common::satelliteAzimuth(satellite_.sat_position, measurement_.position);
    ...
    if (!is_estimate_atmosphere_)
    {
        ...
        // ionosphere error
        if (satellite_.ionosphere != 0.0 && satellite_.ionosphere_type == IonoType::Augmentation)
        {
            ionosphere_var = square(error_parameter_.ionosphere_augment);
        }
        else if (satellite_.ionosphere != 0.0 && satellite_.ionosphere_type == IonoType::DualFrequency)
        {
        ...
        else
        {
            double ionosphere_delay =
                gnss_common::ionosphereBroadcast(timestamp, measurement_.position, azimuth, elevation,
                                                 observation_.wavelength, measurement_.ionosphere_parameters);
            ionosphere_var = square(error_parameter_.ionosphere_broadcast_factor * ionosphere_delay);
        }
        // troposphere error
        double troposphere_delay = gnss_common::troposphereSaastamoinen(timestamp, measurement_.position, elevation);
        troposphere_var = square(error_parameter_.troposphere_model_factor * troposphere_delay);
    ...
    double covariance = (square(factor(0)) + square(factor(1) / sin(elevation))) * ratio + ephemeris_var +
    ...
    if (satellite_.getSystem() == 'G' &&
        checkEqual(observation_.wavelength, CLIGHT / gnss_common::phaseToFrequency('G', PHASE_L5)))
    {
...
template <int... Ns>
bool PseudorangeError<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool PseudorangeError<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                           double **jacobians, double **jacobians_minimal) const
{
    ...
    if (!is_estimate_atmosphere_)
    {
        ...
        troposphere_delay = gnss_common::troposphereSaastamoinen(timestamp, t_WR_ECEF, elevation);
        // troposphere wet delay
        if (measurement_.troposphere_wet != 0.0)
        {
        ...
        if (satellite_.ionosphere != 0.0 && satellite_.ionosphere_type == IonoType::Augmentation)
        {
            ...
            ionosphere_delay = gnss_common::ionosphereConvertFromBase(ionosphere_delay, observation_.wavelength);
        }
        else if (satellite_.ionosphere != 0.0 && satellite_.ionosphere_type == IonoType::DualFrequency)
        {
            ionosphere_delay = satellite_.ionosphere;
        ...
        else
        {
            ionosphere_delay = gnss_common::ionosphereBroadcast(
                timestamp, t_WR_ECEF, azimuth, elevation, observation_.wavelength, measurement_.ionosphere_parameters);
        }
    }
    ...
    double pseudorange_estimate = rho + clock - satellite_.sat_clock + ifb + troposphere_delay + ionosphere_delay;

    // Compute error
    double pseudorange = observation_.pseudorange;
    Eigen::Matrix<double, 1, 1> error = Eigen::Matrix<double, 1, 1>(pseudorange - pseudorange_estimate);
    ...
    if (jacobians != nullptr)
    {
        ...
        Eigen::Matrix<double, 1, 3> J_t_ECEF = -((t_WR_ECEF - satellite_.sat_position) / rho).transpose();
        ...
        Eigen::Matrix<double, 1, 1> J_iono =
            -Eigen::MatrixXd::Identity(1, 1) * gnss_common::ionosphereConvertFromBase(1.0, observation_.wavelength);
