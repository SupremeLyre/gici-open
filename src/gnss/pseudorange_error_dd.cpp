...
template <int... Ns>
PseudorangeErrorDD<Ns...>::PseudorangeErrorDD(const GnssMeasurement &measurement_rov,
                                              ...
                                              const GnssErrorParameter &error_parameter)
{
    ...
    // Group 1
    if (dims_.kNumParameterBlocks == 1 && dims_.GetDim(0) == 3)
    {
    ...
    // Group 2
    else if (dims_.kNumParameterBlocks == 2 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 3)
    {
    ...
    // Group 3
    else if (dims_.kNumParameterBlocks == 4 && dims_.GetDim(0) == 3 && dims_.GetDim(1) == 1 && dims_.GetDim(2) == 1 &&
             dims_.GetDim(3) == 1)
    {
        is_estimate_body_ = false;
    ...
    // Group 4
    else if (dims_.kNumParameterBlocks == 5 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 3 && dims_.GetDim(2) == 1 &&
             dims_.GetDim(3) == 1 && dims_.GetDim(4) == 1)
    {
        is_estimate_body_ = true;
        is_estimate_atmosphere_ = true;
...
// Set the information.
template <int... Ns> void PseudorangeErrorDD<Ns...>::setInformation(const GnssErrorParameter &error_parameter)
{
    // compute variance
    ...
    covariance_ = covariance_t((square(factor(0)) + square(factor(1) / sin(elevation_rov))) * ratio * 4.0);
...
template <int... Ns>
bool PseudorangeErrorDD<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool PseudorangeErrorDD<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                             double **jacobians, double **jacobians_minimal) const
{
    ...
    double dpseudorange_estimate =
        rho_rov - rho_ref - rho_rov_base + rho_ref_base + dtroposphere_delay + dionosphere_delay;
    ...
    Eigen::Matrix<double, 1, 1> error = Eigen::Matrix<double, 1, 1>(dpseudorange - dpseudorange_estimate);
