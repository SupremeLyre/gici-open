...
bool RelativeIsbError::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
bool RelativeIsbError::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                    double **jacobians, double **jacobians_minimal) const
{
    ...
    // Compute error
    Eigen::Matrix<double, 1, 1> error = Eigen::Matrix<double, 1, 1>(0 - isb_estimate);
    ...
    if (jacobians != nullptr)
    {
        for (size_t i = 0; i < 4; i++)
        {
            ...
            if (jacobians[i] != nullptr)
            {
                ...
                Ji = -square_root_information_ * Eigen::MatrixXd::Identity(1, 1) * coefficient;
