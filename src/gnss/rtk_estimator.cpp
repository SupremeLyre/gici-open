...
RtkEstimator::RtkEstimator(const RtkEstimatorOptions &options, const GnssEstimatorBaseOptions &gnss_base_options,
                           const EstimatorBaseOptions &base_options,
                           const AmbiguityResolutionOptions &ambiguity_options)
    : rtk_options_(options), ambiguity_options_(ambiguity_options), GnssEstimatorBase(gnss_base_options, base_options),
      EstimatorBase(base_options)
{
    type_ = EstimatorType::Rtk;
    is_use_phase_ = true;
    if (options.estimate_velocity)
...
bool RtkEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement_rov,
                                              const GnssMeasurement &measurement_ref)
{
    ...
    if (isFirstEpoch())
    {
        ...
        if (rtk_options_.estimate_velocity)
        {
            addGnssVelocityResidualBlock(curState(), velocity_prior,
                                         gnss_base_options_.error_parameter.initial_velocity);
        }
    }
...
bool RtkEstimator::estimate()
{
    ...
    {
        int distance = static_cast<int>(round((curGnssRov().position - curGnssRef().position).norm() / 1.0e3));
