...
SdgnssEstimator::SdgnssEstimator(const SdgnssEstimatorOptions &options,
    ...
    : sdgnss_options_(options), GnssEstimatorBase(gnss_base_options, base_options), EstimatorBase(base_options)
{
...
bool SdgnssEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement_rov,
                                                 const GnssMeasurement &measurement_ref)
{
    ...
    GnssMeasurementSDIndexPairs index_pairs =
        gnss_common::formPseudorangeSDPair(curGnssRov(), curGnssRef(), gnss_base_options_.common);
    ...
    addSdClockParameterBlocks(curGnssRov(), curGnssRef(), index_pairs, curGnssRov().id, num_valid_system);
    if (sdgnss_options_.estimate_velocity)
    {
        ...
        addFrequencyParameterBlocks(curGnssRov(), curGnssRov().id, num_valid_doppler_system, frequency_prior);
    }

    // Add pseudorange residual blocks
    int num_valid_satellite = 0;
    addSdPseudorangeResidualBlocks(curGnssRov(), curGnssRef(), index_pairs, curState(), num_valid_satellite);
