...
SppEstimator::SppEstimator(const SppEstimatorOptions &options, const GnssEstimatorBaseOptions &gnss_base_options,
                           const EstimatorBaseOptions &base_options)
    : spp_options_(options), GnssEstimatorBase(gnss_base_options, base_options), EstimatorBase(base_options)
{
...
SppEstimator::SppEstimator(const SppEstimatorOptions &options, const GnssEstimatorBaseOptions &gnss_base_options)
    : spp_options_(options), GnssEstimatorBase(gnss_base_options, EstimatorBaseOptions()),
...
SppEstimator::SppEstimator(const GnssEstimatorBaseOptions &gnss_base_options)
    : spp_options_(SppEstimatorOptions()), GnssEstimatorBase(gnss_base_options, EstimatorBaseOptions()),
      EstimatorBase(EstimatorBaseOptions())
{
    base_options_.max_iteration = 10;
    base_options_.max_solver_time = 0.05;
...
bool SppEstimator::addGnssMeasurementAndState(const GnssMeasurement &measurement)
{
    // Get prior position
    ...
    else
    {
        ...
        Eigen::Vector3d avg_satellite_position = sum_satellite_position / static_cast<double>(num_sum_satellites);
    ...
    if (spp_options_.use_dual_frequency)
        arrangeDualFrequency(curGnss());
    ...
    {
        if (BackendId(parameter.first).bundleId() == curState().id.bundleId())
            continue;
...
bool SppEstimator::estimate()
{
    ...
    if (gnss_base_options_.use_outlier_rejection)
        while (1)
        {
            optimize();
            // reject outlier
            if (!rejectPseudorangeOutlier(curState(), gnss_base_options_.reject_one_outlier_once))
                break;
        }
    ...
    if (!checkZero(curGnss().position))
    {
        ...
        {
            LOG(INFO) << "High GDOP! Our tolerant is " << gnss_base_options_.common.max_gdop
                      << " in maximum, but current GDOP is " << gdop_ << "!";
            // erase parameters
            Graph::ParameterBlockCollection parameters = graph_->parameters();
            for (auto parameter : parameters)
                graph_->removeParameterBlock(parameter.first);
    ...
    {
        LOG(INFO) << "High chi-square! Chi-square is " << chi_square << ". Chi-square threshold is "
                  << chisqr[n_residual - n_parameter - 1] << ".";
    }
    ...
    if (spp_options_.estimate_velocity && dop_valid)
    {
        ...
        if (!checkSufficientSatellite(num_valid_doppler_satellite, num_valid_doppler_system, false))
        {
    ...
    if (!dop_valid)
        status_ = EstimatorStatus::InvalidEpoch;
    return dop_valid;
}
...
void SppEstimator::arrangeDualFrequency(GnssMeasurement &measurement)
{
    ...
    for (auto it_sat = measurement.satellites.begin(); it_sat != measurement.satellites.end();)
    {
        ...
        int valid_cnt = 0;
        for (auto it_obs = it_sat->second.observations.begin(); it_obs != it_sat->second.observations.end();)
        {
            int phase_id = gnss_common::getPhaseID(system, it_obs->first);
            if ((phase_id != phase_id_base_lhs && phase_id != phase_id_base_rhs) ||
                !checkObservationValid(measurement, GnssMeasurementIndex(it_sat->first, it_obs->first)))
            {
...
Eigen::Vector3d SppEstimator::getPositionEstimate(const State &state)
{
    std::shared_ptr<PositionParameterBlock> block_ptr =
        std::static_pointer_cast<PositionParameterBlock>(graph_->parameterBlockPtr(state.id.asInteger()));
...
Eigen::Vector3d SppEstimator::getVelocityEstimate(const State &state)
{
    ...
    std::shared_ptr<VelocityParameterBlock> block_ptr =
        std::static_pointer_cast<VelocityParameterBlock>(graph_->parameterBlockPtr(velocity_id.asInteger()));
    CHECK(block_ptr != nullptr);
    return block_ptr->estimate();
}
