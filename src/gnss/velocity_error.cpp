...
template <int... Ns>
VelocityError<Ns...>::VelocityError(const Eigen::Vector3d &velocity_measurement, const Eigen::Matrix3d &information)
{
    ...
    // Group 2
    else if (dims_.kNumParameterBlocks == 3 && dims_.GetDim(0) == 7 && dims_.GetDim(1) == 9 && dims_.GetDim(2) == 3)
    {
...
template <int... Ns>
bool VelocityError<Ns...>::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
template <int... Ns>
bool VelocityError<Ns...>::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals,
                                                        double **jacobians, double **jacobians_minimal) const
{
    ...
    if (jacobians != nullptr)
    {
        ...
        if (is_estimate_body_)
        {
            ...
            Eigen::Matrix<double, 3, 3> J_q_WS =
        ...
        if (parameter_block_group_ == 1)
        {
            ...
            if (jacobians[0] != nullptr)
            {
                ...
                {
                    Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> J0_minimal_mapped(jacobians_minimal[0]);
        ...
        if (parameter_block_group_ == 2)
        {
            ...
            if (jacobians[0] != nullptr)
            {
                ...
                if (jacobians_minimal != nullptr && jacobians_minimal[0] != nullptr)
                {
                    Eigen::Map<Eigen::Matrix<double, 3, 6, Eigen::RowMajor>> J0_minimal_mapped(jacobians_minimal[0]);
            ...
            if (jacobians[1] != nullptr)
            {
                ...
                {
                    Eigen::Map<Eigen::Matrix<double, 3, 9, Eigen::RowMajor>> J1_minimal_mapped(jacobians_minimal[1]);
            ...
            if (jacobians[2] != nullptr)
            {
                ...
                {
                    Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> J2_minimal_mapped(jacobians_minimal[2]);
