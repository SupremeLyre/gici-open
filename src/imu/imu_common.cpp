...
bool initPoseAndBiases(const ImuMeasurements &imu_measurements, const double gravity, Transformation &T_WS,
                       SpeedAndBias &speed_and_bias, const double timestamp_start, const double timestamp_end)
{
    // set to zero
    T_WS.setIdentity();
    ...
    {
        if (imu_measurements[i].timestamp < timestamp_start || imu_measurements[i].timestamp > timestamp_end)
            continue;
        n_measurements++;
