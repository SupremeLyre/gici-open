...
ImuEstimatorBase::ImuEstimatorBase(const ImuEstimatorBaseOptions &options, const EstimatorBaseOptions &base_options)
    : imu_base_options_(options), EstimatorBase(base_options)
{
    Eigen::Quaterniond q_BI = eulerAngleToQuaternion(imu_base_options_.body_to_imu_rotation * D2R);
...
{
    if (imu_measurements_.size() != 0 && imu_measurements_.back().timestamp > imu_measurement.timestamp)
    {
    ...
    else
    {
        ...
        imu_measurements_.back().angular_velocity = rotateImuToBody(imu_measurements_.back().angular_velocity);
        imu_measurements_.back().linear_acceleration = rotateImuToBody(imu_measurements_.back().linear_acceleration);
        imu_mutex_.unlock();
    }
...
// IMU integration
bool ImuEstimatorBase::imuIntegration(const double last_timestamp, const double timestamp, Transformation &T_WS,
                                      SpeedAndBias &speed_and_bias)
{
    // Do not need to integrate
    if (checkEqual(timestamp, last_timestamp))
        return true;

    // Integrate to desired timestamp
    imu_mutex_.lock();
    ImuError::propagation(imu_measurements_, imu_base_options_.imu_parameters, T_WS, speed_and_bias, last_timestamp,
...
bool ImuEstimatorBase::imuIntegration(const double last_timestamp, const double timestamp, Transformation &T_WS,
                                      SpeedAndBias &speed_and_bias, Eigen::Matrix<double, 15, 15> &covariance)
{
    // Do not need to integrate
    if (checkEqual(timestamp, last_timestamp))
        return true;
...
bool ImuEstimatorBase::getMotionFromEstimateAndImuIntegration(const State &estimated_state, const double timestamp,
                                                              Transformation &T_WS, SpeedAndBias &speed_and_bias)
{
    ...
    // Do not need to integrate
    if (checkEqual(timestamp, estimated_state.timestamp))
        return true;
                                                              ...
                                                              Eigen::Matrix<double, 15, 15> &covariance)
{
    ...
    // Integrate to desired timestamp
    return imuIntegration(estimated_state.timestamp, timestamp, T_WS, speed_and_bias, covariance);
}

// Get pose estimate at given timestmap
bool ImuEstimatorBase::getPoseEstimateAt(const double timestamp, Transformation &T_WS)
{
    ...
    if (!checkEqual(last_base_state_.timestamp, base_state.timestamp) || !checkLessEqual(last_timestamp_, timestamp))
    {
        if (need_covariance_)
        {
            ret = getMotionFromEstimateAndImuIntegration(base_state, timestamp, T_WS, speed_and_bias, covariance);
        }
        else
        {
            ret = getMotionFromEstimateAndImuIntegration(base_state, timestamp, T_WS, speed_and_bias);
        }
    }
    ...
    else
    {
        ...
        if (need_covariance_)
        {
            ret = imuIntegration(last_timestamp_, timestamp, T_WS, speed_and_bias, covariance);
        }
        else
        {
            ret = imuIntegration(last_timestamp_, timestamp, T_WS, speed_and_bias);
        }
    }
...
bool ImuEstimatorBase::getSpeedAndBiasEstimateAt(const double timestamp, SpeedAndBias &speed_and_bias)
{
    // Check if we have already applied integration
    if (checkEqual(timestamp, last_timestamp_))
    {
    ...
    // Apply integration
    Transformation T_WS;
    if (!getPoseEstimateAt(timestamp, T_WS))
        return false;
    speed_and_bias = last_speed_and_bias_;
...
bool ImuEstimatorBase::getCovarianceAt(const double timestamp, Eigen::Matrix<double, 15, 15> &covariance)
{
    // Check if we have already applied integration
...
void ImuEstimatorBase::addImuSpeedAndBiasParameterBlock(const BackendId backend_id, const SpeedAndBias &prior)
{
    ...
    std::shared_ptr<SpeedAndBiasParameterBlock> speed_and_bias_parameter_block =
        std::make_shared<SpeedAndBiasParameterBlock>(prior, speed_and_bias_id.asInteger());
    CHECK(graph_->addParameterBlock(speed_and_bias_parameter_block));
}

// Add pose block to graph
void ImuEstimatorBase::addPoseParameterBlock(const BackendId backend_id, const Transformation &T_WS_prior)
{
    if (graph_->parameterBlockExists(backend_id.asInteger()))
        return;
...
void ImuEstimatorBase::addImuResidualBlock(const State &last_state, State &cur_state)
{
    ...
    std::shared_ptr<ImuError> imu_error =
        std::make_shared<ImuError>(imu_measurements_, imu_base_options_.imu_parameters, last_timestamp, timestamp);
    imu_mutex_.unlock();
    cur_state.imu_residual_to_lhs = graph_->addResidualBlock(
        imu_error, nullptr, graph_->parameterBlockPtr(last_pose_id.asInteger()),
        graph_->parameterBlockPtr(last_speed_and_bias_id.asInteger()),
        graph_->parameterBlockPtr(cur_pose_id.asInteger()), graph_->parameterBlockPtr(speed_and_bias_id.asInteger()));
...
void ImuEstimatorBase::addImuSpeedAndBiasResidualBlock(const State &state, const SpeedAndBias &speed_and_bias,
                                                       const double std_speed, const double std_bg, const double std_ba)
{
    ...
    std::shared_ptr<SpeedAndBiasError> speed_and_bias_error =
        std::make_shared<SpeedAndBiasError>(speed_and_bias, square(std_speed), square(std_bg), square(std_ba));
    ceres::ResidualBlockId residual_id = graph_->addResidualBlock(
        speed_and_bias_error, nullptr, graph_->parameterBlockPtr(speed_and_bias_id.asInteger()));
}

// Add pose residual block to graph
void ImuEstimatorBase::addPoseResidualBlock(const State &state, const Transformation &T_WS, const double std_roll_pitch,
                                            const double std_yaw)
{
    ...
    std::shared_ptr<PoseError> pose_error = std::make_shared<PoseError>(T_WS, information);
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(pose_error, nullptr, graph_->parameterBlockPtr(pose_id.asInteger()));
}

// Add yaw residual block to graph
void ImuEstimatorBase::addYawResidualBlock(const State &state, const double yaw, const double std_yaw)
{
    BackendId pose_id = state.id_in_graph;
#if 0
  std::shared_ptr<YawError> yaw_error = 
    ...
    information *= 1.0e-6;
    information(5, 5) = 1.0 / square(std_yaw);
    std::shared_ptr<PoseError> yaw_error = std::make_shared<PoseError>(T_WS, information);
#endif
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(yaw_error, nullptr, graph_->parameterBlockPtr(pose_id.asInteger()));
}

// Add roll and pitch residual block to graph
void ImuEstimatorBase::addRollPitchResidualBlock(const State &state, const Eigen::Vector2d &pitch_and_roll,
                                                 const double std_pitch_and_roll)
{
  ...
  std::shared_ptr<RollAndPitchError> roll_pitch_error = 
    ...
    std::shared_ptr<PoseError> roll_pitch_error = std::make_shared<PoseError>(T_WS, information);
#endif
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(roll_pitch_error, nullptr, graph_->parameterBlockPtr(pose_id.asInteger()));
}
...
void ImuEstimatorBase::addHMCResidualBlock(const State &state)
{
    ...
    if (speed.norm() < imu_base_options_.car_motion_min_velocity)
        return;
    double angular_velocity_norm = getImuMeasurementNear(state.timestamp).angular_velocity.norm();
    if (angular_velocity_norm * R2D > imu_base_options_.car_motion_max_anguler_velocity)
        return;

    double std = sqrt(square(imu_base_options_.body_to_imu_rotation_std * D2R) + square(0.1 / speed.head<2>().norm()));
    std::shared_ptr<HMCError> hmc_error = std::make_shared<HMCError>(std);
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(hmc_error, nullptr, graph_->parameterBlockPtr(pose_id.asInteger()),
...
void ImuEstimatorBase::addNHCResidualBlock(const State &state)
{
    ...
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(nhc_error, nullptr, graph_->parameterBlockPtr(pose_id.asInteger()),
                                 graph_->parameterBlockPtr(speed_and_bias_id.asInteger()));
}
...
{
    if (!imu_base_options_.zupt_duration)
        return;
    ...
    for (int i = imu_measurements_.size() - 1; i >= 0; i--)
    {
        ...
        if (imu.timestamp > state.timestamp)
            continue;
        if (imu.timestamp < state.timestamp - imu_base_options_.zupt_duration)
            break;
        if (i == 0 && imu.timestamp > state.timestamp - imu_base_options_.zupt_duration)
        {
            imu_mutex_.unlock();
            return;
        }
    ...
    for (int i = 0; i < 3; i++)
    {
        if (std_acc[i] > imu_base_options_.zupt_max_acc_std)
            return;
        if (std_gyro[i] > imu_base_options_.zupt_max_gyro_std)
            return;
        if (fabs(median_gyro[i]) > imu_base_options_.zupt_max_gyro_median)
            return;
    }
    ...
    const double speed_information = 1.0 / square(imu_base_options_.zupt_sigma_zero_velocity);
    ...
    BackendId speed_and_bias_id = changeIdType(state.id_in_graph, IdType::ImuStates);
    std::shared_ptr<SpeedAndBiasError> error = std::make_shared<SpeedAndBiasError>(speed_and_bias, information);
    graph_->addResidualBlock(error, huber_loss_function_ ? huber_loss_function_.get() : nullptr,
                             graph_->parameterBlockPtr(speed_and_bias_id.asInteger()));

    // set velocity parameter as zero
    auto speed_and_bias_parameter = graph_->parameterBlockPtr(speed_and_bias_id.asInteger());
...
size_t ImuEstimatorBase::insertImuState(const double timestamp, const BackendId &backend_id,
                                        const Transformation &T_WS_prior, const SpeedAndBias &speed_and_bias_prior,
                                        const bool use_prior)
{
    ...
    else if (latest_timestamp == 0.0 || checkLessEqual(timestamp, oldestState().timestamp))
    {
    ...
    else if (checkLessEqual(latest_timestamp, timestamp))
    {
        ...
        if (!use_prior)
        {
            getMotionFromEstimateAndImuIntegration(last_state, timestamp, T_WS, speed_and_bias);
        }
    ...
    else
    {
        ...
        if (!use_prior)
        {
            getMotionFromEstimateAndImuIntegration(states_[index_lhs], timestamp, T_WS, speed_and_bias);
        }
        addPoseParameterBlock(backend_id, T_WS);
        addImuSpeedAndBiasParameterBlock(backend_id, speed_and_bias);
        auto it_lhs = states_.begin();
        std::advance(it_lhs, index_lhs + 1);
        State state;
        state.timestamp = timestamp;
        state.id = backend_id;
        state.id_in_graph = backend_id;
        ...
        auto it_rhs_new = it_cur;
        it_rhs_new++;
        auto it_lhs_new = it_cur;
        it_lhs_new--;
...
void ImuEstimatorBase::addImuSpeedAndBiasParameterMarginBlockWithResiduals(const State &state, bool keep)
{
    BackendId speed_and_bias_id = changeIdType(state.id_in_graph, IdType::ImuStates);
    CHECK(graph_->parameterBlockExists(speed_and_bias_id.asInteger()));
    Graph::ResidualBlockCollection residuals = graph_->residuals(speed_and_bias_id.asInteger());
...
void ImuEstimatorBase::addPoseParameterMarginBlockWithResiduals(const State &state, bool keep)
{
    BackendId pose_id = state.id_in_graph;
    CHECK(graph_->parameterBlockExists(pose_id.asInteger()));
    Graph::ResidualBlockCollection residuals = graph_->residuals(pose_id.asInteger());
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
...
void ImuEstimatorBase::addImuSpeedAndBiasResidualMarginBlock(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kSpeedAndBiasError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
void ImuEstimatorBase::addPoseResidualMarginBlock(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kPoseError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
void ImuEstimatorBase::addHMCResidualMarginBlock(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kHMCError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
void ImuEstimatorBase::addNHCResidualMarginBlock(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kNHCError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
void ImuEstimatorBase::addImuResidualMarginBlocks(const State &state)
{
    ...
    {
        if (residuals[r].error_interface_ptr->typeInfo() != ErrorType::kSpeedAndBiasError &&
            residuals[r].error_interface_ptr->typeInfo() != ErrorType::kIMUError &&
            residuals[r].error_interface_ptr->typeInfo() != ErrorType::kPoseError &&
            residuals[r].error_interface_ptr->typeInfo() != ErrorType::kHMCError &&
            residuals[r].error_interface_ptr->typeInfo() != ErrorType::kNHCError)
            continue;
        marginalization_error_->addResidualBlock(residuals[r].residual_block_id);
    }
}
...
{
    if (cur_state.imu_residual_to_lhs == nullptr)
        return;
...
void ImuEstimatorBase::eraseImuState(const State &state)
{
    ...
    const BackendId speed_and_bias_id = changeIdType(parameter_id, IdType::ImuStates);
...
void ImuEstimatorBase::downWeightImuResidualBlock(const ceres::ResidualBlockId residual_id, double factor)
{
    ...
    std::shared_ptr<ImuError> imu_error = std::dynamic_pointer_cast<ImuError>(interface);
