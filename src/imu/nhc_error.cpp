...
bool NHCError::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
bool NHCError::EvaluateWithMinimalJacobians(double const *const *parameters, double *residuals, double **jacobians,
                                            double **jacobians_minimal) const
{
    ...
    // weigh it
    Eigen::Matrix3d J_q_full = T_WS.getRotationMatrix().transpose() * skewSymmetric(v_WS_W);
    Eigen::Matrix<double, 2, 3> J_q = J_lower * J_q_full;
    Eigen::Matrix3d covariance_q = Eigen::Matrix3d::Identity() * square(attitude_std_);
    ...
    if (jacobians != nullptr)
    {
        if (jacobians[0] != nullptr)
        {
            ...
            J0_full.setZero();
            J0_full.bottomRightCorner(3, 3) = T_WS.getRotationMatrix().transpose() * skewSymmetric(v_WS_W);
        ...
        {
            Eigen::Map<Eigen::Matrix<double, 2, 9, Eigen::RowMajor>> J1(jacobians[1]);
            ...
            if (jacobians_minimal != nullptr)
            {
                ...
                {
                    Eigen::Map<Eigen::Matrix<double, 2, 9, Eigen::RowMajor>> J1_minimal_mapped(jacobians_minimal[1]);
