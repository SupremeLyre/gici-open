...
// Handle GNSS data
void GnssDataIntegration::handleGNSS(const std::string &formator_tag, const std::shared_ptr<DataCluster::GNSS> &gnss)
{
    // Check if initialized
    if (gnss_local_ == nullptr || gnss_local_->ephemeris == NULL || gnss_local_->observation == NULL ||
        gnss_local_->antenna == NULL)
        return;
    ...
    for (size_t i = 0; i < behaviors_.at(formator_tag).size(); i++)
    {
        ...
        if (roles[i] == GnssRole::Rover || roles[i] == GnssRole::Reference || roles[i] == GnssRole::Heading)
            role_out = roles[i];
    }
    ...
    for (auto it : gnss->types)
    {
        ...
        if (it == GnssDataType::SSR)
        {
            ...
            gnss_common::updateSsr(gnss->ephemeris->ssr, gnss_local_, update_types, false);
            for (auto it_role : roles)
            {
                if (it_role == GnssRole::CodeBias)
                    updateCodeBias();
    ...
    for (auto it : gnss->types)
        if (it == GnssDataType::Observation)
    ...
    for (auto it : roles)
        if (it == GnssRole::Rover || it == GnssRole::Reference || it == GnssRole::Heading)
            has_observation_role = true;
    if (!has_observation_role)
        return;

    // Set to epoch data
    GnssMeasurement epoch;
    epoch.timestamp = gnss_common::gpsTimeToUtcTime(gnss_common::gtimeToDouble(gnss->observation->data[0].time));
    epoch.role = role_out;
    ...
    double invalid_ephemeris_ratio = getDivide(num_invalid_ephemeris, num_invalid_ephemeris + num_valid_ephemeris);
    ...
    if (role_out == GnssRole::Reference)
    {
        ...
        epoch.position = Eigen::Map<Eigen::Vector3d>(gnss_local_->antenna->pos);
    }

    // GPS ionosphere parameters
    epoch.ionosphere_parameters = Eigen::Map<Eigen::VectorXd>(gnss_local_->ephemeris->ion_gps, 8);
...
// Data callback
void ImuDataIntegration::dataCallback(const std::string &input_tag, const std::shared_ptr<DataCluster> &data)
{
    if (data->imu && valid_)
    {
        mutex_.lock();
...
// Handle IMU data
void ImuDataIntegration::handleIMU(const std::string &formator_tag, const std::shared_ptr<DataCluster::IMU> &imu)
{
    // Get role
    if (behaviors_.find(formator_tag) == behaviors_.end())
    {
    ...
    // Convert IMU data
    ImuMeasurement epoch(imu->time, Eigen::Map<Eigen::Vector3d>(imu->angular_velocity),
                         Eigen::Map<Eigen::Vector3d>(imu->acceleration));

    // Call INS processor
    for (auto it_imu_callback : estimator_callbacks_)
    {
        EstimatorDataCluster estimator_data(epoch, roles[0], formator_tag.substr(4, formator_tag.size() - 4));
        it_imu_callback(estimator_data);
    }
}

// Data callback
void ImageDataIntegration::dataCallback(const std::string &input_tag, const std::shared_ptr<DataCluster> &data)
{
                                       ...
                                       const std::shared_ptr<DataCluster::Image> &image)
{
    ...
    {
        EstimatorDataCluster estimator_data(image_mat, roles[0], formator_tag.substr(4, formator_tag.size() - 4),
                                            image->time);
...
SolutionDataIntegration::SolutionDataIntegration(const std::shared_ptr<EstimatingBase> &estimating,
                                                 const std::shared_ptr<EstimatingBase> &input_estimating,
                                                 const std::string &input_tag, const std::vector<std::string> &roles)
{
...
// Data callback
void SolutionDataIntegration::dataCallback(const std::string &input_tag, const std::shared_ptr<DataCluster> &data)
{
...
void SolutionDataIntegration::handleSolution(const std::string &input_tag, const std::shared_ptr<Solution> &solution)
{
    // Get role
    ...
    {
        EstimatorDataCluster estimator_data(*solution, roles[0], input_tag.substr(4, input_tag.size() - 4));
        it_solution_callback(estimator_data);
    }
}

// Check and adjust solution role
SolutionRole SolutionDataIntegration::adjustSolutionRole(const Solution &solution, const SolutionRole &role)
{
    if (role == SolutionRole::None)
        return role;
    ...
    if (role == SolutionRole::Pose)
    {
        ...
        if (position_valid && attitude_valid)
            ...
            return SolutionRole::Position;
        else if (attitude_valid)
            return SolutionRole::Attitude;
        else
            return SolutionRole::None;
    }
    ...
    if (role == SolutionRole::PoseAndVelocity)
    {
        ...
        bool velocity_valid = checkVelocity(solution);
        bool attitude_valid = checkAttitude(solution);
        if (position_valid && velocity_valid && attitude_valid)
            return role;
        else if (position_valid && attitude_valid)
            return SolutionRole::Pose;
        else if (position_valid && velocity_valid)
            return SolutionRole::PositionAndVelocity;
        else if (position_valid)
            return SolutionRole::Position;
        else if (velocity_valid)
            return SolutionRole::Velocity;
