...
FileReaderBase::FileReaderBase(const YAML::Node &node) : fd_(NULL), current_timestamp_(0.0), initial_timestamp_(0.0)
{
    // Type check
    ...
    if (!option_tools::safeGet(node, "latency", &latency_))
        latency_ = 0.0;
    // format control
    ...
    if (formatTypeValid(format_type_str))
        formator_ = makeFormator(format_node);
    else
        formator_ = nullptr;
...
bool FileReaderBase::get(DataClusterPair &data, const double timestamp)
{
...
{
#define SET_TIMESTAMP(t) current_timestamp_ = current_timestamp_ < t ? t : current_timestamp_;
...
{
    return (type == "gnss-rtcm-2" || type == "gnss-rtcm-3" || type == "gnss-raw" || type == "gnss-rinex" ||
            type == "image-v4l2" || type == "image-pack" || type == "imu-pack" || type == "option" || type == "nmea" ||
            type == "dcb-file" || type == "atx-file" || type == "imu-text" || type == "sp3-file" || type == "clk-file");
}
...
bool ImuTextReader::load()
{
    ...
    while (!feof(fd_))
    {
        if (fgets((char *)buf_, max_buf_size_, fd_) == NULL)
            break;

        // Check if header
        if (strstr((char *)buf_, "Timestamp") != NULL)
            continue;

        // Decode data
        std::shared_ptr<DataCluster> data = std::make_shared<DataCluster>(FormatorType::IMUText);
        auto &imu = data->imu;
        sscanf((char *)buf_, "%lf %lf %lf %lf %lf %lf %lf\n", &imu->time, &imu->acceleration[0], &imu->acceleration[1],
               &imu->acceleration[2], &imu->angular_velocity[0], &imu->angular_velocity[1], &imu->angular_velocity[2]);
        double timestamp = burst_load_ ? 0.0 : getTimestamp(data);
        data_buf_.push_back(std::make_pair(timestamp, data));
        has_new_data = true;
...
bool Sp3FileReader::load()
{
    ...
    std::shared_ptr<DataCluster> data = std::make_shared<DataCluster>(FormatorType::Sp3File);
...
bool ClkFileReader::load()
{
    ...
    std::shared_ptr<DataCluster> data = std::make_shared<DataCluster>(FormatorType::ClkFile);
    nav_t *nav = data->gnss->ephemeris;
    nav->ne = nav->nemax = 0;
    nav->nc = nav->ncmax = 0;
    fclose(fd_);
...
#define MAP_FILE_READER(Type, Reader)                                                                                  \
    if (type == Type)                                                                                                  \
    {                                                                                                                  \
        return std::make_shared<Reader>(node);                                                                         \
    }
