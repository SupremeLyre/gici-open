...
DataCluster::DataCluster(FormatorType type)
{
    if (type == FormatorType::RTCM2 || type == FormatorType::RTCM3 || type == FormatorType::GnssRaw ||
        type == FormatorType::RINEX || type == FormatorType::DcbFile || type == FormatorType::AtxFile ||
...
// Decode stream to data
int RINEXFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    // Clear old informations and get GNSS data handle
    std::vector<std::shared_ptr<DataCluster::GNSS>> gnss_data;
    for (size_t i = 0; i < data_.size(); i++)
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        *p_memory_ = 0x00;
        ...
        if (!header_decoded_)
        {
            if (readrnxh(fp_memory_, &rnx_.ver, &rnx_.type, &rnx_.sys, &rnx_.tsys, rnx_.tobs, &rnx_.nav, &rnx_.sta))
            {
                // handle new header data
                int ret[] = {static_cast<int>(GnssDataType::IonAndUtcPara), static_cast<int>(GnssDataType::AntePos)};
                gnss_common::updateStreamData(ret[0], obs, nav, sta, NULL, iobs, *sat, gnss_data);
                gnss_common::updateStreamData(ret[1], obs, nav, sta, NULL, iobs, *sat, gnss_data);
                GnssDataType type[] = {static_cast<GnssDataType>(ret[0]), static_cast<GnssDataType>(ret[1])};
                std::shared_ptr<DataCluster::GNSS> &gnss = gnss_data[0];
                for (size_t k = 0; k < 2; k++)
                    if (std::find(gnss->types.begin(), gnss->types.end(), type[k]) == gnss->types.end())
                    {
        ...
        {
            int n, flag;
            if ((n = readrnxobsb(fp_memory_, rnx_.opt, rnx_.ver, &rnx_.tsys, rnx_.tobs, &flag, rnx_.obs.data,
                                 &rnx_.sta)) > 0)
            {
        ...
        else
        {
            ...
            if ((stat = readrnxnavb(fp_memory_, rnx_.opt, rnx_.ver, rnx_.sys, &type, &eph, &geph, &seph)) > 0)
            {
                ...
                { /* other ephemeris */
                    ...
                    if (sys == SYS_GAL)
                    {
                        ...
                        if (sel == 0 && !(eph.code & (1 << 9)))
                            valid = false;
                        if (sel == 1 && !(eph.code & (1 << 8)))
                            valid = false;
                    }
                    if (valid)
                    {
