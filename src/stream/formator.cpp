...
void DataCluster::GNSS::init()
{
    if (!(observation = (obs_t *)malloc(sizeof(obs_t))) ||
        ...
        !(ephemeris = (nav_t *)malloc(sizeof(nav_t))) || !(antenna = (sta_t *)malloc(sizeof(sta_t))))
    {
        free();
    }
    ...
    eph_t eph0 = {0, -1, -1};
    geph_t geph0 = {0, -1};
    for (int i = 0; i < MAXSAT * 2; i++)
        ephemeris->eph[i] = eph0;
    for (int i = 0; i < NSATGLO * 2; i++)
...
extern void updateObservation(obs_t *obs, std::shared_ptr<DataCluster::GNSS> &gnss_data)
{
    int n = 0;
...
// Update ephemeris
extern void updateEphemeris(nav_t *nav, int sat, std::shared_ptr<DataCluster::GNSS> &gnss_data)
{
...
extern void updateIonAndUTC(nav_t *nav, std::shared_ptr<DataCluster::GNSS> &gnss_data)
{
...
extern void updateAntennaPosition(sta_t *sta, std::shared_ptr<DataCluster::GNSS> &gnss_data)
{
    if (sta == NULL)
    {
...
extern void updateSsr(ssr_t *ssr, std::shared_ptr<DataCluster::GNSS> &gnss_data, std::vector<UpdateSsrType> type,
                      bool reset_ssr_status)
{
    if (ssr == NULL)
    {
    ...
    for (int i = 0; i < MAXSAT; i++)
    {
        ...
        // update ephemeris
        if (std::find(type.begin(), type.end(), UpdateSsrType::Ephemeris) != type.end())
        {
        ...
        // update code bias
        if (std::find(type.begin(), type.end(), UpdateSsrType::CodeBias) != type.end() && ssr[i].t0[4].time)
        {
            gnss_data->ephemeris->ssr[i].t0[4] = ssr[i].t0[4];
            gnss_data->ephemeris->ssr[i].udi[4] = ssr[i].udi[4];
            memcpy(gnss_data->ephemeris->ssr[i].cbias, ssr[i].cbias, sizeof(float) * MAXCODE);
            gnss_data->ephemeris->ssr[i].isdcb = ssr[i].isdcb;
        }
        // update phase bias
        if (std::find(type.begin(), type.end(), UpdateSsrType::PhaseBias) != type.end() && ssr[i].t0[5].time)
        {
            gnss_data->ephemeris->ssr[i].t0[5] = ssr[i].t0[5];
            gnss_data->ephemeris->ssr[i].udi[5] = ssr[i].udi[5];
            memcpy(gnss_data->ephemeris->ssr[i].pbias, ssr[i].pbias, sizeof(double) * MAXCODE);
            memcpy(gnss_data->ephemeris->ssr[i].stdpb, ssr[i].stdpb, sizeof(float) * MAXCODE);
...
extern void updateStreamData(int ret, obs_t *obs, nav_t *nav, sta_t *sta, ssr_t *ssr, int iobs, int sat,
...
inline bool loadStartTime(YAML::Node &node, double &start_time)
{
    ...
    int hour = 12;
    int min = 0, sec = 0;
    double ep[] = {(double)year, (double)month, (double)day, (double)hour, (double)min, (double)sec};
...
int RTCM2Formator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        gnss_common::updateStreamData(ret, obs, nav, sta, ssr, iobs, sat, gnss_data);
        GnssDataType type = static_cast<GnssDataType>(ret);
        std::shared_ptr<DataCluster::GNSS> &gnss = type == GnssDataType::Observation ? gnss_data[iobs] : gnss_data[0];
...
// Decode stream to data
int RTCM3Formator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    // Clear old informations and get GNSS data handle
    std::vector<std::shared_ptr<DataCluster::GNSS>> gnss_data;
    for (size_t i = 0; i < data_.size(); i++)
...
// Encode data to stream
int RTCM3Formator::encode(const std::shared_ptr<DataCluster> &data, uint8_t *buf)
{
...
// Decode stream to data
int GnssRawFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    // Clear old informations and get GNSS data handle
    std::vector<std::shared_ptr<DataCluster::GNSS>> gnss_data;
    for (size_t i = 0; i < data_.size(); i++)
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        sta_t *sta = &raw_.sta;
        int sat = raw_.ephsat;
        gnss_common::updateStreamData(ret, obs, nav, sta, NULL, iobs, sat, gnss_data);
        GnssDataType type = static_cast<GnssDataType>(ret);
        std::shared_ptr<DataCluster::GNSS> &gnss = type == GnssDataType::Observation ? gnss_data[iobs] : gnss_data[0];
        if (std::find(gnss->types.begin(), gnss->types.end(), type) == gnss->types.end())
        {
...
ImageV4L2Formator::ImageV4L2Formator(Option &option)
{
    ...
    data_.push_back(std::make_shared<DataCluster>(FormatorType::ImageV4L2, option.width, option.height, option.step));
}
...
// Decode stream to data
int ImageV4L2Formator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    int ret = input_image_v4l2(&image_, buf, size);
    if (ret <= 0)
        return 0;

    memcpy(data_[0]->image->image, image_.image, sizeof(uint8_t) * image_.width * image_.height * image_.step);
    data = data_;
...
ImagePackFormator::ImagePackFormator(Option &option)
{
    ...
    {
        data_.push_back(
            std::make_shared<DataCluster>(FormatorType::ImagePack, option.width, option.height, option.step));
    }
}
...
// Decode stream to data
int ImagePackFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    int iobs = 0;
    for (int i = 0; i < size; i++)
    {
        ...
        // a memory leak.
        memcpy(data_[iobs]->image->image, image_.image, sizeof(uint8_t) * image_.width * image_.height * image_.step);
...
// Encode data to stream
int ImagePackFormator::encode(const std::shared_ptr<DataCluster> &data, uint8_t *buf)
{
    img_t *image;
    init_img(image, data->image->width, data->image->height, data->image->step);
    image->time = gnss_common::doubleToGtime(data->image->time);
    memcpy(image->image, data->image->image, data->image->width * data->image->height * data->image->step);
...
// Decode stream to data
int IMUPackFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    int n_data = 0;
    data.clear();
    for (int i = 0; i < size; i++)
...
// Decode stream to data
int OptionFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    return 0;
}
...
// Decode stream to data
int NmeaFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
...
int NmeaFormator::encodeGGA(const Solution &solution, uint8_t *buf)
{
    ...
    p += sprintf(p,
                 ...
                 pos[0] >= 0 ? "N" : "S", dms2[0], dms2[1] + dms2[2] / 60.0, pos[1] >= 0 ? "E" : "W", solq, sol.ns, dop,
                 pos[2] - h, h, sol.age, refid);
    for (q = (char *)buf + 1, sum = 0; *q; q++)
        sum ^= *q; /* check-sum */
    p += sprintf(p, "*%02X\r\n", sum);
    return p - (char *)buf;
...
int NmeaFormator::encodeRMC(const Solution &solution, uint8_t *buf)
{
    ...
    p += sprintf(p,
                 ...
                 option_.talker_id.data(), ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0,
                 pos[0] >= 0 ? "N" : "S", dms2[0], dms2[1] + dms2[2] / 60.0, pos[1] >= 0 ? "E" : "W", vel / KNOT2M, dir,
                 ep[2], ep[1], (int)ep[0] % 100, amag, emag, mode, status);
    for (q = (char *)buf + 1, sum = 0; *q; q++)
        sum ^= *q; /* check-sum */
    p += sprintf(p, "*%02X\r\n", sum);
    return p - (char *)buf;
...
int NmeaFormator::encodeESA(const Solution &solution, uint8_t *buf)
{
    ...
    p +=
        sprintf(p,
                ...
                "%+.3f,%+.3f,%+.3f",
                option_.talker_id.data(), ep[3], ep[4], ep[5], sol.rr[3], sol.rr[4], sol.rr[5], rpy[0], rpy[1], rpy[2]);
    for (q = (char *)buf + 1, sum = 0; *q; q++)
        sum ^= *q; /* check-sum */
    p += sprintf(p, "*%02X\r\n", sum);
    return p - (char *)buf;
...
int NmeaFormator::encodeESD(const Solution &solution, uint8_t *buf)
{
    ...
    time = gpst2utc(sol.time);
    time2epoch(time, ep);
    p += sprintf(p, "$%sESD,%02.0f%02.0f%06.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", option_.talker_id.data(),
                 ep[3], ep[4], ep[5], std_p[0], std_p[1], std_p[2], std_v[0], std_v[1], std_v[2], std_a[0], std_a[1],
                 std_a[2]);
    for (q = (char *)buf + 1, sum = 0; *q; q++)
        sum ^= *q; /* check-sum */
    p += sprintf(p, "*%02X\r\n", sum);
...
void NmeaFormator::convertSolution(const Solution &solution, sol_t &sol)
{
    ...
    rr = solution.coordinate->convert(solution.pose.getPosition(), GeoType::ENU, GeoType::ECEF);
...
// Decode stream to data
int DcbFileFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        else
        {
            if (passed_header_ && line_.substr(1, 3) == "DSB")
            {
                ...
                if (std::find(getGnssSystemList().begin(), getGnssSystemList().end(), system) !=
                    getGnssSystemList().end())
                {
    ...
    if (finished_reading_)
    {
        std::shared_ptr<DataCluster> data_cluster = std::make_shared<DataCluster>(FormatorType::DcbFile);
        ...
        for (auto prn : prns)
        {
            ...
            std::vector<Dcb> dcbs;
            for (auto dcb = dcbs_.lower_bound(prn); dcb != dcbs_.upper_bound(prn); dcb++)
            {
...
// Decode stream to data
int AtxFileFormator::decode(const uint8_t *buf, int size, std::vector<std::shared_ptr<DataCluster>> &data)
{
    const pcv_t pcv0 = {0};
    double neu[3];
    ...
    {
        std::shared_ptr<DataCluster> data_cluster = std::make_shared<DataCluster>(FormatorType::AtxFile);
        nav_t *ephemeris = data_cluster->gnss->ephemeris;
