...
NodeHandle::NodeHandle(const NodeOptionHandlePtr &nodes)
{
    ...
    if (!nodes->replay_options.IsDefined() || !option_tools::safeGet(nodes->replay_options, "enable", &enable_replay))
    {
    ...
    for (auto estimating : estimatings_)
    {
        ...
        for (size_t i = 0; i < input_tags.size(); i++)
        {
            ...
            if (option_tools::sensorType(roles[0]) == SensorType::GNSS)
            {
                ...
                if (!found)
                    gnss_streamings.push_back(streaming_from_formator);
            }
            else if (option_tools::sensorType(roles[0]) == SensorType::IMU)
            {
                ...
                if (!found)
                    imu_streamings.push_back(streaming_from_formator);
            }
            else if (option_tools::sensorType(roles[0]) == SensorType::Camera)
            {
                ...
                if (!found)
                    image_streamings.push_back(streaming_from_formator);
            }
            else if (option_tools::sensorType(roles[0]) == SensorType::GeneralSolution)
            {
                ...
                if (!found)
                    solution_streamings.push_back(streaming_from_formator);
            }
        }
    ...
    for (auto estimating : estimatings_)
    {
        ...
        for (size_t i = 0; i < output_tags.size(); i++)
        {
            ...
            EstimatingBase::OutputDataCallback out_callback =
                std::bind(&Streaming::outputDataCallback, stream.get(), std::placeholders::_1, std::placeholders::_2);
...
void NodeHandle::bindEstimatorToEstimator(const NodeOptionHandlePtr &nodes)
{
    for (size_t k = 0; k < estimatings_.size(); k++)
    {
        ...
        for (size_t i = 0; i < input_tags.size(); i++)
        {
            ...
            {
                if (estimating_input->getTag() != input_tag)
                    continue;
                data_integrations.push_back(
                    std::make_shared<SolutionDataIntegration>(estimating, estimating_input, input_tag, roles));
            }
        }
    }
}
