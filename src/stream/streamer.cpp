...
void StreamerBase::syncStreams()
{
    ...
    {
        if (static_this_[i]->stream_.type != STR_FILE)
            continue;
        if (static_this_[i]->stream_.port == NULL)
            continue;
...
int SerialStreamer::open(StreamerRWType type)
{
    ...
    path << option_.port << ":" << option_.baudrate << ":" << option_.parity << ":" << option_.stop_bit << ":"
...
int FileStreamer::open(StreamerRWType type)
{
    ...
    {
#if 1
        path << "::T::+" << replay_options_.start_offset << "::x" << replay_options_.speed
             << "::S=" << option_.swap_interval << "::P=4";
#else
        path << "::T::+" << replay_options_.start_offset << "::x" << replay_options_.speed
             << "::S=" << option_.swap_interval << "::P=8";
#endif
    }
...
{
    std::stringstream path;
    path << ":" << option_.passward << "@" << option_.ip << ":" << option_.port << "/" << option_.mountpoint << ":";
...
{
    std::stringstream path;
    path << option_.username << ":" << option_.passward << "@" << option_.ip << ":" << option_.port << "/"
         << option_.mountpoint;
...
int V4l2Streamer::open(StreamerRWType type)
{
    ...
    {
        LOG(ERROR) << "V4L2 device open failed!";
        return 0;
    }
    ...
    {
        LOG(ERROR) << "V4L2 VIDIOC_TRY_FMT failed!";
        return 0;
    }
    ...
    {
        LOG(ERROR) << "V4L2 VIDIOC_S_FMT failed!";
        return 0;
    }
    ...
    if (ioctl(dev_, VIDIOC_REQBUFS, &req) != 0 || req.count < option_.buffer_count)
    {
        LOG(ERROR) << "V4L2 VIDIOC_REQBUFS failed!";
        return 0;
    }
    ...
    for (int i = 0; i < static_cast<int>(req.count); i++)
    {
        ...
        {
            LOG(ERROR) << "V4L2 VIDIOC_QUERYBUF failed!";
            return 0;
        }
        v4l2_buf[i] = (uint8_t *)mmap(NULL, buffer.length, PROT_READ | PROT_WRITE, MAP_SHARED, dev_, buffer.m.offset);
        if (v4l2_buf[i] == MAP_FAILED)
        {
            LOG(ERROR) << "V4L2 mmap failed!";
            return 0;
        }
        ...
        {
            LOG(ERROR) << "V4L2 VIDIOC_QBUF failed!";
            return 0;
        }
    }
    ...
    {
        LOG(ERROR) << "V4L2 VIDIOC_STREAMON failed!";
        return 0;
    }
...
int V4l2Streamer::read(uint8_t *buf, int max_size)
{
    ...
    if (ret != 0)
    {
        LOG(ERROR) << "V4L2 VIDIOC_DQBUF failed!";
        return 0;
    }
...
int V4l2Streamer::write(uint8_t *buf, int size)
{
    ...
    // Desired brightness in AGC and AEC mode
        ...
        ctrl.id = V4L2_CID_USER_BASE | 0x1002;
    // Gain
    ...
    {
        LOG(ERROR) << "V4L2 VIDIOC_S_CTRL failed!";
        return 0;
    }
...
#define MAP_STREAMER(Type, Streamer)                                                                                   \
    if (type == Type)                                                                                                  \
    {                                                                                                                  \
        return std::make_shared<Streamer>(node);                                                                       \
    }
