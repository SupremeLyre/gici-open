...
Streaming::Streaming(const NodeOptionHandlePtr &nodes, size_t i_streamer) : valid_(false), opened_(false)
{
    // Get streamer option
    ...
    if (formator_tags.size() > 0)
    {
        ...
        for (auto &tag : formator_tags)
        {
            NodeOptionHandle::FormatorNodeBasePtr formator_node =
                std::static_pointer_cast<NodeOptionHandle::FormatorNodeBase>(nodes->tag_to_node.at(tag));
            ...
            if (type == StreamIOType::Input)
                has_input_ = true;
            if (type == StreamIOType::Log)
                has_logging_ = true;
            if (type == StreamIOType::Output)
                has_output_ = true;
        }
    }
    ...
    for (const auto &tag : node->output_tags)
    {
        if (tag.substr(0, 4) == "str_")
            output_streamer_tags.push_back(tag);
    }
    ...
    {
        if (tag.substr(0, 4) == "str_")
            input_streamer_tags.push_back(tag);
    }
    ...
    if ((has_input_ && !(buf_input_ = (uint8_t *)malloc(sizeof(uint8_t) * max_buf_size_))) ||
        (has_logging_ && !(buf_logging_ = (uint8_t *)malloc(sizeof(uint8_t) * max_buf_size_))) ||
        (has_output_ && !(buf_output_ = (uint8_t *)malloc(sizeof(uint8_t) * max_buf_size_))))
    {
        free(buf_input_);
        free(buf_logging_);
        free(buf_output_);
    ...
    // Open streamer
    StreamerRWType rw;
    if (has_logging_ || has_output_)
        rw = StreamerRWType::Write;
    if (has_input_)
        rw = StreamerRWType::Read;
...
void Streaming::pipelineConvertCallback(const std::string &tag, const std::shared_ptr<DataCluster> &data)
{
    // Find formator
    std::shared_ptr<FormatorBase> formator = nullptr;
    for (size_t i = 0; i < formators_.size(); i++)
    {
        if (formators_[i].type != StreamIOType::Log)
            continue;
        if (formators_[i].input_tag != tag)
            continue;
...
void Streaming::outputDataCallback(const std::string tag, const std::shared_ptr<DataCluster> &data)
{
    // Find formator
    std::shared_ptr<FormatorBase> formator = nullptr;
    for (size_t i = 0; i < formators_.size(); i++)
    {
        if (formators_[i].type != StreamIOType::Output)
            continue;
        if (formators_[i].input_tag != tag)
            continue;
        formator = formators_[i].formator;
...
void Streaming::bindLogWithInput()
{
    ...
    for (size_t i = 0; i < static_this_.size(); i++)
        ...
        {
            if (!static_this_[i]->has_logging_)
                continue;
            auto &streaming_log = static_this_[i];
            if (!(streaming_log->formators_[j].type == StreamIOType::Log))
                continue;
            ...
            for (size_t m = 0; m < static_this_.size(); m++)
                ...
                {
                    if (static_this_[m]->formators_[n].type != StreamIOType::Input)
                        continue;
                    if (static_this_[m]->formators_[n].tag != input_tag)
                        continue;
                    auto &streaming_in = static_this_[m];
                    // name the log_tag to encode the data
                    PipelineConvert pipeline = std::bind(&Streaming::pipelineConvertCallback, streaming_log,
                                                         std::placeholders::_1, std::placeholders::_2);
                    streaming_in->pipelines_convert_[input_tag].insert(std::make_pair(log_tag, pipeline));
                }
        }
    ...
    for (size_t i = 0; i < static_this_.size(); i++)
    {
        ...
        {
            if (!static_this_[j]->has_input_)
                continue;
            if (static_this_[j]->tag_ != input_tag)
                continue;
            auto &streaming_in = static_this_[j];
            PipelineDirect pipeline = std::bind(&Streaming::pipelineDirectCallback, streaming_log,
                                                std::placeholders::_1, std::placeholders::_2);
...
void Streaming::processInput()
{
    ...
    if (buf_size_input_ == 0)
        return;

    // Decode stream
    for (size_t i = 0; i < formators_.size(); i++)
    {
        if (formators_[i].type != StreamIOType::Input)
            continue;
        ...
        for (int iobs = 0; iobs < nobs; iobs++)
        {
            ...
            if (it_i == pipelines_convert_.end())
                continue;
            if (it_i->second.size() == 0)
                continue;
