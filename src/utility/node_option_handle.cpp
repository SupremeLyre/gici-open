...
NodeOptionHandle::NodeOptionHandle(const YAML::Node &yaml_node) : valid(true)
{
    ...
    if (yaml_node["stream"].IsDefined())
    {
        ...
        if (stream_node["streamers"].IsDefined())
        {
            ...
            for (size_t i = 0; i < streamer_nodes.size(); i++)
            {
                ...
                StreamerNodeBasePtr streamer = std::make_shared<StreamerNodeBase>(streamer_node);
        ...
        if (stream_node["formators"].IsDefined())
        {
            ...
            for (size_t i = 0; i < formator_nodes.size(); i++)
            {
                ...
                FormatorNodeBasePtr formator = std::make_shared<FormatorNodeBase>(formator_node);
    ...
    if (yaml_node["estimate"].IsDefined())
    {
        ...
        for (size_t i = 0; i < estimator_nodes.size(); i++)
        {
            ...
            EstimatorNodeBasePtr estimator = std::make_shared<EstimatorNodeBase>(estimator_node);
    ...
    for (size_t i = 0; i < nodes.size(); i++)
    {
        ...
        for (auto &output_tag : nodes[i]->output_tags)
        {
            if (output_tag.substr(0, 4) == "fmt_")
                n_out_fmts++;
            if (output_tag.substr(0, 4) == "str_")
                n_out_strs++;
            if (output_tag.substr(0, 4) == "est_")
                n_out_ests++;
        ...
        for (auto &input_tag : nodes[i]->input_tags)
        {
            if (input_tag.substr(0, 4) == "fmt_")
                n_in_fmts++;
            if (input_tag.substr(0, 4) == "str_")
                n_in_strs++;
            if (input_tag.substr(0, 4) == "est_")
                n_in_ests++;
        ...
        {
            if (nodes[i]->type == "post-file")
                has_post_file = true;
            if (n_out_ests > 0 && (nodes[i]->type != "ros" && nodes[i]->type != "post-file"))
            {
                LOG(ERROR) << nodes[i]->tag << ": "
                           << "Only ROS streamer or post file is allowed to output to estimator!";
                valid = false;
                return;
            }
            if (n_in_strs > 1)
        ...
        {
            FormatorNodeBasePtr formator = std::static_pointer_cast<FormatorNodeBase>(nodes[i]);
...
NodeOptionHandle::NodeBase::NodeBase(const YAML::Node &yaml_node) : valid(true)
{
...
NodeOptionHandle::StreamerNodeBase::StreamerNodeBase(const YAML::Node &yaml_node) : NodeBase(yaml_node)
{
    if (!valid)
        return;
...
NodeOptionHandle::FormatorNodeBase::FormatorNodeBase(const YAML::Node &yaml_node) : NodeBase(yaml_node)
{
    if (!valid)
        return;
...
// Constructor for FormatorNodeBase
NodeOptionHandle::EstimatorNodeBase::EstimatorNodeBase(const YAML::Node &yaml_node) : NodeBase(yaml_node)
{
    if (!valid)
        return;
...
bool NodeOptionHandle::tagExists(const std::vector<std::string> &list, const std::string &tag)
{
    for (auto t : list)
    {
        if (t == tag)
