...
template <typename InType, typename OutType> void convert(const InType &in, OutType &out)
{
    LOG(FATAL) << "Convertion from " << typeid(in).name() << " to " << typeid(out).name() << " not supported!";
}

template <> void convert<std::string, StreamerType>(const std::string &in, StreamerType &out)
{
...
template <> void convert<std::string, FormatorType>(const std::string &in, FormatorType &out)
{
...
template <> void convert<std::string, GnssRawFormats>(const std::string &in, GnssRawFormats &out)
{
...
template <> void convert<std::string, StreamIOType>(const std::string &in, StreamIOType &out)
{
...
template <> void convert<std::string, GnssRole>(const std::string &in, GnssRole &out)
{
...
template <> void convert<std::string, ImuRole>(const std::string &in, ImuRole &out)
{
...
template <> void convert<std::string, CameraRole>(const std::string &in, CameraRole &out)
{
...
template <> void convert<std::string, EstimatorType>(const std::string &in, EstimatorType &out)
{
...
template <> void convert<std::string, SolutionRole>(const std::string &in, SolutionRole &out)
{
...
template <> void convert<std::string, DetectorType>(const std::string &in, DetectorType &out)
{
...
template <> void convert<std::string, VisualInitializerType>(const std::string &in, VisualInitializerType &out)
{
...
template <> void convert<std::string, ceres::LinearSolverType>(const std::string &in, ceres::LinearSolverType &out)
{
...
template <>
void convert<std::string, ceres::TrustRegionStrategyType>(const std::string &in, ceres::TrustRegionStrategyType &out)
{
...
#define LOAD_COMMON(opt)                                                                                               \
    ...
    {                                                                                                                  \
        LOG(INFO) << __FUNCTION__ << ": Unable to load " << #opt << ". Using default instead.";                        \
    }
...
inline bool checkSubOption(YAML::Node &node, std::string subname, bool fatal = false)
{
...
inline void delete_space(std::string &strs)
{
    ...
    strs.erase(strs.find_last_not_of(' ') + 1, strs.size() - strs.find_last_not_of(' ') - 1);
}
...
// Load options
template <typename OptionType> void loadOptions(YAML::Node &node, OptionType &options)
{
    LOG(FATAL) << "Loading " << typeid(options).name() << " not supported!";
}

template <> void loadOptions<GnssCommonOptions>(YAML::Node &node, GnssCommonOptions &options)
{
    ...
    if (option_tools::safeGet(node, "min_SNR", &min_SNR) && min_SNR.size() == 2)
    {
    ...
    if (option_tools::safeGet(node, "receiver_pco", &receiver_pco) && receiver_pco.size() == 3)
    {
...
template <> void loadOptions<GnssErrorParameter>(YAML::Node &node, GnssErrorParameter &options)
{
    ...
    if (option_tools::safeGet(node, "relative_position", &relative_position) && relative_position.size() == 3)
    {
        for (size_t i = 0; i < 3; i++)
        {
    ...
    if (option_tools::safeGet(node, "relative_velocity", &relative_velocity) && relative_velocity.size() == 3)
    {
        for (size_t i = 0; i < 3; i++)
        {
...
template <> void loadOptions<AmbiguityResolutionOptions>(YAML::Node &node, AmbiguityResolutionOptions &options)
{
    LOAD_COMMON(min_elevation);
...
template <> void loadOptions<ImuParameters>(YAML::Node &node, ImuParameters &options)
{
    LOAD_COMMON(a_max);
    LOAD_COMMON(g_max);
...
template <> void loadOptions<DetectorOptions>(YAML::Node &node, DetectorOptions &options)
{
...
template <> void loadOptions<FeatureTrackerOptions>(YAML::Node &node, FeatureTrackerOptions &options)
{
...
template <> void loadOptions<VisualInitializationOptions>(YAML::Node &node, VisualInitializationOptions &options)
{
...
template <> void loadOptions<FeatureHandlerOptions>(YAML::Node &node, FeatureHandlerOptions &options)
{
...
template <> void loadOptions<EstimatorBaseOptions>(YAML::Node &node, EstimatorBaseOptions &options)
{
    ...
    if (option_tools::safeGet(node, "trust_region_strategy_type", &trust_region_strategy_type))
    {
    ...
    {
        if (options.solver_type != ceres::SPARSE_SCHUR && options.solver_type != ceres::DENSE_SCHUR &&
            options.solver_type != ceres::DENSE_QR && options.solver_type != ceres::SPARSE_NORMAL_CHOLESKY)
        {
            LOG(FATAL) << "Ceres solver do not support " << solver_type << " for " << trust_region_strategy_type;
        }
    }

    std::vector<double> initial_global_position;
    if (option_tools::safeGet(node, "initial_global_position", &initial_global_position) &&
...
template <> void loadOptions<GnssEstimatorBaseOptions>(YAML::Node &node, GnssEstimatorBaseOptions &options)
{
...
template <> void loadOptions<GnssLooseEstimatorBaseOptions>(YAML::Node &node, GnssLooseEstimatorBaseOptions &options)
{
    LOAD_COMMON(use_outlier_rejection);
...
template <> void loadOptions<ImuEstimatorBaseOptions>(YAML::Node &node, ImuEstimatorBaseOptions &options)
{
    ...
    if (option_tools::safeGet(node, "body_to_imu_rotation", &body_to_imu_rotation) && body_to_imu_rotation.size() == 3)
    {
        for (size_t i = 0; i < 3; i++)
        {
...
template <> void loadOptions<SppEstimatorOptions>(YAML::Node &node, SppEstimatorOptions &options)
{
    LOAD_COMMON(estimate_velocity);
    LOAD_COMMON(use_dual_frequency);
}

template <> void loadOptions<PppEstimatorOptions>(YAML::Node &node, PppEstimatorOptions &options)
{
...
template <> void loadOptions<RtkEstimatorOptions>(YAML::Node &node, RtkEstimatorOptions &options)
{
    LOAD_COMMON(max_window_length);
    LOAD_COMMON(use_ambiguity_resolution);
    LOAD_COMMON(estimate_velocity);
...
template <> void loadOptions<SdgnssEstimatorOptions>(YAML::Node &node, SdgnssEstimatorOptions &options)
{
    LOAD_COMMON(estimate_velocity);
    LOAD_COMMON(max_age);
}

template <> void loadOptions<DgnssEstimatorOptions>(YAML::Node &node, DgnssEstimatorOptions &options)
{
    LOAD_COMMON(estimate_velocity);
    LOAD_COMMON(max_age);
}

template <> void loadOptions<GnssImuLcEstimatorOptions>(YAML::Node &node, GnssImuLcEstimatorOptions &options)
{
    LOAD_COMMON(max_window_length);
}

template <> void loadOptions<SppImuTcEstimatorOptions>(YAML::Node &node, SppImuTcEstimatorOptions &options)
{
    LOAD_COMMON(max_window_length);
}

template <> void loadOptions<RtkImuTcEstimatorOptions>(YAML::Node &node, RtkImuTcEstimatorOptions &options)
{
    LOAD_COMMON(max_window_length);
}

template <> void loadOptions<PppImuTcEstimatorOptions>(YAML::Node &node, PppImuTcEstimatorOptions &options)
{
    LOAD_COMMON(max_window_length);
}

template <> void loadOptions<GnssImuInitializerOptions>(YAML::Node &node, GnssImuInitializerOptions &options)
{
    LOAD_COMMON(max_iteration);
    LOAD_COMMON(num_threads);
    LOAD_COMMON(max_solver_time);
    ...
    if (option_tools::safeGet(node, "gnss_extrinsics", &gnss_extrinsics) && gnss_extrinsics.size() == 3)
    {
        for (size_t i = 0; i < 3; i++)
        {
    ...
    if (option_tools::safeGet(node, "gnss_extrinsics_initial_std", &gnss_extrinsics_initial_std) &&
...
template <>
void loadOptions<GnssImuCameraSrrEstimatorOptions>(YAML::Node &node, GnssImuCameraSrrEstimatorOptions &options)
{
...
template <>
void loadOptions<SppImuCameraRrrEstimatorOptions>(YAML::Node &node, SppImuCameraRrrEstimatorOptions &options)
{
    LOAD_COMMON(max_keyframes);
    LOAD_COMMON(max_gnss_window_length_minor);
    LOAD_COMMON(min_yaw_std_init_visual);
...
template <> void loadOptions<VisualEstimatorBaseOptions>(YAML::Node &node, VisualEstimatorBaseOptions &options)
{
    ...
    if (option_tools::safeGet(node, "camera_extrinsics_initial_std", &camera_extrinsics_initial_std) &&
...
template <typename OptionTypeIn, typename OptionTypeOut> void copyOptions(const OptionTypeIn &in, OptionTypeOut &out)
{
    LOG(FATAL) << "Copying from " << typeid(in).name() << " to " << typeid(out).name() << " is not supported!";
}

template <> void copyOptions(const PppEstimatorOptions &in, SppEstimatorOptions &out)
{
