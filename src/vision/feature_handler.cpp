...
FeatureHandler::FeatureHandler(const FeatureHandlerOptions &options, const ImuEstimatorBaseOptions &imu_options)
    : options_(options), cams_(options.cameras), map_(new Map()), local_scale_initialized_(false),
      speed_and_bias_timestamp_(0.0), speed_and_bias_(SpeedAndBias::Zero()), global_scale_initialized_(false)
{
    // initialize modules
    detector_ = feature_detection_utils::makeDetector(options.detector, cams_->getCameraShared(0));
...
// Add images
bool FeatureHandler::addImageBundle(const std::vector<std::shared_ptr<cv::Mat>> &imgs, const double timestamp)
{
    if (!isFirstFrame())
    {
        ...
        {
            LOG(WARNING) << "Dropping frame: timestamp older than last frame of id " << lastFrames()->getBundleId();
            return false;
        }
    }
    ...
    {
        frames.push_back(std::make_shared<Frame>(cams_->getCameraShared(i), imgs[i]->clone(),
                                                 static_cast<int64_t>(timestamp * 1.0e9),
                                                 options_.max_pyramid_level + 1));
...
bool FeatureHandler::addImageBundle(const std::vector<std::shared_ptr<cv::Mat>> &imgs, const double timestamp,
                                    const std::vector<Transformation> &T_WSs)
{
    if (!addImageBundle(imgs, timestamp))
        return false;
...
bool FeatureHandler::processImageBundle()
{
    ...
    {
        while (frame_bundles_.front()->at(0)->id() < map_->getOldsestKeyframe()->id())
        {
            frame_bundles_.pop_front();
        }
    }
...
void FeatureHandler::setPoseAndAdjust(const double timestamp, const Transformation &T_WS)
{
    ...
    for (auto it = it_start; it != frame_bundles_.end(); it++, index++)
    {
        ...
        for (size_t i = 0; i < frame->landmark_vec_.size(); i++)
        {
            ...
            if (landmark->in_ba_graph_)
                continue;
            // just initialized landmarks
            if (!isSeed(frame->type_vec_[i]))
                continue;
            // adjusted
            if (adjusted_ids.find(landmark->id()) != adjusted_ids.end())
                continue;

            // adjust landmark position
            Eigen::Vector3d t_C = (frame->T_cam_imu() * old_poses[index].inverse()) * landmark->pos();
...
bool FeatureHandler::needKeyFrame(const FramePtr &last_keyframe, const FramePtr &frame)
{
    // Get last keyframe
    ...
    {
        LOG(INFO) << "Select new keyframe by number: " << n_tracked_fts << " vs " << options_.kfselect_min_numkfs;
        return true;
    }
    ...
    {
        const double a =
            Quaternion::log(frame->T_f_w_.getRotation() * keyframe->T_f_w_.getRotation().inverse()).norm() * 180 / M_PI;
        const double d = (frame->pos() - keyframe->pos()).norm();
        if (!(a < options_.kfselect_min_angle && d < options_.kfselect_min_dist_metric))
        {
            LOG(INFO) << "Select new keyframe by motivation: angle = " << a << ", distance = " << d;
            return true;
        }
    }
    ...
    else
    {
        ...
        {
            LOG(INFO) << "Select new keyframe by disparity: " << disparity << " vs " << options_.kfselect_min_disparity;
            return true;
        }
    }

    // Check time duration
    const double time_duration = getCurrent(frame_bundles_)->at(0)->getTimestampSec() - keyframe->getTimestampSec();
    if (time_duration > options_.kfselect_min_dt)
    {
        LOG(INFO) << "Select new keyframe by time duration: " << time_duration << " vs " << options_.kfselect_min_dt;
        return true;
    }
...
void FeatureHandler::detectFeatures(const FramePtr &frame)
{
    ...
    detector_->detect(frame->img_pyr_, frame->getMask(), max_n_features, new_px, new_scores, new_levels, new_grads,
                      new_types);
...
void FeatureHandler::trackFeatures()
{
    ...
    {
        tracker_->track(getLast(frame_bundles_)->at(0), getCurrent(frame_bundles_)->at(0), detector_->grid_, q_cur_ref);
    }
    else
    {
        tracker_->track(getLast(frame_bundles_)->at(0), getCurrent(frame_bundles_)->at(0), detector_->grid_);
    }
...
void FeatureHandler::optimizePose()
{
    ...
    std::shared_ptr<ceres::LossFunction> loss_function = std::make_shared<ceres::CauchyLoss>(1);
    ...
    if (frame_bundles_.size() > 2)
    {
        ...
        Eigen::Vector3d t_last_cur =
            last_frame->T_imu_world().getRotation().rotate(last_frame->imuPos() - last_last_frame->imuPos());
    ...
    for (size_t kp_idx = 0; kp_idx < frame->numFeatures(); ++kp_idx)
    {
        ...
        std::shared_ptr<HomogeneousPointParameterBlock> landmark_parameter_block =
            std::make_shared<HomogeneousPointParameterBlock>(landmark->pos(), landmark_id.asInteger());
        CHECK(graph->addParameterBlock(landmark_parameter_block, Graph::HomogeneousPoint));
        graph->setParameterBlockConstant(landmark_id.asInteger());

        // Add reprojection errors
        Eigen::Matrix2d information = Eigen::Matrix2d::Identity();
        information *= 1.0 / static_cast<double>(1 << frame->level_vec_(kp_idx));
        // we do not believe not-in-graph landmarks, relatively.
        if (global_scale_initialized_ && !landmark->in_ba_graph_)
            information /= 1.0e4;

        std::shared_ptr<ReprojectionError> reprojection_error =
            std::make_shared<ReprojectionError>(frame->cam(), frame->px_vec_.col(kp_idx), information);
        graph->addResidualBlock(reprojection_error, loss_function.get(), graph->parameterBlockPtr(pose_id.asInteger()),
    ...
    if (num_valid_features < 10)
    {
        LOG(WARNING) << "Insufficient tracked features to estimate "
                     << "current pose! num_valid_features = " << num_valid_features << " " << frame->numFeatures();
    ...
    std::shared_ptr<PoseParameterBlock> block_ptr =
        std::static_pointer_cast<PoseParameterBlock>(graph->parameterBlockPtr(pose_id.asInteger()));
...
double FeatureHandler::getDisparity(const FramePtr &ref_frame, const FramePtr &cur_frame)
{
    std::vector<std::pair<size_t, size_t>> matches_ref_cur;
...
void FeatureHandler::initializeLandmarks(const FramePtr &keyframe)
{
    ...
    for (size_t i = 0; i < frame->numFeatures(); i++)
    {
        ...
        if (frame->type_vec_[i] == FeatureType::kOutlier)
            continue;
        ...
        if (ref_frame == nullptr)
            continue;
        if (ref_frame->id() == frame->id())
            continue;

        // check disparity
        double disparity = (ref_frame->px_vec_.col(index_ref) - frame->px_vec_.col(i)).norm();
        if (disparity < options_.min_disparity_init_landmark)
            continue;

        Transformation T_cur_ref = frame->T_f_w_ * ref_frame->T_f_w_.inverse();

        // check translation
        if (T_cur_ref.getPosition().norm() < options_.min_translation_init_landmark)
            continue;
        ...
        if (angle < options_.min_parallax_angle_init_landmark)
            continue;

        // apply traingulation
        double depth;
        if (!(depthFromTriangulation(T_cur_ref, f_ref, f_cur, &depth) == FeatureMatcher::MatchResult::kSuccess))
            continue;
        if (depth < 0.1)
            continue;
...
bool FeatureHandler::processFrame()
{
    ...
    // Select keyframe
    if (!needKeyFrame(map_->getLastKeyframe(), curFrame()))
        return true;
