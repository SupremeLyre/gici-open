...
void FeatureTracker::track(const FramePtr &ref_frame, const FramePtr &cur_frame, OccupandyGrid2D &grid,
                           const Eigen::Quaterniond &q_cur_ref)
{
    ...
    {
        ref_points.push_back(cv::Point2f(ref_frame->px_vec_.col(i)[0], ref_frame->px_vec_.col(i)[1]));
    }

    std::vector<unsigned char> status;
    if (options_.use_relative_rotation && !checkEqual(q_cur_ref.coeffs(), Eigen::Quaterniond::Identity().coeffs()))
    {
        ...
        cv::calcOpticalFlowPyrLK(
            ref_frame->img_pyr_[0], cur_frame->img_pyr_[0], ref_points, cur_points, status, cv::noArray(),
            cv::Size(options_.window_size[0], options_.window_size[1]), options_.max_level,
            cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, options_.max_count, options_.epsilon),
    ...
    cv::findFundamentalMat(ref_bearings, cur_bearings, cv::FM_RANSAC,
                           options_.ransac_threshold / cur_frame->cam()->errorMultiplier(), options_.ransac_confidence,
                           status);
    ...
    for (int i = 0; i < num_tracked; i++)
    {
        ...
        if (ref_frame->type_vec_[j] == FeatureType::kOutlier)
            continue;

        if (!mask.empty() &&
            mask.at<uint8_t>(static_cast<int>(cur_points[j].y), static_cast<int>(cur_points[j].x)) == 0)
        {
            continue;
        }
        ...
        if (!grid.isOccupied(grid_index))
        {
            ...
            cur_frame->px_vec_.col(index) = Eigen::Vector2d(cur_points[j].x, cur_points[j].y);
    ...
    frame_utils::computeBearingVectors(cur_frame->px_vec_, *cur_frame->cam(), &cur_frame->f_vec_);
}

// Predict initial point for optical flow tracking
void FeatureTracker::predictFeatureTracking(const FramePtr &ref_frame, const FramePtr &cur_frame,
                                            const Eigen::Quaterniond &q_cur_ref,
                                            ...
                                            std::vector<cv::Point2f> &pre_points)
{
    ...
    if (pre_points.size() != ref_points.size())
        pre_points.resize(ref_points.size());
