...
VisualEstimatorBase::VisualEstimatorBase(const VisualEstimatorBaseOptions &options,
...
BackendId VisualEstimatorBase::addCameraExtrinsicsParameterBlock(const int32_t id, const Transformation &T_SC_prior)
{
    ...
    std::shared_ptr<PoseParameterBlock> camera_extrinsic_parameter_block =
        std::make_shared<PoseParameterBlock>(T_SC_prior, camera_extrinsics_id.asInteger());
    ...
    for (size_t kp_idx = 0; kp_idx < frame->numFeatures(); ++kp_idx)
    {
        ...
        if (!isLandmarkInEstimator(createLandmarkId(point->id())))
        {
            ...
            std::shared_ptr<HomogeneousPointParameterBlock> point_parameter_block =
                std::make_shared<HomogeneousPointParameterBlock>(point->pos(), landmark_backend_id.asInteger());
            CHECK(graph_->addParameterBlock(point_parameter_block, Graph::HomogeneousPoint));

            // add landmark to map
            auto it_landmark = landmarks_map_.emplace_hint(landmarks_map_.end(), landmark_backend_id, MapPoint(point));
                ...
                if (FramePtr f = obs->frame.lock())
                {
                    if (f->isKeyframe())
                    {
                        ...
                        for (auto s = states_.rbegin(); s != states_.rend(); s++)
                        {
                            if (!s->valid())
                                continue;
                            if (s->id != createNFrameId(f->bundleId()))
                                continue;
                            state = *s;
                            break;
                        }
...
ceres::ResidualBlockId VisualEstimatorBase::addReprojectionErrorResidualBlock(const State &state, const FramePtr &frame,
                                                                              const size_t keypoint_idx)
{
    ...
    {
        information /=
            square(visual_base_options_.feature_error_std * static_cast<double>(1 << frame->level_vec_(keypoint_idx)));
    }

    // create error term
    std::shared_ptr<ReprojectionError> reprojection_error =
        std::make_shared<ReprojectionError>(frame->cam(), frame->px_vec_.col(keypoint_idx), information);
    ceres::ResidualBlockId ret_val =
        graph_->addResidualBlock(reprojection_error, cauchy_loss_function_ ? cauchy_loss_function_.get() : nullptr,
    ...
    landmarks_map_.at(landmark_backend_id)
        .observations.insert(std::pair<KeypointIdentifier, uint64_t>(kid, reinterpret_cast<uint64_t>(ret_val)));
...
void VisualEstimatorBase::addReprojectionErrorResidualBlocks(const State &state, const FramePtr &frame)
{
    for (size_t kp_idx = 0; kp_idx < frame->numFeatures(); ++kp_idx)
    {
        const PointPtr &point = frame->landmark_vec_[kp_idx];
...
void VisualEstimatorBase::addCameraExtrinsicsResidualBlock(const BackendId &camera_extrinsics_id,
                                                           const Transformation &T_SC_prior,
                                                           const Eigen::Vector3d &std_translation,
                                                           const Eigen::Vector3d &std_orientation)
{
    ...
    std::shared_ptr<PoseError> pose_error = std::make_shared<PoseError>(T_SC_prior, information);
    ceres::ResidualBlockId residual_id =
        graph_->addResidualBlock(pose_error, nullptr, graph_->parameterBlockPtr(camera_extrinsics_id.asInteger()));
}
...
{
    const double outlier_threshold = visual_base_options_.landmark_outlier_rejection_threshold;
    ...
    for (auto it = landmarks_map_.begin(); it != landmarks_map_.end();)
    {
        ...
        for (auto obs : map_point.observations)
        {
            ...
            graph_->problem()->EvaluateResidualBlock(residual_block_id, false, nullptr, Residuals.data(), nullptr);
    ...
    if (base_options_.verbose_output && rejected_landmarks.size() > 0)
    {
        ...
        {
            LOG(INFO) << "Rejected landmark outlier " << std::fixed << residual_vec[i].first
...
void VisualEstimatorBase::addLandmarkParameterMarginBlocksWithResiduals(const State &state, bool keep)
{
    ...
    for (auto frame = active_keyframes_.begin(); frame != active_keyframes_.end(); frame++)
    {
        if ((*frame)->bundleId() != pose_id.bundleId())
            continue;
        active_keyframes_.erase(frame);
        break;
    }
}

// Erase landmark observation
void VisualEstimatorBase::eraseReprojectionErrorResidualBlock(ceres::ResidualBlockId residual_block_id)
{
...
void VisualEstimatorBase::eraseReprojectionErrorResidualBlocks(const State &state)
{
    ...
    for (size_t r = 0; r < residuals.size(); ++r)
    {
        ...
        const Graph::ParameterBlockCollection parameters = graph_->parameters(residuals[r].residual_block_id);
        const BackendId landmark_id(parameters.at(1).first);
        MapPoint &map_point = landmarks_map_.at(landmark_id);
...
void VisualEstimatorBase::updateLandmarks()
{
    ...
    {
        // update coordinates
        id_and_map_point.second.hom_coordinates = std::static_pointer_cast<HomogeneousPointParameterBlock>(
...
void VisualEstimatorBase::updateFrameStateToFrontend(const State &state, const FramePtr &frame)
{
...
Transformation VisualEstimatorBase::getCameraExtrinsicsEstimate()
{
    std::shared_ptr<PoseParameterBlock> block_ptr =
        std::static_pointer_cast<PoseParameterBlock>(graph_->parameterBlockPtr(camera_extrinsics_id_.asInteger()));
