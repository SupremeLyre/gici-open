...
AbstractVisualInitialization::AbstractVisualInitialization(const VisualInitializationOptions &init_options,
...
bool AbstractVisualInitialization::trackFeaturesAndCheckDisparity(const FrameBundlePtr &frames)
{
    ...
    for (size_t i = 0; i < ref_frames_->frames_.size(); i++)
    {
        ...
        disparity.push_back(getDisparityPercentile(ref_frames_->frames_.at(i), frames->frames_.at(i),
...
bool AbstractVisualInitialization::triangulateAndInitializePoints(
    const FramePtr &frame_cur, const FramePtr &frame_ref, const Transformation &T_cur_ref,
    const double reprojection_threshold, const size_t min_inliers_threshold,
    std::vector<std::pair<size_t, size_t>> &matches_cur_ref, const double depth_at_current_frame)
{
    Positions points_in_cur;
    triangulatePoints(*frame_cur, *frame_ref, T_cur_ref, reprojection_threshold, matches_cur_ref, points_in_cur);
    ...
    rescaleAndInitializePoints(frame_cur, frame_ref, matches_cur_ref, points_in_cur, T_cur_ref, depth_at_current_frame);

    return true;
}

void AbstractVisualInitialization::triangulatePoints(const Frame &frame_cur, const Frame &frame_ref,
                                                     ...
                                                     Positions &points_in_cur)
{
    ...
    for (size_t i = 0; i < matches_cur_ref.size(); ++i)
    {
        ...
        const Position xyz_in_cur =
            vk::triangulateFeatureNonLin(T_cur_ref.getRotationMatrix(), T_cur_ref.getPosition(), f_cur, f_ref);
...
void AbstractVisualInitialization::rescaleAndInitializePoints(
    const FramePtr &frame_cur, const FramePtr &frame_ref, const std::vector<std::pair<size_t, size_t>> &matches_cur_ref,
    const Positions &points_in_cur, const Transformation &T_cur_ref, const double depth_at_current_frame)
{
    ...
    frame_cur->T_f_w_.getPosition() =
        -frame_cur->T_f_w_.getRotation().rotate(frame_ref->pos() + scale * (frame_cur->pos() - frame_ref->pos()));
...
int AbstractVisualInitialization::estimateFundamental(const Bearings &f_cur, const Bearings &f_ref,
                                                      ...
                                                      Transformation &T_cur_ref)
{
    ...
    cv::Mat essential = cv::findEssentialMat(ref_pts, cur_pts, camera_matrix, cv::FM_RANSAC, 0.999, thresh);

    // recover pose
    cv::Mat R_cv, t_cv;
    int inlier_cnt = cv::recoverPose(essential, ref_pts, cur_pts, camera_matrix, R_cv, t_cv);
    Eigen::Vector3d t;
    Eigen::Matrix3d R;
...
VisualInitResult HomographyVisualInit::addFrameBundle(const FrameBundlePtr &frames_cur)
{
    ...
    // Compute model
    const vk::Homography H_cur_ref = vk::estimateHomography(f_cur, f_ref, ref_frames_->at(0)->getErrorMultiplier(),
                                                            options_.reproj_error_thresh, options_.init_min_inliers);
    if (H_cur_ref.score < options_.init_min_inliers)
    {
        LOG(WARNING) << "Init Homography: Have " << H_cur_ref.score << "inliers. " << options_.init_min_inliers
                     << " inliers minimum required.";
        return VisualInitResult::kFailure;
    }
    Transformation T_cur_ref = Transformation(Quaternion(H_cur_ref.R_cur_ref), H_cur_ref.t_cur_ref);

    // Triangulate
    if (triangulateAndInitializePoints(frames_cur->at(0), ref_frames_->at(0), T_cur_ref, options_.reproj_error_thresh,
...
VisualInitResult FundamentalVisualInit::addFrameBundle(const FrameBundlePtr &frames_cur)
{
    // Track and detect features.
    if (!trackFeaturesAndCheckDisparity(frames_cur))
        return VisualInitResult::kTracking;
    ...
    // Compute model
    Transformation T_cur_ref;
    int num_valid = estimateFundamental(f_cur, f_ref, ref_frames_->at(0)->getErrorMultiplier(),
                                        options_.reproj_error_thresh, T_cur_ref);
    if (num_valid < options_.init_min_inliers)
    {
        LOG(WARNING) << "Init Fundamental: Have " << num_valid << " inliers. " << options_.init_min_inliers
                     << " inliers minimum required.";
        return VisualInitResult::kFailure;
    }
