...
int main(int argc, char **argv)
{
    ...
    // Load options
    NodeOptionHandlePtr nodes = std::make_shared<NodeOptionHandle>(yaml_node);
    if (!nodes->valid)
    {
    ...
    if (yaml_node["rosbags"].IsDefined())
    {
        ...
        for (size_t i = 0; i < rosbag_nodes.size(); i++)
        {
            ...
            NodeOptionHandle::NodeBasePtr rosbag = std::make_shared<NodeOptionHandle::NodeBase>(rosbag_node);
    ...
    for (size_t i = 0; i < nodes->streamers.size(); i++)
    {
        ...
        NodeOptionHandle::FormatorNodeBasePtr formator_node =
            std::static_pointer_cast<NodeOptionHandle::FormatorNodeBase>(nodes->tag_to_node.at(formator_tags[0]));
    ...
    for (size_t i = 0; i < rosbags.size(); i++)
    {
        ...
        if (option.format == "gnss_raw")
        {
            option_tools::safeGet(rosbag_node, "enable_observation", &option.enable_observation);
            option_tools::safeGet(rosbag_node, "enable_ephemeris", &option.enable_ephemeris);
            option_tools::safeGet(rosbag_node, "enable_antenna_position", &option.enable_antenna_position);
            option_tools::safeGet(rosbag_node, "enable_ionosphere_parameter", &option.enable_ionosphere_parameter);
            option_tools::safeGet(rosbag_node, "enable_ssr_code_bias", &option.enable_ssr_code_bias);
            option_tools::safeGet(rosbag_node, "enable_ssr_phase_bias", &option.enable_ssr_phase_bias);
    ...
    for (size_t i = 0; i < in_files.size(); i++)
    {
        ...
        if (in_to_out_map.find(i) == in_to_out_map.end())
            continue;
        ...
        uint8_t *buf;
        int buffer_length;
        if (!option_tools::safeGet(nodes->streamers[i]->this_node, "buffer_length", &buffer_length))
        {
            LOG(INFO) << nodes->streamers[i]->tag << ": Unable to load buffer length! Using default instead.";
            buffer_length = 32768;
        }
        ...
        while (fread(&in_file->tick_n, sizeof(uint32_t), 1, in_file->fp_tag) &&
               fread((in_file->size_fpos == 4) ? (void *)&fpos_4B : (void *)&fpos_8B, in_file->size_fpos, 1,
                     in_file->fp_tag))
        {
